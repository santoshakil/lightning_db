...
impl BatchMigrationProcessor {
    ...
    async fn process_task(
        ...
        match task.task_type {
            ...
            TaskType::WriteBatch { path, data } => Self::process_write_batch(path, data).await,
            TaskType::VerifyBatch { source, target } => {
                Self::process_verify_batch(source, target).await
            }
            TaskType::CreateCheckpoint { name } => Self::process_create_checkpoint(name).await,
        }
    }

    async fn process_read_batch(path: PathBuf, offset: u64, size: usize) -> MigrationResult<()> {
        use tokio::fs::File;
    ...
    async fn process_write_batch(path: PathBuf, data: Vec<u8>) -> MigrationResult<()> {
    ...
    async fn process_verify_batch(source: Vec<u8>, target: Vec<u8>) -> MigrationResult<()> {
        if source != target {
