...
use super::{
    MigrationVersion, Migration, MigrationContext, MigrationStatus, MigrationStep, HistoryManager, MigrationValidator,
};
...
impl MigrationRunner {
    ...
    fn parse_migration_file(&self, content: &str, path: &Path) -> DatabaseResult<Migration> {
        let filename = path.file_stem()
            .and_then(|s| s.to_str())
            .ok_or_else(|| DatabaseError::MigrationError("Invalid migration filename".to_string()))?;
        ...
        let version = version_str.parse::<u64>()
            .map_err(|_| DatabaseError::MigrationError(
                format!("Invalid version number: {}", version_str)
            ))?;
    ...
    fn parse_migration_content(&self, content: &str) -> DatabaseResult<(String, Option<String>)> {
        ...
        Ok((up_script.trim().to_string(), down_script.map(|s| s.trim().to_string())))
    }
    ...
    pub fn migrate_down(
        ...
        let to_rollback = applied_migrations.into_iter()
            .rev()
            .take(steps)
            .collect::<Vec<_>>();
    ...
    fn validate_migration(
        ...
        if !result.is_valid {
            return Err(DatabaseError::MigrationError(
                format!("Migration validation failed: {:?}", result.errors)
            ));
        }
    ...
    pub fn abort_migration(&self, version: MigrationVersion) -> DatabaseResult<()> {
        ...
        if let Some(status) = active.get(&version) {
            if *status == MigrationStatus::Running {
                ...
                Err(DatabaseError::MigrationError(
                    format!("Migration {} is not currently running", version)
                ))
            }
        } else {
            Err(DatabaseError::MigrationError(
...
impl MigrationScriptStep {
    ...
    fn execute_sql_statement(&self, statement: &str, _ctx: &MigrationContext) -> DatabaseResult<()> {
