...
impl DataTransformationEngine {
    ...
    fn add_index(
        ...
        let _sql = format!(
            "CREATE {}INDEX {} ON ({})",
            unique_clause, name, columns_str
        );
        Ok(())
    }
        ...
        let columns_sql: Vec<String> = schema
            ...
            .map(|col| {
                ...
                let default = col
                    ...
                    .map(|v| format!(" DEFAULT {}", v))
                    .unwrap_or_default();
                let auto_inc = if col.auto_increment {
                    " AUTO_INCREMENT"
                } else {
                    ""
                };
                format!(
                    "{} {}{}{}{}",
                    col.name, col.data_type, nullable, default, auto_inc
                )
            })
            .collect();
...
impl DataMigrator {
    ...
    pub fn migrate_data(
        ...
        for (batch_index, batch) in records.chunks(self.batch_size).enumerate() {
...

            {
                ...
                if let Some(current_version) = progress_guard.current_migration {
                    let step = format!(
                        "Migrating data: batch {} of {}",
                        batch_index + 1,
                        total_records.div_ceil(self.batch_size)
                    );
                    progress_guard.update_migration_progress(
                        current_version,
                        &step,
                        progress.min(100),
                    );
                ...
                if let Some(value) = record.get_field(source_field) {
                    if let Some(transformation) = mapping.transformations.get(target_field) {
                        let transformed_value =
                            self.apply_transformation(value, transformation, record)?;
    ...
    fn apply_transformation(
        ...
        match transformation {
            ...
            FieldTransformation::Split {
                field,
                delimiter,
                index,
            } => self.split_field(record, field, delimiter, *index),
            FieldTransformation::Default { value } => Ok(FieldValue::String(value.clone())),
            FieldTransformation::Lookup {
                table,
                key_field,
                value_field,
            } => self.lookup_value(value, table, key_field, value_field),
            FieldTransformation::Custom {
                function,
                parameters,
    ...
    fn apply_conversion_expression(
        &self,
        _value: &FieldValue,
        _expression: &str,
    ) -> DatabaseResult<FieldValue> {
