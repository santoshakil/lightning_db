...
impl MigrationProgress {
    ...
    pub fn update_migration_progress(
        ...
        let detail = self
            .migration_details
            .entry(version)
            .or_insert_with(|| MigrationProgressDetail::new(version));
    ...
    fn calculate_overall_progress(&mut self) {
        ...
        let overall_progress =
            (self.completed_migrations as f64 / self.total_migrations as f64) * 100.0;
        ...
        if let Some(current_version) = self.current_migration {
            if let Some(current_detail) = self.migration_details.get(&current_version) {
                let current_contribution = (current_detail.progress_percentage as f64 / 100.0)
                    * (1.0 / self.total_migrations as f64)
                    * 100.0;
                self.progress_percentage =
                    (overall_progress + current_contribution).min(100.0) as u8;
            } else {
    ...
    pub fn get_remaining_time(&self) -> Option<Duration> {
        self.estimated_completion
            .and_then(|completion| completion.duration_since(SystemTime::now()).ok())
    }

    pub fn get_migration_detail(
        &self,
        version: MigrationVersion,
    ) -> Option<&MigrationProgressDetail> {
...
impl ProgressTracker {
    ...
    pub fn update_migration(&mut self, version: MigrationVersion, step: &str, percentage: u8) {
...
impl ProgressReporter {
    ...
    pub fn format_progress_summary(progress: &MigrationProgress) -> String {
        ...
        format!(
            ...
            percentage, completed, total, elapsed_str, remaining_str, progress.current_step
        )
    }
    ...
    pub fn format_detailed_report(progress: &MigrationProgress) -> String {
        ...
        for detail in sorted_details {
            report.push_str(&format!(
                "  {} - {:?} ({}%) - {}\n",
                detail.version, detail.status, detail.progress_percentage, detail.current_step
            ));
