use super::{
    MigrationConfig, MigrationContext, MigrationGenerator, MigrationManager,
    MigrationTemplateManager, MigrationVersion, RollbackManager, RollbackStrategy,
};
...
impl MigrationCliRunner {
    ...
    pub fn run(&mut self, cli: MigrationCli) -> DatabaseResult<()> {
        ...
        match cli.command {
            ...
            MigrationCommand::Status => self.show_status(database_path),
            ...
            MigrationCommand::Templates => self.list_templates(),
            MigrationCommand::Reset { version, confirm } => {
                self.reset_migrations(database_path, version, confirm)
            }
            MigrationCommand::Import { file } => self.import_history(database_path, &file),
    ...
    fn create_migration(
        ...
        let file_path =
            MigrationGenerator::save_migration_to_file(&migration, migration_dir, &filename)?;
    ...
    fn run_migrations(
        ...
        let mut ctx =
            MigrationContext::new(database_path.to_string(), current_version, target_version)
        ...
        if let Some(step_count) = steps {
            let pending = self
                .manager
                .get_pending_migrations(current_version, Some(target_version));
            let limited_target = pending
                .get(step_count.saturating_sub(1))
                .map(|m| m.metadata.version)
        ...
        println!(
            "Running migrations from {} to {}",
            current_version, ctx.target_version
        );
        if dry_run {
    ...
    fn rollback_migrations(
        ...
        if let Some(target_version) = version.map(MigrationVersion::new) {
            ...
            self.rollback_manager
                .execute_rollback(&mut ctx, &plan, &HashMap::new())?;
    ...
    fn show_status(&self, database_path: &str) -> DatabaseResult<()> {
        ...
        if !pending.is_empty() {
            ...
            for migration in pending {
                println!(
                    "  {} - {}",
                    migration.metadata.version, migration.metadata.name
                );
            }
            println!();
        }

        let failed_migrations: Vec<_> = statuses
            .iter()
    ...
    fn show_history(
        ...
        for entry in entries {
            println!("{} - {} ({:?})", entry.version, entry.name, entry.status);
    ...
    fn list_templates(&self) -> DatabaseResult<()> {
        ...
        for template_name in templates {
            if let Some(template) = self.template_manager.get_template(template_name) {
                ...
                println!(
                    "  Type: {:?}, Mode: {:?}",
                    template.migration_type, template.mode
                );
