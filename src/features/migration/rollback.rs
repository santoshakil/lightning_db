use super::{HistoryManager, Migration, MigrationContext, MigrationStatus, MigrationVersion};
...
impl RollbackManager {
    ...
    pub fn create_rollback_plan(
        ...
        let applied_migrations =
            self.get_applied_migrations_since(db_path, target_version, current_version)?;

        let migrations_to_rollback =
            self.order_rollback_migrations(&applied_migrations, migrations)?;

        let estimated_duration =
            self.estimate_rollback_duration(&migrations_to_rollback, migrations);

        let backup_required = self.requires_backup(&strategy, &migrations_to_rollback, migrations);

        let pre_rollback_checks =
            self.generate_pre_rollback_checks(&migrations_to_rollback, migrations);

        let post_rollback_actions =
            self.generate_post_rollback_actions(&migrations_to_rollback, migrations);
    ...
    fn execute_staged_rollback(
        ...
        let chunk_size = 5;
        let chunks: Vec<&[MigrationVersion]> =
            plan.migrations_to_rollback.chunks(chunk_size).collect();
        ...
        for version in &plan.migrations_to_rollback {
            if let Some(migration) = migrations.get(version) {
                if let Err(e) = self.rollback_single_migration(ctx, migration) {
                    state
                        .errors
                        .push(format!("Failed to rollback {}: {}", version, e));
                    continue;
                }
                state.last_checkpoint = Some(*version);
            }
    ...
    fn requires_backup(
        ...
        matches!(
            strategy,
            RollbackStrategy::Graceful | RollbackStrategy::Staged
        )
    }
