...
impl MigrationVersion {
    ...
    pub fn from_string(s: &str) -> MigrationResult<Self> {
        ...
        if parts.len() < 3 {
            return Err(MigrationError::VersionConflict(format!(
...
impl MigrationEngine {
    ...
    pub async fn execute_migration(
        ...
        let result = self
            .execute_migration_internal(target_version, context)
            .await;
    ...
    async fn execute_migration_internal(
        ...
        let migration_path =
            self.determine_migration_path(current_version.as_ref(), &target_version, &migrations)?;

        for version in migration_path {
            let migration = migrations.get(&version).ok_or_else(|| {
                MigrationError::VersionConflict(format!(
                    "Migration {} not found",
                    version.to_string()
                ))
            })?;

            self.execute_single_migration(migration.as_ref(), context)
                .await?;
    ...
    fn determine_migration_path(
        ...
        let start_version = current
            .cloned()
            .unwrap_or_else(|| MigrationVersion::new(0, 0, 0));
    ...
    pub async fn rollback_to_version(
        ...
        let migrations = self.migrations.read().await;
        let rollback_path =
            self.determine_rollback_path(&current_version, &target_version, &migrations)?;
    ...
    pub async fn get_pending_migrations(&self) -> Vec<MigrationMetadata> {
        ...
        let start_version = current_version.unwrap_or_else(|| MigrationVersion::new(0, 0, 0));
