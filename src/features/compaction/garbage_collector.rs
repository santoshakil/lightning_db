...
impl GarbageCollector {
    ...
    pub async fn collect(&self) -> Result<u64> {
        ...
        let total_time = stats.avg_collection_time.as_secs_f64() * (stats.total_collections - 1) as f64 + duration.as_secs_f64();
        stats.avg_collection_time = Duration::from_secs_f64(total_time / stats.total_collections as f64);
    ...
    pub async fn get_stats(&self) -> GCStats {
        ...
        let (active_count, old_count) = version_store.values()
    ...
    pub async fn estimate_collectible_bytes(&self) -> Result<u64> {
        ...
        let collectible_bytes = version_store.values()
            .filter(|version| {
                // Can collect if:
    ...
    async fn update_version_watermarks(&self) -> Result<()> {
        ...
        if active_txns.is_empty() {
            ...
            let version_store = self.version_store.read().await;
            let min_active_version = active_txns.iter()
                .filter_map(|txn_id| {
