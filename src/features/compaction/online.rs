...
impl OnlineCompactor {
    ...
    pub async fn compact(&self, compaction_id: u64) -> Result<u64> {
        let _permit =
            self.concurrency_limiter
                ...
                .map_err(|_| Error::ResourceExhausted {
                    resource: "Too many concurrent compactions".into(),
                })?;
        ...
        let result = self
            .perform_online_compaction(compaction_id, progress.clone(), cancel_token)
            .await;
    ...
    pub async fn minor_compact(&self, compaction_id: u64) -> Result<u64> {
        ...
        let result = self
            .perform_minor_compaction(compaction_id, progress.clone(), cancel_token)
            .await;
    ...
    async fn perform_online_compaction(
        ...
        for (phase_name, phase_weight) in phases {
            ...
            match phase_name {
                ...
                "Compacting pages" => {
                    let result = self
                        .compact_pages_online(batch_size, &progress, &cancel_token)
                        .await?;
    ...
    async fn perform_minor_compaction(
        ...
        bytes_reclaimed += self
            .compact_recent_writes(batch_size, &cancel_token)
            .await?;
    ...
    async fn identify_compaction_candidates(
        &self,
        cancel_token: &tokio_util::sync::CancellationToken,
    ) -> Result<u64> {
        if cancel_token.is_cancelled() {
    ...
    async fn update_indexes_online(
        &self,
        cancel_token: &tokio_util::sync::CancellationToken,
    ) -> Result<u64> {
        if cancel_token.is_cancelled() {
    ...
    async fn compact_recent_writes(
        &self,
        batch_size: usize,
        cancel_token: &tokio_util::sync::CancellationToken,
    ) -> Result<u64> {
