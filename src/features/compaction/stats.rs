...
use super::{CompactionConfig, CompactionStats, CompactionType, CompactionState};
...
impl StatsCollector {
    ...
    pub async fn record_compaction(&self, 
        ...
        // Update basic stats
        {
            ...
            stats.avg_compaction_time = Duration::from_secs_f64(total_time / stats.total_compactions as f64);
        }
        
        // Update detailed stats
        self.update_type_stats(compaction_type.clone(), bytes_processed, bytes_reclaimed, duration, state).await?;
        self.update_hourly_stats(bytes_reclaimed, duration).await?;
        self.update_daily_stats(bytes_reclaimed, duration).await?;
        self.update_performance_metrics(bytes_processed, bytes_reclaimed, duration).await?;
    ...
    pub async fn record_error(&self, error_type: String) -> Result<()> {
        ...
        *detailed.error_analysis.error_types.entry(error_type.clone()).or_insert(0) += 1;
        
        // Update most common error
        let max_count = detailed.error_analysis.error_types.values().max().unwrap_or(&0);
        if let Some((error, _count)) = detailed.error_analysis.error_types.iter().find(|(_, &c)| c == *max_count) {
        ...
        if total_operations > 0 {
            detailed.error_analysis.error_rate = detailed.error_analysis.total_errors as f64 / total_operations as f64;
        }
    ...
    pub async fn get_performance_report(&self) -> Result<String> {
        ...
        report.push_str(&format!("Space Reclaimed: {:.2} MB\n", stats.space_reclaimed as f64 / (1024.0 * 1024.0)));
        ...
        report.push_str(&format!("  I/O Efficiency: {:.2}%\n", detailed.performance_metrics.io_efficiency * 100.0));
        report.push_str(&format!("  Compaction Frequency: {:.2}/hour\n", detailed.performance_metrics.compaction_frequency));
        report.push_str(&format!("  Space Amplification: {:.2}x\n\n", detailed.performance_metrics.space_amplification));
        
        // By Type
        report.push_str("By Compaction Type:\n");
        for (comp_type, type_stats) in &detailed.by_type {
            report.push_str(&format!("  {:?}: {} operations, {:.2} MB reclaimed, {:.2}s avg\n",
                comp_type, type_stats.count, type_stats.total_bytes_reclaimed as f64 / (1024.0 * 1024.0),
        ...
        if detailed.performance_metrics.io_efficiency < 0.7 {
            report.push_str("  - Consider tuning compaction thresholds to improve I/O efficiency\n");
        }
        if detailed.performance_metrics.compaction_frequency > 10.0 {
            report.push_str("  - High compaction frequency detected - consider increasing write buffer size\n");
        }
    ...
    async fn update_type_stats(&self, 
        compaction_type: CompactionType, 
        bytes_processed: u64, 
        bytes_reclaimed: u64, 
        duration: Duration,
        ...
        let type_stats = detailed.by_type.entry(compaction_type.clone()).or_insert(TypeStats {
        ...
        let total_time = type_stats.avg_duration.as_secs_f64() * (type_stats.count - 1) as f64 + duration.as_secs_f64();
        ...
        match state {
            ...
            CompactionState::Failed => {
                ...
                type_stats.success_rate = (type_stats.success_rate * (type_stats.count - 1) as f64) / type_stats.count as f64;
            },
            _ => {}
        }
    ...
    async fn update_hourly_stats(&self, bytes_reclaimed: u64, duration: Duration) -> Result<()> {
        ...
        if current_hour < 24 {
            ...
            // Update average duration
            let total_time = hour_stats.avg_duration.as_secs_f64() * (hour_stats.compactions_count - 1) as f64 + duration.as_secs_f64();
            hour_stats.avg_duration = Duration::from_secs_f64(total_time / hour_stats.compactions_count as f64);
        }
    ...
    async fn update_performance_metrics(&self, bytes_processed: u64, bytes_reclaimed: u64, duration: Duration) -> Result<()> {
        let mut detailed = self.detailed_stats.write().await;
        let stats = self.stats.read().await;
        ...
        if duration.as_secs_f64() > 0.0 {
            ...
            detailed.performance_metrics.throughput_mb_per_sec = mb_processed / duration.as_secs_f64();
        }
        
        // Update I/O efficiency
        if bytes_processed > 0 {
            detailed.performance_metrics.io_efficiency = bytes_reclaimed as f64 / bytes_processed as f64;
        }
        
        // Update compaction frequency (compactions per hour)
        let uptime_hours = self.collection_start_time.elapsed().unwrap_or_default().as_secs_f64() / 3600.0;
        if uptime_hours > 0.0 {
            detailed.performance_metrics.compaction_frequency = stats.total_compactions as f64 / uptime_hours;
        }
        
        // Update space amplification (simplified calculation)
        if stats.space_reclaimed > 0 {
            detailed.performance_metrics.space_amplification = stats.bytes_compacted as f64 / stats.space_reclaimed as f64;
        }
    ...
    pub async fn get_trend_analysis(&self, days: u32) -> Result<HashMap<String, f64>> {
        ...
        if recent_days.len() >= 2 {
            ...
            let count_trend = if first.compactions_count > 0 {
                (last.compactions_count as f64 - first.compactions_count as f64) / first.compactions_count as f64 * 100.0
            } else {
                0.0
            };
            ...
            let bytes_trend = if first.bytes_reclaimed > 0 {
                (last.bytes_reclaimed as f64 - first.bytes_reclaimed as f64) / first.bytes_reclaimed as f64 * 100.0
            } else {
                0.0
            };
            ...
            let duration_trend = if first.total_duration.as_secs_f64() > 0.0 {
                (last.total_duration.as_secs_f64() - first.total_duration.as_secs_f64()) / first.total_duration.as_secs_f64() * 100.0
            } else {
                0.0
            };
