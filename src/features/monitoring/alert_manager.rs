...
impl NotificationChannel for WebhookNotificationChannel {
    ...
    fn send_alert(&self, alert: &Alert) -> Result<()> {
        ...
        let payload = serde_json::json!({
            ...
            "labels": alert.labels,
            "timestamp": alert.timestamp.duration_since(UNIX_EPOCH).unwrap_or(Duration::ZERO).as_secs(),
            "state": alert.state,
    ...
    fn send_alert_resolution(&self, alert: &Alert) -> Result<()> {
        ...
        let payload = serde_json::json!({
            ...
            "labels": alert.labels,
            "timestamp": alert.timestamp.duration_since(UNIX_EPOCH).unwrap_or(Duration::ZERO).as_secs(),
            "resolved_at": alert.resolved_at.map(|t| t.duration_since(UNIX_EPOCH).unwrap_or(Duration::ZERO).as_secs()),
