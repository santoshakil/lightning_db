...
impl EncryptionManager {
    ...
    pub fn initialize(&mut self, master_password: &str) -> Result<()> {
        ...
        self.key_store
            .keys
            .insert(initial_key.key_id.clone(), initial_key);
    ...
    pub fn encrypt_backup(&self, data: &[u8]) -> Result<(Vec<u8>, EncryptionInfo)> {
        ...
        // Generate random nonce using cryptographically secure OsRng
        let mut nonce_bytes = vec![0u8; 12]; // 96-bit nonce for GCM
        OsRng.fill_bytes(&mut nonce_bytes);

        // Generate salt for additional security using cryptographically secure OsRng
        ...
        let (encrypted_data, auth_tag) = match key.algorithm {
            ...
            EncryptionAlgorithm::XSalsa20Poly1305 => {
                return Err(Error::Generic(
                    "XSalsa20Poly1305 not yet implemented".to_string(),
                ));
            }
        };
    ...
    pub fn decrypt_backup(
        ...
        let key = self
            .key_store
            .keys
            .get(&encryption_info.key_id)
            .ok_or_else(|| {
                Error::Generic(format!(
                    "Decryption key not found: {}",
                    encryption_info.key_id
                ))
            })?;
        ...
        if let Some(current_key_id) = &self.current_key_id {
            if let Some(current_key) = self.key_store.keys.get(current_key_id) {
                ...
                let rotation_interval =
                    Duration::from_secs(self.config.key_rotation_interval_hours * 3600);
    ...
    pub fn revoke_key(&mut self, key_id: &str, reason: &str) -> Result<()> {
        ...
        key.metadata
            .insert("revoked_at".to_string(), format!("{:?}", SystemTime::now()));

        tracing::info!("Key revoked: {}", reason);
        Ok(())
    }
    ...
    pub fn get_encryption_stats(&self) -> EncryptionStatistics {
        ...
        let total_usage = self.key_store.keys.values().map(|k| k.usage_count).sum();
...
mod tests {
    ...
    fn test_encryption_decryption() {
        ...
        let decrypted_data = manager
            .decrypt_backup(&encrypted_data, &encryption_info)
            .unwrap();
    ...
    fn test_key_rotation() {
        ...
        let new_key_id = manager
            .rotate_key("SecureTestPassword123!", RotationReason::Manual)
            .unwrap();
