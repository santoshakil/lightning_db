...
use super::span::{DatabaseSpanGuard, DatabaseSpans};
...
impl TracedDatabase {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        let context = current_context()
            .unwrap_or_else(|| DatabaseContextPropagation::create_db_context("GET", None));
    ...
    pub fn get_with_context(&self, context: &TraceContext, key: &[u8]) -> Result<Option<Vec<u8>>> {
        with_context(context.clone(), || {
            ...
            let mut span_guard =
                DatabaseSpanGuard::new(DatabaseSpans::get(context, &key_str).build());
            ...
            match self.inner.get(key) {
                Ok(Some(value)) => {
                    ...
                    if let Some(completed) = span_guard.finish() {
                        if completed.duration_micros() > 1000 {
                            // Log slow operations
                            println!(
                                "SLOW GET: {} took {}μs",
                                key_str,
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        let context = current_context()
            .unwrap_or_else(|| DatabaseContextPropagation::create_db_context("PUT", None));
    ...
    pub fn put_with_context(&self, context: &TraceContext, key: &[u8], value: &[u8]) -> Result<()> {
        with_context(context.clone(), || {
            let key_str = String::from_utf8_lossy(key);
            let mut span_guard =
                DatabaseSpanGuard::new(DatabaseSpans::put(context, &key_str, value.len()).build());
            ...
            match self.inner.put(key, value) {
                Ok(_) => {
                    ...
                    if let Some(completed) = span_guard.finish() {
                        if completed.duration_micros() > 5000 {
                            // Log slow writes
                            println!(
                                "SLOW PUT: {} took {}μs",
                                key_str,
    ...
    pub fn delete(&self, key: &[u8]) -> Result<bool> {
        let context = current_context()
            .unwrap_or_else(|| DatabaseContextPropagation::create_db_context("DELETE", None));
    ...
    pub fn delete_with_context(&self, context: &TraceContext, key: &[u8]) -> Result<bool> {
        with_context(context.clone(), || {
            let key_str = String::from_utf8_lossy(key);
            let mut span_guard =
                DatabaseSpanGuard::new(DatabaseSpans::delete(context, &key_str).build());
        ...
        F: FnMut(&[u8], &[u8]) -> Result<bool>,
    {
        let context = current_context().unwrap_or_else(|| {
            DatabaseContextPropagation::create_query_context(
                "SCAN",
                start_key
        ...
        F: FnMut(&[u8], &[u8]) -> Result<bool>,
    {
        with_context(context.clone(), || {
            ...
            let mut span_guard = DatabaseSpanGuard::new(
                DatabaseSpans::scan(context, start_key_str.as_deref(), end_key_str.as_deref())
                    .build(),
            );
            ...
            let result = self
                .inner
                .scan(start_key_vec, end_key_vec)
                .and_then(|iterator| {
                    for result in iterator {
                        ...
                        if scanned_count % 1000 == 0 {
                            // Trace every 1000th record to avoid overhead
                            let child_context = context.create_child();
                            let mut record_span =
                                DatabaseSpans::get(&child_context, &String::from_utf8_lossy(&key))
            ...
            match result {
                Ok(_) => {
                    ...
                    if let Some(completed) = span_guard.finish() {
                        println!(
                            "SCAN completed: {} records, {} bytes in {}μs",
                            scanned_count,
                            total_bytes,
                            completed.duration_micros()
                        );
    ...
    pub fn batch_put(&self, operations: &[(&[u8], &[u8])]) -> Result<()> {
        let context = current_context()
            .unwrap_or_else(|| DatabaseContextPropagation::create_db_context("BATCH_PUT", None));

        self.batch_put_with_context(&context, operations)
    }

    /// Batch operations with explicit trace context
    pub fn batch_put_with_context(
        &self,
        context: &TraceContext,
        operations: &[(&[u8], &[u8])],
    ) -> Result<()> {
        with_context(context.clone(), || {
            ...
            let total_bytes: usize = operations.iter().map(|(k, v)| k.len() + v.len()).sum();
            ...
            for (i, (key, value)) in operations.iter().enumerate() {
                ...
                if operations.len() > 100 && i % 100 == 0 {
                    let child_context = context.create_child();
                    let mut op_span = DatabaseSpans::put(
                        &child_context,
                    ...
                    match self.inner.put(key, value) {
                        ...
                        Err(e) => {
                            op_span.record_error(
                                &format!("Batch operation {} failed: {}", i, e),
                                Some("BATCH_OP_ERROR"),
                            );
                            failed_ops += 1;
                        }
            ...
            if failed_ops == 0 {
                ...
                span_guard.record_error(
                    &format!(
                        "Batch operation had {} failures out of {} operations",
                        failed_ops,
                        operations.len()
                    ),
                    Some("BATCH_PARTIAL_ERROR"),
                );
            }

            if let Some(completed) = span_guard.finish() {
                println!(
                    "BATCH PUT: {}/{} operations successful in {}μs",
                    successful_ops,
                    operations.len(),
                    completed.duration_micros()
                );
            ...
            if failed_ops > 0 {
                Err(crate::core::error::Error::Generic(format!(
...
macro_rules! trace_db_operation {
    ($operation:expr, $context:expr, $body:block) => {{
        ...
        with_context($context.clone(), || {
            let mut span_guard =
                DatabaseSpanGuard::new(DatabaseSpanBuilder::new($context, $operation).build());
...
impl PerformanceMonitor {
    ...
    pub fn check_performance(operation: &str, duration_micros: u64) -> Option<String> {
        ...
        if duration_micros > thresholds.1 {
            Some(format!(
                "PERFORMANCE ERROR: {} took {}μs (threshold: {}μs)",
                operation, duration_micros, thresholds.1
            ))
        } else if duration_micros > thresholds.0 {
            Some(format!(
                "PERFORMANCE WARNING: {} took {}μs (threshold: {}μs)",
