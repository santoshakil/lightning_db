    ...
    fn send_http_request(&self, payload: serde_json::Value) -> Result<()> {
        #[cfg(feature = "http-client")]
        {
            ...
            let response = request
                .json(&payload)
                .send()
                .map_err(|e| crate::core::error::Error::IoError(e.to_string()))?;

            if !response.status().is_success() {
                return Err(crate::core::error::Error::IoError(format!(
        ...
        #[cfg(not(feature = "http-client"))]
        {
            Err(crate::core::error::Error::IoError("HTTP export requires 'http-client' feature".to_string()))
        }
    }
}
...
mod tests {
    ...
    fn test_metrics_exporter() {
        ...
        span2.status = SpanStatus::Error("test error".to_string());
        span2.finish();
