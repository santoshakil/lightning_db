...
impl TraceViewer {
    ...
    pub fn generate_html_viewer(&self, trace_id: &str) -> Result<String> {
        let trace = self
            .get_trace(trace_id)
            .ok_or_else(|| crate::core::error::Error::Generic(format!("Trace {} not found", trace_id)))?;
    ...
    pub fn subscribe(&self, subscriber: Box<dyn TraceSubscriber + Send>) -> Result<()> {
        if let Ok(mut subscribers) = self.subscribers.lock() {
            if subscribers.len() < self.config.max_subscribers {
                ...
                Err(crate::core::error::Error::Generic("Max subscribers reached".to_string()))
            }
        } else {
            Err(crate::core::error::Error::Generic("Failed to acquire lock".to_string()))
        }
    }
...
mod tests {
    use super::*;
    use crate::features::distributed_tracing::TraceContext;
