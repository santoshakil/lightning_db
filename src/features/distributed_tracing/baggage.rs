...
impl BaggageItem {
    ...
    pub fn ttl(&self) -> Option<u64> {
        self.get_metadata("ttl").and_then(|ttl| ttl.parse().ok())
    }

    /// Get priority if set
    pub fn priority(&self) -> Option<u8> {
        self.get_metadata("priority").and_then(|p| p.parse().ok())
    }
}
...
impl Baggage {
    ...
    {
        let mut filtered =
            Baggage::with_limits(self.max_size, self.max_key_length, self.max_value_length);
    ...
    pub fn high_priority_baggage(&self, min_priority: u8) -> Baggage {
        self.filter(|_, item| item.priority().map_or(false, |p| p >= min_priority))
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BaggageError::KeyTooLong(actual, max) => {
                write!(
                    f,
                    "Baggage key too long: {} characters (max: {})",
                    actual, max
                )
            }
            BaggageError::ValueTooLong(actual, max) => {
                write!(
                    f,
                    "Baggage value too long: {} characters (max: {})",
                    actual, max
                )
            }
...
impl BaggageSerializer {
    ...
    pub fn serialize(baggage: &Baggage) -> String {
        baggage
            ...
            .iter()
            .map(|(key, item)| {
                let mut entry = format!("{}={}", percent_encode(key), percent_encode(&item.value));
...
impl DatabaseBaggage {
    ...
    pub fn for_transaction(tx_id: u64, isolation_level: &str) -> Baggage {
        ...
        let _ = baggage.set(
            "db.transaction_id".to_string(),
            BaggageItem::new(tx_id.to_string()).as_system(),
        );
        let _ = baggage.set_value(
            "db.isolation_level".to_string(),
...
fn parse_key_value(input: &str) -> Result<(String, String), BaggageError> {
    if let Some(eq_pos) = input.find('=') {
        ...
        Err(BaggageError::ParseError(format!(
...
mod tests {
    ...
    fn test_baggage_limits() {
        ...
        // Test value too long
        let result = baggage.set_value(
            "key".to_string(),
            "very_long_value_that_exceeds_limit".to_string(),
        );
    ...
    fn test_baggage_serialization() {
        ...
        baggage
            .set_value("key2".to_string(), "value with spaces".to_string())
            .unwrap();
        ...
        assert_eq!(
            deserialized.get_value("key2"),
            Some(&"value with spaces".to_string())
        );
        assert_eq!(
            deserialized.get("key1").unwrap().get_metadata("priority"),
            Some(&"5".to_string())
        );
    }
    ...
    fn test_database_baggage() {
        ...
        assert_eq!(
            op_baggage.get_value("db.operation"),
        ...
        assert_eq!(
            tx_baggage.get_value("db.transaction_id"),
            Some(&"42".to_string())
        );
        assert_eq!(
            tx_baggage.get_value("db.isolation_level"),
            Some(&"READ_COMMITTED".to_string())
        );

        let user_baggage = DatabaseBaggage::for_user("user123", Some("session456"));
        assert_eq!(
            user_baggage.get_value("user.id"),
            Some(&"user123".to_string())
        );
        assert_eq!(
            user_baggage.get_value("user.session_id"),
