...
impl KeyManager {
    ...
    pub fn get_current_key_id(&self) -> Option<u64> {
        self.current_dek_id.read().ok().and_then(|guard| *guard)
    }
    ...
    fn get_next_key_id(&self) -> u64 {
        match self.next_key_id.write() {
            ...
            Err(_) => {
                // If lock is poisoned, use a fallback approach
                use std::sync::atomic::{AtomicU64, Ordering};
                static FALLBACK_ID: AtomicU64 = AtomicU64::new(1);
                FALLBACK_ID.fetch_add(1, Ordering::SeqCst)
            }
        }
    }
    ...
    pub fn clear_keys(&self) {
        ...
        if let Ok(mut kek_cache) = self.kek_cache.write() {
            for (_, mut key) in kek_cache.drain() {
                key.key_material.zeroize();
            }
        }

        if let Ok(mut dek_cache) = self.dek_cache.write() {
