...
use opentelemetry::{
    trace::{Span as OtelSpan, TraceError, Tracer, TracerProvider},
    Context, KeyValue,
};
#[cfg(feature = "telemetry")]
...
impl DatabaseSpan {
    ...
    pub fn set_status(&mut self, status: SpanStatus) {
        match status {
            ...
            SpanStatus::Error(message) => self
                .span
                .set_status(opentelemetry::trace::Status::error(message)),
        }
    }
    ...
    pub fn new() -> Self {
        Self {
            ...
            metrics_collector: Arc::new(
                crate::features::logging::metrics::HistogramCollector::new(),
            ),
        }
    }
    ...
    fn on_end(&self, span: &DatabaseSpanData) {
        ...
        if let Some(duration) = span.duration {
            self.metrics_collector
                .record(&span.operation_name, duration);
        }
...
impl DistributedTracing {
    pub fn new(manager: Arc<TelemetryManager>, correlation_header: String) -> Self {
        Self {
            manager,
            correlation_header,
        }
    ...
    pub fn extract_context_from_headers(
        ...
        if let Some(correlation_id) = headers.get(&self.correlation_header) {
            let mut context = TelemetryContext::new().with_correlation_id(correlation_id.clone());
