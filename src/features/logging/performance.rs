...
impl PerformanceMonitor {
    ...
    pub fn start_operation(&self, name: &str) -> Option<PerformanceToken> {
        ...
        self.overhead_tracker.record_overhead("start_operation", overhead_start.elapsed());
    ...
    pub fn end_operation(&self, name: &str, start_time: Instant) {
        ...
        let timer = timers.entry(name.to_string()).or_insert_with(|| {
            OperationTimer {
                name: name.to_string(),
        ...
        self.overhead_tracker.record_overhead("end_operation", overhead_start.elapsed());
    }
    ...
    pub fn get_operation_stats(&self, name: &str) -> Option<OperationStats> {
        ...
        if let Some(timer) = timers.get(name) {
            ...
            Some(OperationStats::from_samples(name, &samples, timer.total_operations, timer.slow_operations))
        } else {
            None
        }
    }
    ...
    pub fn get_all_operation_stats(&self) -> HashMap<String, OperationStats> {
        ...
        for (name, timer) in timers.iter() {
            ...
            stats.insert(
                name.clone(),
                OperationStats::from_samples(name, &samples, timer.total_operations, timer.slow_operations),
            );
        }
    ...
    pub fn check_alerts(&self) -> Vec<PerformanceAlert> {
        ...
        for (name, stat) in stats {
            if stat.slow_operation_rate > 0.1 { // More than 10% slow operations
                alerts.push(PerformanceAlert {
                    alert_type: AlertType::SlowOperations,
                    message: format!("Operation '{}' has high slow operation rate: {:.2}%", 
                                   name, stat.slow_operation_rate * 100.0),
        ...
        if resource_usage.memory_bytes > self.alert_thresholds.memory_usage_threshold {
            alerts.push(PerformanceAlert {
                ...
                message: format!("High memory usage: {} MB", 
    ...
    fn from_samples(name: &str, samples: &[Duration], total_ops: u64, slow_ops: u64) -> Self {
        if samples.is_empty() {
            return Self {
                ...
                slow_operation_rate: if total_ops > 0 { slow_ops as f64 / total_ops as f64 } else { 0.0 },
...
impl OverheadTracker {
    ...
    fn record_overhead(&self, operation: &str, overhead: Duration) {
        ...
        let entry = samples.entry(operation.to_string()).or_insert_with(VecDeque::new);
    ...
    fn get_stats(&self) -> OverheadStats {
        ...
        for (operation, durations) in samples.iter() {
            if !durations.is_empty() {
                ...
                stats.insert(operation.clone(), OperationOverhead {
