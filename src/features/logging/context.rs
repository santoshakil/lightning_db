...
impl AsyncContextPropagator {
        ...
        F: std::future::Future<Output = R> + Send,
    {
        ...
        self.manager.store_context(context_key.clone(), context.clone());
...
mod tests {
    ...
    fn test_context_from_headers() {
        ...
        headers.insert("x-correlation-id".to_string(), "test-correlation-id".to_string());
        headers.insert("x-user-id".to_string(), "user123".to_string());
        headers.insert("baggage".to_string(), "key1=value1, key2=value2".to_string());
