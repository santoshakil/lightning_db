...
use crate::storage::{Page, PageManager, OptimizedPageManager};
...
impl PageManagerWrapper {
    ...
    pub fn free_page(&self, page_id: u32) {
        match self {
            PageManagerWrapper::Standard(manager) => {
                manager.write().free_page(page_id)
            }
            PageManagerWrapper::Optimized(manager) => {
    ...
    pub fn free_page_count(&self) -> usize {
        match self {
            PageManagerWrapper::Standard(manager) => {
                manager.read().free_page_count()
            }
            PageManagerWrapper::Optimized(manager) => {
                manager.free_page_count()
            }
        }
    }
}
