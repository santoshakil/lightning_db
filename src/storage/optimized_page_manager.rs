...
use crate::storage::{Page, OptimizedMmapManager, MmapConfig, PAGE_SIZE};
...
impl OptimizedPageManager {
    pub fn create<P: AsRef<Path>>(path: P, initial_size: u64, config: MmapConfig) -> Result<Self> {
        ...
        info!("Created optimized page manager with initial size: {} bytes", initial_size);
    ...
    fn init_header_page(&self) -> Result<()> {
        ...
        // Magic number
        header_data[0..4].copy_from_slice(&crate::storage::page::MAGIC.to_le_bytes());
    ...
    fn load_header_page(&self) -> Result<()> {
        ...
        if magic != crate::storage::page::MAGIC {
