...
impl GracefulFile {
    ...
    pub fn create_with_fallback<P: AsRef<Path>>(path: P) -> Result<FileOpenResult> {
            ...
            .open(path)
        {
            ...
            Err(e) => {
                Err(Error::Io(format!("Failed to create file {:?}: {}", path, e)))
            }
        }
    }
    ...
    pub fn sync_data(&self) -> Result<()> {
        ...
        self.file.sync_data().map_err(|e| {
            Error::Io(format!("Sync data failed: {}", e))
        })
    }
    ...
    pub fn set_len(&self, size: u64) -> Result<()> {
        ...
        self.file.set_len(size).map_err(|e| {
            Error::Io(format!("Failed to set file length: {}", e))
        })
    }
}
...
pub fn is_path_read_only<P: AsRef<Path>>(path: P) -> Result<bool> {
    ...
    let metadata = std::fs::metadata(path).map_err(|e| {
        Error::Io(format!("Failed to get metadata for {:?}: {}", path, e))
    })?;
