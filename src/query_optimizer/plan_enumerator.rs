...
use super::{
    CardinalityEstimator, CostModel, IndexUsage, JoinCondition,
    JoinType, LogicalJoin, LogicalQuery, OptimizationMetadata, OptimizerConfig, PhysicalOperator,
    PhysicalPlan, Predicate,
};
#[cfg(test)]
use super::{AggregateFunction, ComparisonOp, SortDirection, SortKey, Value};
...
impl PlanEnumerator {
                ...
                for split_size in 1..size {
                    ...
                    for (left_subset, right_subset) in splits {
                            ...
                            (dp.get(&left_subset), dp.get(&right_subset))
                        {
                            // Create join plan
                            if let Ok(join_plan) =
                                self.create_join_plan(left_plan.clone(), right_plan.clone(), query)
                            {
    ...
    fn enumerate_table_access_plans(
        &self,
        table: &str,
        query: &LogicalQuery,
    ) -> Result<Vec<PhysicalPlan>> {
    ...
    fn build_plan_for_join_order(
        ...
        if join_order.len() == 1 {
            ...
            return Ok(plans.into_iter().next().unwrap_or_else(|| PhysicalPlan {
    ...
    fn create_join_plan(
        ...
        let join_condition =
            self.find_join_condition(&left.join_order, &right.join_order, query)?;
    ...
    fn generate_join_algorithm_variants(
        &self,
...
mod tests {
    ...
    fn test_table_access_plan_generation() {
        ...
        let plans = enumerator
            .enumerate_table_access_plans("users", &query)
            .unwrap();
        assert!(!plans.is_empty());

        // Should have at least table scan plan
        let has_table_scan = plans
            .iter()
            .any(|p| matches!(p.operation, PhysicalOperator::TableScan { .. }));
