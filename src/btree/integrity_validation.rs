use crate::btree::{BTreeNode, NodeType, MIN_KEYS_PER_NODE, MAX_KEYS_PER_NODE};
...
use crate::storage::{Page, PageManagerTrait};
...
impl BTreeIntegrityValidator {
    ...
    fn validate_node_structure(
        ...
        if node.page_id != page_id {
            violations.push(crate::integrity::error_types::create_critical_violation(
        ...
        match node.node_type {
            NodeType::Leaf => {
                if node.entries.len() > MAX_KEYS_PER_NODE {
                    violations.push(crate::integrity::error_types::create_violation(
...
pub mod btree_validators {
    ...
    pub fn quick_validate_node(node: &BTreeNode, page_id: u32) -> ValidationResult<()> {
        ...
        if node.page_id != page_id {
            let violation = crate::integrity::error_types::create_critical_violation(
                IntegrityError::StructuralViolation {
                    structure: "btree_node".to_string(),
        ...
        if node.entries.len() > MAX_KEYS_PER_NODE {
            let violation = crate::integrity::error_types::create_violation(
                IntegrityError::PageCapacityViolation {
    ...
    pub fn validate_key_insertion(
        ...
        let estimated_size = 8 + key.len() + value.len() + 8; // overhead + key + value + timestamp
        if node.entries.len() >= MAX_KEYS_PER_NODE {
            violations.push(crate::integrity::error_types::create_violation(
                IntegrityError::PageCapacityViolation {
