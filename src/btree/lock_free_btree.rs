...
impl LockFreeBTree {
        ...
        loop {
            ...
            if node.meta.is_leaf {
                ...
                if index < key_count {
                    ...
                    if found_key == key {
                        let value =
                            unsafe { node.values.values.as_ref()[index].load(Ordering::Acquire) };
                        return SearchResult {
                            value: Some(value),
                            path,
                        };
                ...
                let child_ptr =
                    unsafe { node.values.children.as_ref()[child_index].load(Ordering::Acquire) };
    ...
    fn update_existing(
        ...
        Err(Error::Transaction(
            "Version mismatch during update".to_string(),
        ))
    }
    ...
    fn insert_into_node(
        ...
        if key_count >= MAX_KEYS || index > key_count {
            return Err(Error::Transaction(format!(
                "Invalid insert: key_count={}, index={}, MAX_KEYS={}",
                key_count, index, MAX_KEYS
            )));
        }
    ...
    fn split_and_insert(
        ...
        for i in split_index..key_count {
            ...
            if node.meta.is_leaf {
                let value_val = unsafe { node.values.values.as_ref()[i].load(Ordering::Acquire) };
                unsafe {
                    new_node.values.values.as_ref()[i - split_index]
                        .store(value_val, Ordering::Release)
                };
            }
        }
        ...
        new_node
            .meta
            .key_count
            .store(key_count - split_index, Ordering::Release);
    ...
    fn insert_into_parent_shared(
        &self,
        path: &[NodePath],
        new_node_shared: Shared<Node>,
        guard: &Guard,
    ) -> Result<()> {
        if path.len() <= 1 {
            ...
            unsafe {
                new_root.values.children.as_ref()[0]
                    .store(old_root.as_raw() as *mut Node, Ordering::Release);
                new_root.values.children.as_ref()[1]
                    .store(new_node_shared.as_raw() as *mut Node, Ordering::Release);
            }
            ...
            if parent_key_count < MAX_KEYS {
                ...
                return Err(crate::Error::Index(
                    "Parent node full - recursive splitting not yet implemented".to_string(),
                ));
            }
        }
    ...
    fn insert_separator_into_parent(
        ...
        for i in (insert_pos..key_count).rev() {
            ...
            let child_ptr =
                unsafe { parent.values.children.as_ref()[i + 1].load(Ordering::Acquire) };
        ...
        unsafe {
            parent.values.children.as_ref()[insert_pos + 1]
                .store(new_child.as_raw() as *mut Node, Ordering::Release);
        }
        ...
        parent
            .meta
            .key_count
            .store(key_count + 1, Ordering::Release);
        ...
        if let Some(leaf_path) = search_result.path.last() {
            ...
            if node.meta.version.load(Ordering::Acquire) != leaf_path.version {
                return Err(Error::Transaction(
                    "Version mismatch during delete".to_string(),
                ));
            }
            ...
            for i in leaf_path.index..key_count - 1 {
                ...
                let value_val =
                    unsafe { node.values.values.as_ref()[i + 1].load(Ordering::Acquire) };
...
impl Node {
    ...
    fn new_leaf() -> Owned<Node> {
        let node = Owned::new(Node {
            ...
            values: NodeValues {
                values: std::mem::ManuallyDrop::new(create_atomic_u64_array()),
            },
    ...
    fn new_internal() -> Owned<Node> {
        let node = Owned::new(Node {
            ...
            keys: create_atomic_u64_array(),
            values: NodeValues {
                children: std::mem::ManuallyDrop::new(create_atomic_ptr_array()),
            },
            next: AtomicPtr::new(ptr::null_mut()),
        });
