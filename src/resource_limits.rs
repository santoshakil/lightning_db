...
impl ResourceEnforcer {
    ...
    pub fn check_write(&self, bytes: u64) -> Result<()> {
        ...
        if current_mem + bytes > self.limits.max_memory_bytes {
            self.record_violation(ViolationType::MemoryLimit, self.limits.max_memory_bytes, current_mem + bytes);
            return Err(Error::ResourceLimitExceeded("Memory limit exceeded".to_string()));
        }
        
        // Check disk limit
        let current_disk = self.current_disk_usage.load(Ordering::Relaxed);
        if current_disk + bytes > self.limits.max_disk_bytes {
            self.record_violation(ViolationType::DiskLimit, self.limits.max_disk_bytes, current_disk + bytes);
            return Err(Error::ResourceLimitExceeded("Disk space limit exceeded".to_string()));
        }
        ...
        if current_throughput + bytes > self.limits.max_write_throughput_bytes_per_sec {
            self.record_violation(ViolationType::WriteThroughputLimit, 
    ...
    pub fn check_read(&self, bytes: u64) -> Result<()> {
        ...
        if current_throughput + bytes > self.limits.max_read_throughput_bytes_per_sec {
            self.record_violation(ViolationType::ReadThroughputLimit,
    ...
    pub fn check_operation_start(&self) -> Result<OperationGuard> {
        ...
        if current >= self.limits.max_concurrent_operations {
            self.record_violation(ViolationType::ConcurrencyLimit,
                                self.limits.max_concurrent_operations as u64,
                                current as u64);
            return Err(Error::ResourceLimitExceeded("Concurrent operation limit exceeded".to_string()));
        }
    ...
    pub fn check_file_open(&self) -> Result<()> {
        ...
        if current >= self.limits.max_open_files {
            self.record_violation(ViolationType::FileLimit,
                                self.limits.max_open_files as u64,
                                current as u64);
            return Err(Error::ResourceLimitExceeded("Open file limit exceeded".to_string()));
        }
    ...
    pub fn record_write(&self, bytes: u64) {
        self.write_throughput_tracker.lock().unwrap().add_bytes(bytes);
        self.current_disk_usage.fetch_add(bytes, Ordering::Relaxed);
    }
    
    /// Record a read operation
    pub fn record_read(&self, bytes: u64) {
        self.read_throughput_tracker.lock().unwrap().add_bytes(bytes);
    }
    ...
    pub fn update_wal_size(&self, bytes: u64) {
        ...
        if bytes > self.limits.max_wal_size_bytes && old_size <= self.limits.max_wal_size_bytes {
            self.record_violation(ViolationType::WalSizeLimit,
    ...
    pub fn get_usage(&self) -> ResourceUsage {
        ResourceUsage {
            ...
            write_throughput_bytes_per_sec: self.write_throughput_tracker.lock().unwrap().current_throughput(),
            read_throughput_bytes_per_sec: self.read_throughput_tracker.lock().unwrap().current_throughput(),
    ...
    pub fn emergency_cleanup(&self) -> Result<()> {
        ...
        self.record_violation(ViolationType::MemoryLimit,
...
impl ResourceUsage {
    ...
    pub fn get_usage_percentages(&self, limits: &ResourceLimits) -> UsagePercentages {
        UsagePercentages {
            ...
            operations_percent: (self.concurrent_operations as f64 / limits.max_concurrent_operations as f64 * 100.0),
            write_throughput_percent: (self.write_throughput_bytes_per_sec as f64 / limits.max_write_throughput_bytes_per_sec as f64 * 100.0),
            read_throughput_percent: (self.read_throughput_bytes_per_sec as f64 / limits.max_read_throughput_bytes_per_sec as f64 * 100.0),
