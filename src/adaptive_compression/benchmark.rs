...
impl CompressionBenchmark {
    ...
    pub fn run_comprehensive_benchmark(
        ...
        let metadata = BenchmarkMetadata {
            ...
            config_summary: format!("{} iterations, {} warmup, {} algorithms", 
                self.config.iterations, self.config.warmup_iterations, algorithms.len()),
        };
    ...
    fn benchmark_algorithm_on_dataset(
        ...
        for _ in 0..self.config.iterations {
            ...
            if decompressed != dataset.data {
                return Err(Error::Compression(
                    format!("Compression/decompression mismatch for algorithm {:?}", algorithm)
                ));
            }
        ...
        let compression_throughput = Self::calculate_throughput(
            dataset.data.len(),
            compression_time_stats.mean_ns,
        );
        let decompression_throughput = Self::calculate_throughput(
            dataset.data.len(),
            decompression_time_stats.mean_ns,
        );
    ...
    fn generate_data_for_type(
        ...
        match data_type {
            DataType::Text => {
                let text = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. ".repeat(size / 50 + 1);
    ...
    fn compute_algorithm_summaries(&self, results: &[BenchmarkResult]) -> HashMap<CompressionAlgorithm, AlgorithmSummary> {
        let mut summaries = HashMap::new();
        
        for algorithm in results.iter().map(|r| r.algorithm).collect::<std::collections::HashSet<_>>() {
            ...
            let avg_compression_ratio = algo_results.iter()
                .map(|r| r.compression_ratio)
                .sum::<f64>() / algo_results.len() as f64;
                
            let avg_compression_speed = algo_results.iter()
                .map(|r| r.compression_throughput)
                .sum::<f64>() / algo_results.len() as f64;
                
            let avg_decompression_speed = algo_results.iter()
                .map(|r| r.decompression_throughput)
                .sum::<f64>() / algo_results.len() as f64;
                
            let avg_efficiency = algo_results.iter()
                .map(|r| r.efficiency_score)
                .sum::<f64>() / algo_results.len() as f64;
            
            let best_use_cases = self.determine_best_use_cases(&algo_results);
            
            summaries.insert(algorithm, AlgorithmSummary {
                algorithm,
    ...
    fn find_best_by_data_type(&self, results: &[BenchmarkResult]) -> HashMap<DataType, CompressionAlgorithm> {
        let mut best_by_type = HashMap::new();
        
        for data_type in results.iter().map(|r| r.data_type.clone()).collect::<std::collections::HashSet<_>>() {
            let type_results: Vec<_> = results.iter().filter(|r| r.data_type == data_type).collect();
            
            if let Some(best) = type_results.iter().max_by(|a, b| {
                a.efficiency_score.partial_cmp(&b.efficiency_score).unwrap_or(std::cmp::Ordering::Equal)
            }) {
    ...
    fn generate_recommendations(
        ...
        let best_overall = summaries.iter()
            .max_by(|a, b| a.1.avg_efficiency.partial_cmp(&b.1.avg_efficiency).unwrap_or(std::cmp::Ordering::Equal))
            .map(|(algo, _)| *algo)
            .unwrap_or(CompressionAlgorithm::LZ4);
            
        let best_for_speed = summaries.iter()
            .max_by(|a, b| a.1.avg_compression_speed.partial_cmp(&b.1.avg_compression_speed).unwrap_or(std::cmp::Ordering::Equal))
            .map(|(algo, _)| *algo)
            .unwrap_or(CompressionAlgorithm::Snappy);
            
        let best_for_ratio = summaries.iter()
            .min_by(|a, b| a.1.avg_compression_ratio.partial_cmp(&b.1.avg_compression_ratio).unwrap_or(std::cmp::Ordering::Equal))
            .map(|(algo, _)| *algo)
            .unwrap_or(CompressionAlgorithm::Zstd);
            
        let best_balanced = summaries.iter()
            .max_by(|a, b| {
                let score_a = (1.0 - a.1.avg_compression_ratio) + a.1.avg_compression_speed / 1000.0;
                let score_b = (1.0 - b.1.avg_compression_ratio) + b.1.avg_compression_speed / 1000.0;
        ...
        use_case_recommendations.insert("memory-constrained".to_string(), CompressionAlgorithm::Snappy);
    ...
    fn determine_best_use_cases(&self, results: &[&BenchmarkResult]) -> Vec<String> {
        ...
        if let Some(result) = results.iter().max_by(|a, b| {
            a.compression_throughput.partial_cmp(&b.compression_throughput).unwrap_or(std::cmp::Ordering::Equal)
        }) {
        ...
        let avg_efficiency = results.iter().map(|r| r.efficiency_score).sum::<f64>() / results.len() as f64;
...
mod tests {
    use super::*;
    use crate::adaptive_compression::algorithms::*;
    ...
    fn test_benchmark_single_algorithm() {
        ...
        let results = benchmark.benchmark_algorithm(
    ...
    fn test_data_generation() {
        ...
        let text_data = benchmark.generate_data_for_type(DataType::Text, 100, base_data).unwrap();
        assert_eq!(text_data.len(), 100);
        
        let json_data = benchmark.generate_data_for_type(DataType::Json, 200, base_data).unwrap();
        assert_eq!(json_data.len(), 200);
        
        let numeric_data = benchmark.generate_data_for_type(DataType::Numeric, 64, base_data).unwrap();
    ...
    fn test_comprehensive_benchmark() {
        ...
        let test_data = b"This is comprehensive test data for benchmarking multiple compression algorithms.";
        
        let results = benchmark.run_comprehensive_benchmark(test_data, &algorithms).unwrap();
        ...
        assert!(results.algorithm_summary.contains_key(&CompressionAlgorithm::LZ4));
        assert!(results.algorithm_summary.contains_key(&CompressionAlgorithm::Snappy));
