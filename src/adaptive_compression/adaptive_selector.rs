...
impl AdaptiveSelector {
    ...
    fn level_meets_constraints(
        ...
        if let Some(time_limit_ms) = context.time_constraint_ms {
            let estimated_time_us = self.estimate_compression_time(
                algorithm, level, context.data_size, context.entropy
            );
    ...
    fn evaluate_algorithm(
        ...
        let estimated_ratio = self.estimate_compression_ratio(
            algorithm, level, context.entropy, algo_stats, data_type_stats
        );
    ...
    fn get_algorithm_bonus(&self, algorithm: CompressionAlgorithm, context: &SelectionContext) -> f64 {
        match algorithm {
            CompressionAlgorithm::None => {
    ...
    fn estimate_compression_ratio(
        ...
        let mut ratio = match algorithm {
            ...
            CompressionAlgorithm::LZ4 => {
                if entropy < 6.0 { 0.4 } else if entropy < 7.0 { 0.6 } else { 0.8 }
            }
            CompressionAlgorithm::Zstd => {
                let base = if entropy < 5.0 { 0.2 } else if entropy < 6.0 { 0.3 } 
                          else if entropy < 7.0 { 0.5 } else { 0.7 };
                // Adjust for compression level
                base * (1.0 - (level.as_int() as f64 - 1.0) * 0.05)
            }
            CompressionAlgorithm::Snappy => {
                if entropy < 6.0 { 0.5 } else if entropy < 7.0 { 0.7 } else { 0.9 }
            }
            CompressionAlgorithm::LZMA => {
                if entropy < 4.0 { 0.1 } else if entropy < 6.0 { 0.2 } 
                else if entropy < 7.0 { 0.4 } else { 0.6 }
            }
            CompressionAlgorithm::Brotli => {
                if entropy < 5.0 { 0.15 } else if entropy < 6.0 { 0.25 } 
                else if entropy < 7.0 { 0.4 } else { 0.6 }
            }
        ...
        if let Some(stats) = algo_stats {
            if stats.usage_count >= self.learning.min_samples {
                ...
                let weight = (stats.usage_count as f64 / (stats.usage_count as f64 + 10.0)).min(0.8);
    ...
    fn calculate_confidence(
        ...
        if let Some(stats) = algo_stats {
            let usage_confidence = (stats.usage_count as f64 / 
                (stats.usage_count as f64 + self.learning.min_samples as f64)).min(1.0);
        ...
        if let Some(data_stats) = data_type_stats {
            let sample_confidence = (data_stats.sample_count as f64 / 
                (data_stats.sample_count as f64 + self.learning.min_samples as f64)).min(1.0);
...
mod tests {
    ...
    fn test_storage_speed_classification() {
        ...
        matches!(storage_speed, 
            StorageSpeed::VeryFast | StorageSpeed::Fast | 
            StorageSpeed::Medium | StorageSpeed::Slow);
    }
