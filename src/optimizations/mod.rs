...
pub use memory_layout::{
    MemoryLayoutOps, CacheAlignedAllocator, OptimizedBTreeNode, 
...
impl OptimizationManager {
    ...
    pub fn get_stats(&self) -> OptimizationStats {
        OptimizationStats {
            simd_operations: self.simd_operations.load(std::sync::atomic::Ordering::Relaxed),
    ...
    pub fn reset_stats(&self) {
        self.simd_operations.store(0, std::sync::atomic::Ordering::Relaxed);
        self.cache_hits.store(0, std::sync::atomic::Ordering::Relaxed);
        self.cache_misses.store(0, std::sync::atomic::Ordering::Relaxed);
        self.pool_hits.store(0, std::sync::atomic::Ordering::Relaxed);
        self.pool_misses.store(0, std::sync::atomic::Ordering::Relaxed);
    }
...
pub mod utils {
    ...
    pub fn detect_optimal_config() -> OptimizationConfig {
        ...
        {
            config.enable_simd = is_x86_feature_detected!("sse4.2") || is_x86_feature_detected!("avx2");
        }
...
// Re-export sys_info for convenience
#[cfg(feature = "sys-info")]
pub use sys_info;
...
mod tests {
    ...
    fn test_workload_config() {
        ...
        assert!(read_heavy.object_pool_sizes.cache_entries >= write_heavy.object_pool_sizes.cache_entries);
        ...
        assert!(analytical.prefetch_config.prefetch_distance >= read_heavy.prefetch_config.prefetch_distance);
    }
}