...
use super::{Migration, MigrationConfig, MigrationManager, Schema, SchemaVersion};
...
impl MigrationTestHarness {
    ...
    pub fn new(config: TestConfig) -> Result<Self> {
        let temp_dir = TempDir::new().map_err(|e| Error::Io(e.to_string()))?;
    ...
    fn test_with_data(&mut self, metrics: &mut TestMetrics) -> Result<bool> {
        info!(
            "Testing migration with {} rows of data",
            self.config.large_data_rows
        );
...
mod tests {
    ...
    fn test_simple_migration() {
        ...
        let migration = TestMigrationBuilder::new(SchemaVersion::new(1, 0), "Test migration")
