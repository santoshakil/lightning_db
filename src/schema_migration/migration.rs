...
use super::{SchemaVersion, Schema};
...
impl MigrationStep {
    ...
    pub fn execute_down(&self, database: &Database) -> Result<()> {
        match self {
            ...
            MigrationStep::DropTable { name: _, if_exists: _ } => {
            ...
            MigrationStep::DropIndex { name: _, if_exists: _ } => {
                // For rollback, we need the original definition
...
impl MigrationBuilder {
    ...
    /// Drop a column
    pub fn drop_column(self, table: impl Into<String>, column: impl Into<String>, if_exists: bool) -> Self {
    ...
    pub fn build(self) -> Result<SimpleMigration> {
        let target_schema = self.target_schema
...
mod tests {
    use super::*;
    use crate::schema_migration::{TableDefinition, ColumnDefinition, DataType};
