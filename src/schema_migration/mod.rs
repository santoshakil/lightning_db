...
pub use migration::{Migration, MigrationStep, MigrationDirection};
pub use versioning::{SchemaVersion, VersionManager};
pub use executor::{MigrationExecutor, ExecutionResult};
pub use validator::{SchemaValidator, ValidationResult};
pub use history::{MigrationHistory, MigrationRecord};
pub use introspection::{SchemaIntrospector, SchemaDiff};
...
pub enum PartitioningStrategy {
    ...
    List { column: String, values: Vec<Vec<Value>> },
}
...
impl MigrationManager {
    ...
    pub fn register_migration(&self, migration: Box<dyn Migration>) -> Result<()> {
        ...
        if migrations.contains_key(&version) {
            return Err(Error::Config(format!("Migration version {} already registered", version)));
        }
    ...
    pub fn migrate(&self) -> Result<Vec<ExecutionResult>> {
        ...
        let pending: Vec<_> = {
            let migrations = self.migrations.read();
            migrations
                .range((std::ops::Bound::Excluded(current.clone()), std::ops::Bound::Included(target)))
                .map(|(v, _)| v.clone())
                .collect()
        };
        ...
        for version in pending {
            ...
            // Execute migration
            match self.executor.execute(migration.as_ref(), MigrationDirection::Up) {
                Ok(result) => {
                    // Update version
    ...
    pub fn rollback_to(&self, target: &SchemaVersion) -> Result<Vec<ExecutionResult>> {
        ...
        let to_rollback: Vec<_> = {
            let migrations = self.migrations.read();
            migrations
                .range((std::ops::Bound::Excluded(target.clone()), std::ops::Bound::Included(current)))
                .rev()
                .map(|(v, _)| v.clone())
                .collect()
        };
        ...
        for version in to_rollback {
            ...
            info!("Rolling back migration {}: {}", version, migration.description());
            
            match self.executor.execute(migration.as_ref(), MigrationDirection::Down) {
                Ok(result) => {
