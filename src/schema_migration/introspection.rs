...
use super::{ConstraintDefinition, IndexDefinition, Schema, SchemaVersion, TableDefinition};
...
pub enum TableChange {
    /// Table added
    Added { table: TableDefinition },

    /// Table removed
    Removed { table: TableDefinition },
...
pub enum ColumnChange {
    /// Column added
    Added { column: super::ColumnDefinition },

    /// Column removed
    Removed { column: super::ColumnDefinition },
...
impl SchemaIntrospector {
    ...
    pub fn get_current_schema(&self) -> Result<Schema> {
        ...
        let scan_result = self
            .database
            .scan(Some(table_prefix.to_vec()), Some(next_prefix(table_prefix)))?;
        ...
        let scan_result = self
            .database
            .scan(Some(index_prefix.to_vec()), Some(next_prefix(index_prefix)))?;
    ...
    pub fn diff_schemas(&self, from: &Schema, to: &Schema) -> Result<SchemaDiff> {
        debug!(
            "Generating schema diff from {} to {}",
    ...
    fn diff_columns(
        ...
        let from_map: BTreeMap<_, _> = from_columns.iter().map(|c| (c.name.clone(), c)).collect();
        let to_map: BTreeMap<_, _> = to_columns.iter().map(|c| (c.name.clone(), c)).collect();
    ...
    fn calculate_summary(
        ...
        let total_changes = summary.tables_added
