...
use super::{Migration, MigrationDirection, MigrationConfig};
use std::sync::Arc;
...
impl MigrationExecutor {
    ...
    fn restore_backup(&self, version: &str) -> Result<()> {
        ...
        if let Some(data) = self.database.get(backup_key.as_bytes())? {
            let metadata: BackupMetadata = bincode::decode_from_slice(&data, bincode::config::standard())
    ...
    pub fn resume_migration(
        ...
        if let Some(data) = self.database.get(checkpoint_key.as_bytes())? {
            let checkpoints: Vec<ExecutionCheckpoint> = bincode::decode_from_slice(&data, bincode::config::standard())
                .map_err(|e| Error::Serialization(e.to_string()))?.0;
