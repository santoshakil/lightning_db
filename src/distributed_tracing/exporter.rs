...
impl HttpExporter {
    ...
    fn convert_to_otel_format(&self, spans: &[Span]) -> serde_json::Value {
        let resource_spans = spans.iter().map(|span| {
            serde_json::json!({
                "resource": {
                    "attributes": [
                        {
    ...
    fn span_to_otel_format(&self, span: &Span) -> serde_json::Value {
        ...
        let end_time_nanos = span.end_time
            .map(|end| end.duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos())
            .unwrap_or(start_time_nanos);
        
        let attributes = span.tags.iter().map(|(k, v)| {
            serde_json::json!({
                "key": k,
                "value": {"stringValue": v}
            })
        ...
        let events = span.logs.iter().map(|log| {
            let timestamp_nanos = log.timestamp
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
                .as_nanos();
...
impl MetricsExporter {
    ...
    fn update_metrics(&self, span: &Span) {
        if let Ok(mut metrics) = self.metrics_store.lock() {
            let op_metrics = metrics.entry(span.operation_name.clone())
                .or_default();
...
mod tests {
    use super::*;
    use crate::distributed_tracing::{TraceContext, Span};
