...
impl W3CTraceContext {
    ...
    pub fn parse_traceparent(header: &str) -> Result<TraceContext> {
        ...
        let flags = u8::from_str_radix(parts[3], 16)
            .map_err(|_| crate::Error::InvalidInput("Invalid trace flags".to_string()))?;
...
mod tests {
    ...
    fn test_with_context() {
        ...
        let result = with_context(context2.clone(), || {
            current_context().unwrap().trace_id
        });
    ...
    fn test_database_context_creation() {
        ...
        assert_eq!(context.get_baggage("db.operation"), Some(&"SELECT".to_string()));
        assert_eq!(context.get_baggage("db.table"), Some(&"users".to_string()));
        assert_eq!(context.get_baggage("db.system"), Some(&"lightning_db".to_string()));
    }
}