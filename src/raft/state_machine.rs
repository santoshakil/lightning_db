...
impl LightningStateMachine {
    ...
    fn check_duplicate(&self, client_id: u64, request_id: u64) -> Option<Vec<u8>> {
        ...
        self.applied_txs.read().get(&(client_id, request_id)).cloned()
    }
...
impl StateMachine for LightningStateMachine {
    ...
    fn restore(&self, snapshot: &[u8]) -> Result<()> {
        ...
        let (metadata, _): (SnapshotMetadata, usize) = decode_from_slice(&snapshot[..metadata_size], config::standard())
            .map_err(|e| Error::Serialization(e.to_string()))?;
        
        // Verify version
        if metadata.version != 1 {
            return Err(Error::InvalidData(format!("Unsupported snapshot version: {}", metadata.version)));
        }
...
impl LightningStateMachine {
    ...
    pub fn apply_with_dedup(&self, command: &Command, client_id: Option<u64>, request_id: Option<u64>) -> Result<Vec<u8>> {
        // Check for duplicate
...
mod mock {
    ...
    impl Database {
        ...
        pub fn restore_snapshot(&self, snapshot: &[u8]) -> Result<()> {
            let (new_data, _): (BTreeMap<Vec<u8>, Vec<u8>>, usize) = decode_from_slice(snapshot, config::standard())
                .map_err(|e| Error::Serialization(e.to_string()))?;
...
#[cfg(test)]
pub use mock::{Database, MockTransaction as Transaction, MockTransactionManager as TransactionManager};
