...
impl RaftNode {
    ...
    pub async fn send_append_entries(&self, peer_id: NodeId, is_heartbeat: bool) -> Result<()> {
        ...
        match self.rpc.send_append_entries(peer_id, args.clone()).await {
            Ok(reply) => {
                self.handle_append_entries_response(peer_id, args, reply)
                    .await?;
            }
            Err(e) => {
    ...
    async fn handle_append_entries_response(
        ...
        if let Some(ref mut leader) = *leader_state {
            ...
            if reply.success {
                ...
                self.metrics
                    .entries_replicated
                    .fetch_add(args.entries.len() as u64, Ordering::Relaxed);
            } else {
    ...
    pub async fn handle_append_entries(
        ...
        if args.prev_log_index > 0 {
            ...
            if !has_prev_entry {
                ...
                let (conflict_term, conflict_index) = if let Some(entry) = persistent
                    .log
                    .iter()
                    .find(|e| e.index == args.prev_log_index)
                {
                    ...
                    (
                        None,
                        Some(persistent.log.last().map(|e| e.index + 1).unwrap_or(1)),
                    )
                };
        ...
        if !args.entries.is_empty() {
            ...
            for new_entry in &args.entries {
                if let Some(existing) = persistent.log.iter().find(|e| e.index == new_entry.index) {
