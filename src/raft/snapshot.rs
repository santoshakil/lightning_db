...
impl RaftNode {
    ...
    async fn install_complete_snapshot(&self, args: InstallSnapshotArgs) -> Result<()> {
        println!("Installing snapshot from leader {} at index {}", 
        ...
        // Update state
        self.volatile.last_applied.store(args.last_included_index, Ordering::Release);
    ...
    async fn store_snapshot_chunk(&self, args: InstallSnapshotArgs) -> Result<()> {
        ...
        Err(Error::NotImplemented("Streaming snapshots not yet supported".to_string()))
    }
    ...
    pub async fn send_snapshot(&self, peer_id: NodeId) -> Result<()> {
        ...
        println!("Sending snapshot to {} (size: {} bytes)", peer_id, snapshot.data.len());
...
impl FileRaftStorage {
    ...
    fn state_path(&self) -> PathBuf {
        self.base_dir.join("state").join(format!("node_{}.state", self.node_id))
    }
    
    fn snapshot_path(&self, index: LogIndex) -> PathBuf {
        self.base_dir.join("snapshots").join(format!("snapshot_{}.bin", index))
    }
...
impl RaftStorage for FileRaftStorage {
    ...
    fn load_snapshot(&self) -> Result<Option<RaftSnapshot>> {
        ...
        let data = std::fs::read(latest_path)
            .map_err(|e| Error::Io(e.to_string()))?;
