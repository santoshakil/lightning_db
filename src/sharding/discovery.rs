...
impl EtcdServiceDiscovery {
    ...
    pub async fn update_shard_assignments(
        &self,
        assignments: HashMap<ShardId, Vec<NodeId>>,
    ) -> Result<()> {
    ...
    pub async fn get_node_weights(&self) -> HashMap<NodeId, f64> {
        ...
        for (node_id, registration) in nodes.iter() {
            ...
            let memory_weight = (registration.metadata.total_memory as f64
    ...
    pub async fn find_optimal_node(&self, shard_id: ShardId, exclude: &[NodeId]) -> Option<NodeId> {
        ...
        weights
            ...
            .filter(|(node_id, &weight)| {
                weight > 0.0 && !assigned_nodes.contains(node_id) && !exclude.contains(node_id)
            })
    ...
    pub fn get_stats(&self) -> DiscoveryStats {
        DiscoveryStats {
            ...
            shard_assignments_changed: self
                .metrics
                .shard_assignments_changed
...
mod tests {
    ...
    async fn test_in_memory_discovery() {
        ...
        // Register nodes
        discovery
            .register_node(1, "node1:8080".to_string())
            .await
            .unwrap();
        discovery
            .register_node(2, "node2:8080".to_string())
            .await
            .unwrap();
