...
impl DefaultRebalancer {
    ...
    fn calculate_load_score(&self, shard_stats: &ShardStats) -> f64 {
        ...
        let size_score =
            (shard_stats.data_size as f64 / (10_i64 * 1024 * 1024 * 1024) as f64).min(1.0); // 10GB max
        ...
        let memory_score =
            (shard_stats.memory_usage as f64 / (1024_i64 * 1024 * 1024) as f64).min(1.0); // 1GB max
    ...
    fn generate_movement_plan(
        &self,
        overloaded: &[ShardLoad],
        underloaded: &[ShardLoad],
    ) -> Vec<DataMovement> {
        let mut movements = Vec::new();
        let mut available_capacity: Vec<(ShardId, f64)> = underloaded
            .iter()
    ...
    async fn execute_movement(&self, movement: DataMovement) -> Result<()> {
        ...
        println!(
            "Completed data movement: {} -> {}",
            movement.from_shard, movement.to_shard
        );
...
impl ShardRebalancer for DefaultRebalancer {
    fn needs_rebalancing(&self, topology: &ClusterTopology) -> Result<bool> {
        ...
        if needs_rebalancing {
            println!(
                "Rebalancing needed: {} overloaded shards, {} underloaded shards",
                overloaded.len(),
                underloaded.len()
            );
        }
    ...
    fn generate_plan(&self, topology: &ClusterTopology) -> Result<RebalancePlan> {
        ...
        let avg_overload: f64 =
            overloaded.iter().map(|s| s.load_score).sum::<f64>() / overloaded.len() as f64;
        let priority = (avg_overload * 100.0) as u32;

        println!(
            "Generated rebalance plan: {} movements, {} bytes, estimated duration: {:?}",
            movements.len(),
            total_data_size,
            estimated_duration
        );
    ...
    async fn execute_plan(&self, plan: RebalancePlan) -> Result<()> {
        ...
        println!(
            "Executing rebalance plan with {} movements",
            plan.movements.len()
        );
        ...
        let semaphore = Arc::new(tokio::sync::Semaphore::new(
            self.config.max_concurrent_movements as usize,
        ));
        ...
        println!(
            "Rebalance plan execution completed: {}/{} movements successful",
...
impl LoadPredictor for TrendPredictor {
    fn predict_load(&self, history: &[LoadSnapshot], horizon: Duration) -> Result<Vec<ShardLoad>> {
        ...
        for shard_id in shard_ids {
            ...
            for snapshot in history {
                if let Some(shard_load) =
                    snapshot.shard_loads.iter().find(|s| s.shard_id == shard_id)
                {
            ...
            let predicted_load = (current_load + load_trend * horizon_steps as f64)
                .max(0.0)
                .min(1.0);
            let predicted_size =
                (current_size as f64 + size_trend * horizon_steps as f64).max(0.0) as u64;
