...
impl TwoPhaseCommitCoordinator {
                ...
                for tx_id in expired_txs {
                    if let Some(mut tx) = transactions.write().remove(&tx_id) {
                        ...
                        if let Some(response_tx) = tx.response_tx.take() {
                            let _ = response_tx.send(Err(Error::Timeout("Transaction timeout".to_string())));
                        }
                    }
                }
            }
            ...
            let deadlock_task = tokio::spawn(async move {
                ...
                loop {
                    ...
                    
                    {
                        ...
                        for (tx_id, tx) in txs.iter() {
                            if matches!(tx.state, TransactionState::Preparing) &&
                               now.duration_since(tx.timeout - Duration::from_secs(30)) > Duration::from_secs(10) {
                    ...
                    if !long_running_txs.is_empty() {
                        println!("Potential deadlock detected: {} long-running transactions", long_running_txs.len());
                        ...
                        if let Some(tx_id) = long_running_txs.first() {
                            if let Some(mut tx) = transactions.write().remove(tx_id) {
                                tx.state = TransactionState::Aborted;
                                println!("Aborted transaction {} to resolve potential deadlock", tx_id);
                                
                                if let Some(response_tx) = tx.response_tx.take() {
                                    let _ = response_tx.send(Err(Error::Deadlock("Transaction aborted due to deadlock".to_string())));
                                }
                            }
                        }
                    }
    ...
    async fn prepare_transaction(&self, tx_id: &str) -> Result<bool> {
        ...
        println!("Preparing transaction {} across {} shards", tx_id, tx.shards.len());
        ...
        for shard_id in &tx.shards {
            ...
            let result = self.send_prepare_request(*shard_id, tx_id, operations).await;
        ...
        for (shard_id, result) in prepare_results {
            match result {
                ...
                Err(e) => {
                    eprintln!("Failed to prepare shard {} for transaction {}: {}", shard_id, tx_id, e);
                    all_prepared = false;
                    break;
                }
            }
    ...
    async fn send_prepare_request(&self, shard_id: ShardId, tx_id: &str, operations: Vec<TransactionOp>) -> Result<bool> {
        ...
        println!("Shard {} prepared successfully for transaction {}", shard_id, tx_id);
        Ok(true)
    }
    ...
    async fn commit_transaction_phase(&self, tx_id: &str) -> Result<()> {
        ...
        println!("Committing transaction {} across {} shards", tx_id, tx.shards.len());
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            match result {
                Ok(()) => {
                    println!("Shard {} committed transaction {} successfully", shard_id, tx_id);
                }
                Err(e) => {
                    eprintln!("Failed to commit transaction {} on shard {}: {}", tx_id, shard_id, e);
    ...
    async fn send_commit_request(&self, shard_id: ShardId, tx_id: &str, operations: Vec<TransactionOp>) -> Result<()> {
        let leader_node = self.find_shard_leader(shard_id).await?;
        ...
        for op in operations {
            ...
            if let Err(e) = result {
                return Err(Error::TransactionFailed(format!("Failed to execute operation: {}", e)));
            }
        }
    ...
    async fn abort_transaction_phase(&self, tx_id: &str) -> Result<()> {
        ...
        println!("Aborting transaction {} across {} shards", tx_id, tx.shards.len());
        
        // Send abort requests to all shards
        for shard_id in &tx.shards {
            if let Err(e) = self.send_abort_request(*shard_id, tx_id).await {
                eprintln!("Failed to abort transaction {} on shard {}: {}", tx_id, shard_id, e);
            }
        }
    ...
    fn generate_tx_id(&self) -> String {
        ...
        format!("tx_{}_{}", counter, Uuid::new_v4().to_string()[..8].to_string())
    }
    
    /// Validate transaction operations
    fn validate_operations(&self, operations: &HashMap<ShardId, Vec<TransactionOp>>) -> Result<()> {
        if operations.is_empty() {
            return Err(Error::InvalidInput("Transaction has no operations".to_string()));
        }
        
        let total_ops: usize = operations.values().map(|ops| ops.len()).sum();
        if total_ops > 1000 {
            return Err(Error::InvalidInput("Too many operations in transaction".to_string()));
        }
        ...
        for (shard_id, ops) in operations {
            ...
            for op in ops {
                ...
                if !keys.insert(key.clone()) {
                    return Err(Error::InvalidInput(format!("Duplicate key in shard {}", shard_id)));
                }
            }
        }
...
impl TransactionCoordinator for TwoPhaseCommitCoordinator {
    ...
    async fn commit_transaction(&self, tx_id: &str) -> Result<()> {
        ...
        if !prepared {
            ...
            return Err(Error::TransactionFailed("Transaction was aborted during prepare phase".to_string()));
        }
        ...
        match self.commit_transaction_phase(tx_id).await {
            Ok(()) => {
                ...
                // Update metrics
                self.metrics.successful_commits.fetch_add(1, Ordering::Relaxed);
                ...
                *avg_duration = (*avg_duration * (current_count - 1.0) + duration) / current_count;
                ...
                println!("Transaction {} committed successfully in {:.2}ms", tx_id, duration);
                Ok(())
            }
            Err(e) => {
...
impl TransactionCoordinator for SimpleCoordinator {
    async fn begin_transaction(&self, operations: HashMap<ShardId, Vec<TransactionOp>>) -> Result<String> {
        if operations.len() != 1 {
            return Err(Error::InvalidInput("SimpleCoordinator only supports single-shard transactions".to_string()));
        }
...
mod tests {
    ...
    fn test_coordinator_creation() {
        ...
        assert_eq!(coordinator.metrics.active_transactions.load(Ordering::Acquire), 0);
    }
    ...
    fn test_operation_validation() {
        ...
        valid_ops.insert(1, vec![TransactionOp::Write { 
            key: b"test".to_vec(), 
