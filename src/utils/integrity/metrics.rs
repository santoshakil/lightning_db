...
impl IntegrityMetrics {
    ...
    pub fn record_validation(&self, operation: &str, success: bool, duration: Duration) {
    ...
    pub fn record_validation_with_path(
        ...
        self.total_validation_time_micros
        ...
        if let Ok(mut paths) = self.paths.write() {
            ...
            path_metrics.error_rate =
                path_metrics.error_count as f64 / path_metrics.validation_count as f64;
            path_metrics.avg_validation_time_micros = (path_metrics.avg_validation_time_micros
                * (path_metrics.validation_count - 1)
                + duration_micros)
...
impl MetricsSnapshot {
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!(
            "Max Validation Time: {} μs\n",
            self.max_validation_time_micros
        ));
        report.push_str(&format!(
            "Min Validation Time: {} μs\n",
            self.min_validation_time_micros
        ));
        ...
        if !self.paths.is_empty() {
            ...
            let problematic_paths: Vec<_> = self
                .paths
                .iter()
    ...
    pub fn get_recommendations(&self) -> Vec<String> {
        ...
        if self.success_rate < 0.95 {
            recommendations.push(
                "Low validation success rate detected. Consider investigating failed validations."
                    .to_string(),
            );
        }
        ...
        if self.critical_errors > 0 {
            recommendations.push(
                "Critical integrity errors detected. Immediate investigation required.".to_string(),
            );
        }
        ...
        if self.timeout_validations > self.total_validations / 20 {
            // > 5%
