...
impl QuotaManager {
        ...
        if let Some(tenant_id) = tenant_id {
            if !self.rate_limiter.check_tenant_read(tenant_id)? {
                self.record_violation(
                    ResourceType::ReadOps,
                    1,
                    Some(tenant_id),
                    ViolationAction::Throttled,
                );
    ...
    pub fn get_resource_usage(&self) -> ResourceUsage {
        let violations = self.violations.lock().iter().cloned().collect();
        ...
        ResourceUsage {
            ...
            disk_used_gb: self.disk_used.load(Ordering::Relaxed) as f64
    ...
    fn record_violation(
        ...
        warn!(
            ...
            resource_type,
            requested,
            self.get_limit_for_resource(resource_type),
            action
        );
    }
    ...
    fn get_limit_for_resource(&self, resource_type: ResourceType) -> u64 {
        match resource_type {
            ...
            ResourceType::Disk => {
                self.config.disk_quota_gb.unwrap_or(u64::MAX) * 1024 * 1024 * 1024
            }
