...
impl ConnectionLimiter {
    ...
    pub fn cleanup_idle_connections(&self) -> Result<usize> {
        ...
        if let Some(max_idle) = self.max_idle_connections {
            let idle_connections: Vec<(u64, Duration)> = self
                .connections
                .iter()
                .map(|entry| {
                    let info = entry.value();
    ...
    pub fn get_stats(&self) -> ConnectionStats {
        ...
        let tenant_stats: Vec<(String, usize)> = self
            .tenant_connections
            .iter()
...
mod tests {
    ...
    fn test_tenant_connection_limit() {
        ...
        // Set tenant limit
        limiter
            .set_tenant_limit("tenant1".to_string(), 2, 1, 1)
            .unwrap();

        // Should respect tenant limit
        let _conn1 = limiter
            .accept_connection(Some("tenant1".to_string()))
            .unwrap();
        let _conn2 = limiter
            .accept_connection(Some("tenant1".to_string()))
            .unwrap();
    ...
    fn test_idle_cleanup() {
        let limiter =
            ConnectionLimiter::new(None, Some(1), Some(Duration::from_millis(100))).unwrap();
