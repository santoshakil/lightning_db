            ...
            if let Some(handle) = managed.handle.take() {
                ...
                if remaining_timeout > Duration::ZERO {
                    ...
                    match Self::join_with_timeout(handle, remaining_timeout) {
                        ...
                        Err(handle) => {
                            warn!("Thread '{}' did not shutdown within timeout, detaching", managed.name);
                    ...
                    if handle.is_finished() {
                        if let Some(handle) = managed.handle.take() {
                            match handle.now_or_never() {
                                Some(Ok(_)) => debug!("Tokio task '{}' completed successfully", managed.name),
                                Some(Err(e)) => error!("Tokio task '{}' failed: {:?}", managed.name, e),
                                None => warn!("Tokio task '{}' finished but result not ready", managed.name),
                            }
                        }
                        false // Remove from registry
                    } else {
