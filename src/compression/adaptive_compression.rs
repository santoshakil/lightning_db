use crate::compression::{get_compressor, CompressionType, Compressor};
...
impl AdaptiveCompressionEngine {
    ...
    pub fn compress(&self, data: &[u8]) -> Result<(Vec<u8>, CompressionType)> {
        ...
        if self.compression_count.fetch_add(1, Ordering::Relaxed)
    ...
    pub fn decompress(&self, data: &[u8], compression_type: CompressionType) -> Result<Vec<u8>> {
        ...
        if let Some(stats) = self.stats.get(&compression_type) {
            stats
                .total_decompression_time_us
                .fetch_add(decompress_time.as_micros() as u64, Ordering::Relaxed);
        }
    ...
    fn predict_best_compression(
        ...
        let similar_samples: Vec<_> = samples
            .iter()
            .filter(|s| {
                s.pattern == *pattern
    ...
    fn update_stats(
        ...
        if let Some(stats) = self.stats.get(&compression_type) {
            ...
            stats
                .total_compression_time_us
                .fetch_add(compress_time.as_micros() as u64, Ordering::Relaxed);
            stats
                .total_decompression_time_us
                .fetch_add(decompress_time.as_micros() as u64, Ordering::Relaxed);
    ...
    fn learn_from_sample(&self, sample: DataSample) {
        ...
        if sample.compression_ratio > 0.3 {
            self.pattern_preferences
                .insert(sample.pattern, sample.best_compression);
        }
    }
        ...
        self.stats
            ...
            .map(|entry| {
                ...
                let stats = CompressionTypeStats {
                    total_compressions: AtomicU64::new(
                        entry.value().total_compressions.load(Ordering::Relaxed),
                    ),
                    total_original_bytes: AtomicU64::new(
                        entry.value().total_original_bytes.load(Ordering::Relaxed),
                    ),
                    total_compressed_bytes: AtomicU64::new(
                        entry.value().total_compressed_bytes.load(Ordering::Relaxed),
                    ),
                    total_compression_time_us: AtomicU64::new(
                        entry
                            .value()
                            .total_compression_time_us
                            .load(Ordering::Relaxed),
                    ),
                    total_decompression_time_us: AtomicU64::new(
                        entry
                            .value()
                            .total_decompression_time_us
                            .load(Ordering::Relaxed),
                    ...
                    success_count: AtomicU64::new(
                        entry.value().success_count.load(Ordering::Relaxed),
                    ),
                    failure_count: AtomicU64::new(
                        entry.value().failure_count.load(Ordering::Relaxed),
                    ),
                };
                (comp_type, stats)
            })
...
mod tests {
    ...
    fn test_adaptive_compression() {
        ...
        #[cfg(feature = "zstd-compression")]
        {
            ...
            test_cases.push((b"AAAAAAAAAA".repeat(100), CompressionType::Zstd));
            // Repetitive
        }
        ...
        #[cfg(not(feature = "zstd-compression"))]
        {
            ...
            test_cases.push((b"AAAAAAAAAA".repeat(100), CompressionType::Snappy));
            // Repetitive
        }
