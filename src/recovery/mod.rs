...
impl RecoveryManager {
    ...
    fn recover_legacy(&self) -> Result<Database> {
        ...
        let header = metadata.read_header().map_err(|e| {
            crate::error::Error::PartialRecoveryFailure {
        ...
        let db = Database::open(&self.db_path, self.config.clone()).map_err(|e| {
            crate::error::Error::RecoveryVerificationFailed {
    ...
    pub fn needs_recovery(&self) -> Result<bool> {
        ...
        if dwb_path.exists() {
            let dwb_size = std::fs::metadata(&dwb_path).map_err(|e| {
                crate::error::Error::RecoveryDependencyError {
        ...
        if wal_dir.exists() {
            let wal_files = std::fs::read_dir(&wal_dir).map_err(|e| {
                crate::error::Error::RecoveryPermissionError {
