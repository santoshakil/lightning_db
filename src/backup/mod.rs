...
use crate::realtime_stats::REALTIME_STATS;
...
impl BackupManager {
    ...
    pub fn create_online_backup<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        for (relative_path, source_file) in &files_to_backup {
            ...
            REALTIME_STATS.write().update_size_metrics(
                bytes_copied,
                0,
                0,
            );
    ...
    pub fn create_incremental_backup<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        let results: Vec<Result<u64>> = files_to_backup
            ...
            .map(|(relative_path, source_file)| {
                ...
                let size = self.copy_file_incremental(
                    source_file,
                    &dest_file,
                    Arc::clone(&hasher),
                )?;
    ...
    pub fn create_point_in_time_backup<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        if base_backup.is_none() {
            return Err(Error::Generic("No full backup found before target timestamp".to_string()));
        }
        ...
        fs::write(&pitr_path, serde_json::to_string_pretty(&pitr_info)
    ...
    fn copy_file_incremental(
        ...
        if self.config.compress {
            ...
            use crate::compression::{Lz4Compressor, Compressor};
