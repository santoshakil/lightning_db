...
pub enum ValidationResult {
    Significant { improvement_pct: f64, confidence: f64 },
    Marginal { improvement_pct: f64, confidence: f64 },
    NoImprovement { decline_pct: f64 },
...
impl PerformanceValidationFramework {
    ...
    pub fn validate_optimization(
        ...
        let session_id = format!("validation_{}", 
        ...
        println!("  Confidence threshold: {:.1}%", self.confidence_threshold * 100.0);
        ...
        let baseline_metrics = self.benchmark_configuration(
            db_path, &baseline_config, "baseline")?;
        
        // Benchmark optimized configuration
        println!("🚀 Benchmarking optimized configuration...");
        let optimized_metrics = self.benchmark_configuration(
            db_path, &optimized_config, "optimized")?;
        
        // Run test scenarios
        println!("🧪 Running comprehensive test scenarios...");
        let test_scenarios = self.run_test_scenarios(
            db_path, &baseline_config, &optimized_config)?;
        ...
        let confidence_score = self.calculate_confidence_score(
            &baseline_metrics, &optimized_metrics, &test_scenarios)?;
    ...
    fn benchmark_configuration(
        ...
        println!("  {} benchmark completed in {:?}", config_name, benchmark_duration);
    ...
    fn aggregate_performance_metrics(&self, samples: &[PerformanceMetrics]) -> Result<PerformanceMetrics> {
        if samples.is_empty() {
            return Err(Error::Generic("No performance samples to aggregate".to_string()));
        }
        ...
        Ok(PerformanceMetrics {
            ...
            read_latency_us: LatencyMetrics {
                ...
                max: samples.iter().map(|s| s.read_latency_us.max).fold(0.0, f64::max),
            },
            write_latency_us: LatencyMetrics {
                ...
                max: samples.iter().map(|s| s.write_latency_us.max).fold(0.0, f64::max),
            },
            memory_usage_mb: (samples.iter().map(|s| s.memory_usage_mb).sum::<u64>() / samples.len() as u64),
            cpu_usage_percent: samples.iter().map(|s| s.cpu_usage_percent).sum::<f64>() / len,
        })
    }

    /// Perform statistical analysis
    fn perform_statistical_analysis(&self, samples: &[PerformanceMetrics]) -> Result<StatisticalAnalysis> {
        if samples.is_empty() {
            return Err(Error::Generic("No samples for statistical analysis".to_string()));
        }
        ...
        let outlier_count = throughputs.iter()
    ...
    fn analyze_latency_distribution(&self, latency_samples: &[f64]) -> Result<LatencyDistribution> {
        ...
        let outlier_count = sorted_latencies.iter().filter(|&&lat| lat > outlier_threshold).count();
    ...
    fn analyze_throughput_consistency(&self, samples: &[PerformanceMetrics]) -> Result<ThroughputConsistency> {
        if samples.is_empty() {
        ...
        let plateau_count = throughputs.iter().filter(|&&t| t >= plateau_threshold).count();
    ...
    fn analyze_resource_efficiency(&self, samples: &[PerformanceMetrics]) -> Result<ResourceEfficiency> {
        if samples.is_empty() {
        ...
        let avg_throughput = samples.iter().map(|s| s.mixed_ops_per_sec).sum::<f64>() / samples.len() as f64;
        let avg_cpu_usage = samples.iter().map(|s| s.cpu_usage_percent).sum::<f64>() / samples.len() as f64;
        let avg_memory_usage = samples.iter().map(|s| s.memory_usage_mb).sum::<u64>() / samples.len() as u64;
    ...
    fn analyze_scalability(&self, _db: &Arc<Database>) -> Result<ScalabilityMetrics> {
        ...
        Ok(ScalabilityMetrics {
            ...
            load_response_curve: vec![
                (0.1, 1.0), (0.5, 0.95), (1.0, 0.85), (2.0, 0.6), (5.0, 0.3)
            ],
    ...
    fn run_test_scenarios(
        ...
        for (scenario_name, workload_type) in scenarios {
            ...
            // Test baseline
            let baseline_performance = self.run_scenario_test(
                db_path, baseline_config, &workload_type)?;
            
            // Test optimized
            let optimized_performance = self.run_scenario_test(
                db_path, optimized_config, &workload_type)?;
    ...
    fn determine_validation_result(&self, improvement: &PerformanceImprovement) -> Result<ValidationResult> {
    ...
    fn calculate_confidence_score(
        ...
        let scenario_confidence = test_scenarios.iter()
            .map(|s| s.confidence_level)
            .sum::<f64>() / test_scenarios.len() as f64;
        confidence_factors.push(scenario_confidence);
        
        // Sample size factor
        let sample_size_factor = (baseline.statistical_analysis.sample_count as f64 / 1000.0).min(1.0);
    ...
    fn print_validation_summary(&self, session: &ValidationSession) -> Result<()> {
        ...
        match &session.validation_result {
            ValidationResult::Significant { improvement_pct, confidence } => {
                println!("✅ SIGNIFICANT IMPROVEMENT: {:.1}% (confidence: {:.1}%)", 
                         improvement_pct, confidence * 100.0);
            }
            ValidationResult::Marginal { improvement_pct, confidence } => {
                println!("🟡 MARGINAL IMPROVEMENT: {:.1}% (confidence: {:.1}%)", 
                         improvement_pct, confidence * 100.0);
            }
        ...
        println!("\n📊 Performance Metrics:");
        println!("  Baseline Throughput: {:.0} ops/sec", 
                 session.baseline_metrics.performance_metrics.mixed_ops_per_sec);
        println!("  Optimized Throughput: {:.0} ops/sec", 
                 session.optimized_metrics.performance_metrics.mixed_ops_per_sec);
        println!("  Overall Improvement: {:.1}%", 
        ...
        for improvement in &session.performance_improvement.significant_improvements {
            println!("  • {}: {:.1}% ({:?} impact, {:.0}% confidence)",
                     improvement.area, improvement.improvement_pct, 
                     improvement.impact, improvement.confidence * 100.0);
        }
        
        println!("\n🧪 Test Scenarios:");
        for scenario in &session.test_scenarios {
            let status = if scenario.statistical_significance { "✅" } else { "⚠️" };
            println!("  {} {}: {:.1}% improvement ({:.0}% confidence)",
                     status, scenario.scenario_name, scenario.improvement_pct,
...
mod tests {
    ...
    fn test_improvement_percentage_calculation() {
        ...
        assert_eq!(framework.calculate_improvement_percentage(100.0, 110.0), 10.0);
        assert_eq!(framework.calculate_improvement_percentage(100.0, 90.0), -10.0);
        assert_eq!(framework.calculate_improvement_percentage(0.0, 100.0), 100.0);
    }
    ...
    fn test_improvement_impact_classification() {
        ...
        assert!(matches!(framework.classify_improvement_impact(25.0), ImprovementImpact::Critical));
        assert!(matches!(framework.classify_improvement_impact(15.0), ImprovementImpact::High));
        assert!(matches!(framework.classify_improvement_impact(7.0), ImprovementImpact::Medium));
        assert!(matches!(framework.classify_improvement_impact(3.0), ImprovementImpact::Low));
        assert!(matches!(framework.classify_improvement_impact(0.5), ImprovementImpact::Negligible));
    }
}