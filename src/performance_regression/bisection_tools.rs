...
impl BisectionTools {
    ...
    pub fn start_bisection(&mut self, regression: &RegressionDetectionResult) -> Result<BisectionSession> {
    ...
    pub fn continue_bisection(
        ...
        let session = self.active_sessions.get_mut(session_id)
            .ok_or_else(|| crate::Error::Generic(format!("Bisection session not found: {}", session_id)))?;

        // Update test point with result
        if let Some(test_point) = session.test_points.iter_mut()
    ...
    fn initialize_bisection(&self, session: &mut BisectionSession) -> Result<()> {
        ...
        session.bad_point = Some(BisectionTestPoint {
            ...
            performance_metrics: Some(TestPerformanceMetrics {
                ...
                throughput_ops_per_sec: session.regression.current_performance.throughput_ops_per_sec,
                ...
                additional_metrics: session.regression.current_performance.additional_metrics.clone(),
            }),
        });
    ...
    fn generate_initial_suspects(&self, regression: &RegressionDetectionResult) -> Vec<RootCauseSuspect> {
        let mut suspects = Vec::new();

        // Code change suspect
        suspects.push(RootCauseSuspect {
            suspect_type: SuspectType::CodeChange,
            description: "Recent code changes may have introduced performance regression".to_string(),
            probability: 0.7,
            evidence: vec![
                format!("Performance degraded by {:.1}%", regression.degradation_percentage * 100.0),
        ...
        suspects.push(RootCauseSuspect {
            ...
            probability: 0.5,
            evidence: vec![
                "Performance change pattern suggests configuration impact".to_string(),
            ],
            time_range: Some(TimeRange {
        ...
        if regression.current_performance.memory_usage_bytes > regression.baseline_performance.memory_baseline * 2 {
            suspects.push(RootCauseSuspect {
                suspect_type: SuspectType::EnvironmentChange,
                description: "Environment or resource changes may be causing performance issues".to_string(),
                probability: 0.6,
                evidence: vec![
                    format!("Memory usage increased from {} MB to {} MB",
    ...
    fn create_test_metadata_static(session: &BisectionSession, timestamp: SystemTime) -> HashMap<String, String> {
        let mut metadata = HashMap::new();
        metadata.insert("bisection_type".to_string(), format!("{:?}", session.bisection_type));
        metadata.insert("iteration".to_string(), session.current_state.current_iteration.to_string());
        metadata.insert("operation_type".to_string(), session.regression.operation_type.clone());
    ...
    fn process_test_result_static(
        ...
        let test_point = session.test_points.iter()
            .find(|tp| tp.test_id == test_point_id)
        ...
        let initial_duration = session.current_state.narrowed_range.end
            .duration_since(session.current_state.narrowed_range.start)
            .unwrap_or_default().as_secs_f64() / 3600.0;
    ...
    fn generate_final_recommendations_static(session: &BisectionSession) -> Vec<BisectionRecommendation> {
        ...
        if let Some(suspect) = session.suspects.iter()
            ...
            recommendations.push(BisectionRecommendation {
                ...
                description: format!("High probability ({:.0}%) that code changes caused regression: {}", 
                    suspect.probability * 100.0, suspect.description),
        ...
        recommendations.push(BisectionRecommendation {
            ...
            description: "Add automated performance tests to prevent similar regressions".to_string(),
            priority: Priority::Medium,
    ...
    fn update_suspect_probabilities_static(session: &mut BisectionSession) {
        ...
        session.suspects.sort_by(|a, b| b.probability.partial_cmp(&a.probability).unwrap());
    }
    ...
    fn calculate_time_overlap_static(range1: &TimeRange, range2: &TimeRange) -> f64 {
        ...
        let overlap_duration = overlap_end.duration_since(overlap_start).unwrap_or_default().as_secs_f64();
        let total_duration = range1.end.duration_since(range1.start).unwrap_or_default().as_secs_f64();
    ...
    pub fn cancel_session(&mut self, session_id: &str) -> Result<()> {
        if let Some(session) = self.active_sessions.get_mut(session_id) {
            ...
            Err(crate::Error::Generic(format!("Session not found: {}", session_id)))
        }
    }
    ...
    pub fn get_bisection_statistics(&self) -> BisectionStatistics {
        ...
        let completed_sessions = self.active_sessions.values()
            .filter(|s| s.completion_status == BisectionStatus::Completed)
            .count();
        let in_progress_sessions = self.active_sessions.values()
            .filter(|s| s.completion_status == BisectionStatus::InProgress)
            .count();
        ...
        let avg_iterations = if completed_sessions > 0 {
            self.active_sessions.values()
                ...
                .sum::<u32>() as f64 / completed_sessions as f64
        } else {
            0.0
        };
...
mod tests {
    use super::*;
    use super::super::{PerformanceMetric, PerformanceBaseline, RegressionSeverity};
    ...
    fn test_continue_bisection() {
        ...
        let test_id = session.current_state.next_test_point.as_ref().unwrap().test_id.clone();
        ...
        let updated_session = tools.continue_bisection(
            &session_id,
            &test_id,
            BisectionTestResult::Good,
            Some(metrics),
    ...
    fn test_suspect_generation() {
        ...
        assert!(suspects.iter().any(|s| matches!(s.suspect_type, SuspectType::CodeChange)));
    }
    ...
    fn test_session_cancellation() {
        ...
        assert_eq!(cancelled_session.completion_status, BisectionStatus::Cancelled);
    }
