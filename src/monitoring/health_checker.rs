...
impl HealthChecker {
    ...
    pub fn perform_health_checks(&self, database: &Database) -> Result<HealthReport> {
            ...
            let result = match check.check(database) {
                Ok(mut result) => {
                    ...
                    if result.duration > self.config.check_timeout {
                        ...
                        result.message = format!("{} (slow execution: {:?})", result.message, result.duration);
                    }
        ...
        // Create summary
        let passed_checks = results.iter().filter(|r| matches!(r.status, CheckStatus::Pass)).count();
        let warning_checks = results.iter().filter(|r| matches!(r.status, CheckStatus::Warn)).count();
        let failed_checks = results.iter().filter(|r| matches!(r.status, CheckStatus::Fail)).count();
...
impl HealthCheck for ConnectivityCheck {
    fn name(&self) -> &str { "connectivity" }
    fn description(&self) -> &str { "Verifies basic database connectivity" }
...
impl HealthCheck for StorageIntegrityCheck {
    fn name(&self) -> &str { "storage_integrity" }
    fn description(&self) -> &str { "Verifies storage integrity and data consistency" }
    fn is_critical(&self) -> bool { true }
...
impl HealthCheck for ResponseTimeCheck {
    fn name(&self) -> &str { "response_time" }
    fn description(&self) -> &str { "Monitors database response times" }
    ...
    fn check(&self, database: &Database) -> Result<HealthCheckResult> {
        ...
        details.insert("write_time_ms".to_string(), write_time.as_millis().to_string());
        details.insert("read_time_ms".to_string(), read_time.as_millis().to_string());
...
impl HealthCheck for ThroughputCheck {
    ...
    fn description(&self) -> &str { "Monitors database throughput" }
    ...
    fn check(&self, database: &Database) -> Result<HealthCheckResult> {
        ...
        let mut details = HashMap::new();
        details.insert("write_ops_per_sec".to_string(), write_ops_per_sec.to_string());
        ...
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status,
            message: format!("Write: {:.0} ops/sec, Read: {:.0} ops/sec", write_ops_per_sec, read_ops_per_sec),
            duration: start.elapsed(),
            timestamp: SystemTime::now(),
            details,
        })
...
impl HealthCheck for LatencyCheck {
    ...
    fn description(&self) -> &str { "Monitors operation latency percentiles" }
    ...
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        ...
        let mut details = HashMap::new();
        details.insert("p50_latency_us".to_string(), p50_latency.as_micros().to_string());
        details.insert("p95_latency_us".to_string(), p95_latency.as_micros().to_string());
        details.insert("p99_latency_us".to_string(), p99_latency.as_micros().to_string());
...
impl HealthCheck for MemoryUsageCheck {
    ...
    fn description(&self) -> &str { "Monitors memory usage" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for DiskSpaceCheck {
    ...
    fn description(&self) -> &str { "Monitors available disk space" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for FileDescriptorCheck {
    fn name(&self) -> &str { "file_descriptors" }
    fn description(&self) -> &str { "Monitors file descriptor usage" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for TransactionHealthCheck {
    fn name(&self) -> &str { "transaction_health" }
    fn description(&self) -> &str { "Monitors transaction system health" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for CacheHealthCheck {
    fn name(&self) -> &str { "cache_health" }
    fn description(&self) -> &str { "Monitors cache system health" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for CompactionHealthCheck {
    fn name(&self) -> &str { "compaction_health" }
    fn description(&self) -> &str { "Monitors compaction system health" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
...
impl HealthCheck for WALHealthCheck {
    fn name(&self) -> &str { "wal_health" }
    fn description(&self) -> &str { "Monitors WAL system health" }
    fn check(&self, _database: &Database) -> Result<HealthCheckResult> {
        Ok(HealthCheckResult {
            check_name: self.name().to_string(),
            status: CheckStatus::Pass,
