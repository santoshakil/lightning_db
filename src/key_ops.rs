...
use crate::{ConsistencyLevel, Database, Key, Result};
...
impl Database {
    ...
    pub fn get_key_with_consistency(
        &self,
        key: &Key,
    ...
    pub fn put_key_with_consistency(
        &self,
        key: &Key,
        value: &[u8],
        level: ConsistencyLevel,
...
mod tests {
    ...
    fn test_inline_key_optimization() -> Result<()> {
        ...
        assert_eq!(
            db.get_key(&small_key)?.as_deref(),
            Some(b"small_value".as_ref())
        );
        assert_eq!(
            db.get_key(&large_key)?.as_deref(),
            Some(b"large_value".as_ref())
        );
