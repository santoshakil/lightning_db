...
impl BTreeWriteBuffer {
    ...
    fn flush_locked(&self, buffer: &mut BTreeMap<Vec<u8>, Vec<u8>>) -> Result<()> {
        if buffer.is_empty() {
            return Ok(());
        }
        
        // Extract entries to avoid holding lock during B+Tree operations
        let entries: Vec<(Vec<u8>, Vec<u8>)> = buffer.clone().into_iter().collect();
        buffer.clear();
...
mod tests {
    ...
    fn test_auto_flush() -> Result<()> {
        ...
        for i in 0..15 {
            buffer.insert(format!("key{}", i).into_bytes(), format!("value{}", i).into_bytes())?;
        }
