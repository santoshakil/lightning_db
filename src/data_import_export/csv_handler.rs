...
pub fn import_csv(
    ...
    let csv_config = match format {
        DataFormat::Csv { delimiter, quote_char, escape_char, headers } => {
            CsvConfig {
        ...
        _ => return Err(crate::Error::InvalidInput("Expected CSV format".to_string())),
    };
    ...
    for item in scan {
        ...
        match serde_json::from_slice::<Value>(&value) {
            Ok(json_value) => {
                ...
                match json_to_csv_record(&json_value, &mut field_order, csv_config.has_headers && !headers_written) {
        ...
        // Progress reporting
        if stats.records_processed % 10000 == 0 {
            println!("  📊 Exported {} records ({} success, {} errors)", 
                    stats.records_processed, stats.records_success, stats.records_failed);
        }
    }
    ...
    for field in &schema.fields {
        if !field.nullable && !csv_headers.contains(&field.name.as_str()) {
            return Err(crate::Error::SchemaValidationFailed(
                format!("Required field '{}' not found in CSV headers", field.name)
            ));
        }
    }
...
fn convert_csv_value_to_json(value: &str, field_name: &str, schema: Option<&SchemaDefinition>) -> Value {
...
impl CsvUtils {
    ...
    pub fn validate_csv_structure<P: AsRef<Path>>(file_path: P, config: &CsvConfig) -> Result<CsvStructureInfo> {
        let file = File::open(file_path)?;
        let buf_reader = BufReader::new(file);
        ...
        for (i, result) in reader.records().enumerate() {
            match result {
                Ok(record) => {
                    ...
                    if i < 5 {
                        info.sample_records.push(record.iter().map(|s| s.to_string()).collect());
                    }
                },
                Err(_) => {
                    info.inconsistent_records += 1;
...
impl CsvStructureInfo {
    pub fn is_valid(&self) -> bool {
        self.total_records > 0 && self.column_count > 0 && 
...
mod tests {
    ...
    fn test_auto_detect_type() {
        ...
        assert_eq!(auto_detect_type("123.45"), Value::Number(Number::from_f64(123.45).unwrap()));
        assert_eq!(auto_detect_type("true"), Value::Bool(true));
        assert_eq!(auto_detect_type("false"), Value::Bool(false));
        assert_eq!(auto_detect_type("hello"), Value::String("hello".to_string()));
    }
    ...
    fn test_json_value_to_csv_string() {
        ...
        assert_eq!(json_value_to_csv_string(&Value::Number(Number::from(42))), "42");
        assert_eq!(json_value_to_csv_string(&Value::String("test".to_string())), "test");
    }
