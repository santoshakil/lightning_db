...
use super::{
    DataFormat, SchemaDefinition, ImportExportConfig, OperationStats, OperationResult,
    validation,
};
...
fn process_stream_record(
    ...
    let json_value = match format {
        DataFormat::Json { .. } => {
            serde_json::from_slice::<Value>(record_data)
...
impl AdaptiveStreamProcessor {
        ...
        F: FnMut() -> Option<Vec<u8>>,
    {
        ...
        self.performance_tracker.record_run(&result.as_ref().unwrap().stats);
...
impl PerformanceTracker {
    ...
    fn get_average_performance(&self) -> PerformanceMetrics {
        ...
        let sum = self.runs.iter().fold(
            PerformanceMetrics::default(),
            |acc, metrics| PerformanceMetrics {
                throughput_records_per_second: acc.throughput_records_per_second + metrics.throughput_records_per_second,
                throughput_mb_per_second: acc.throughput_mb_per_second + metrics.throughput_mb_per_second,
...
mod tests {
    ...
    fn test_file_reader_callback() -> Result<()> {
        ...
        let mut callback = StreamUtils::create_file_reader_callback(
            temp_file.path(),
            1024,
        )?;
