...
impl LockFreePageTracker {
    ...
    pub fn allocate(&self) -> PageAllocation {
        ...
        if let Some(page_id) = self.free_pages.pop() {
            ...
            // Allocate a new page atomically
            // Use a loop to ensure atomic allocation and registration
            loop {
                let page_id = self.next_page_id.fetch_add(1, Ordering::SeqCst);
                
                // Try to insert the page_id atomically
                // DashSet::insert returns true if the value was not present
                if self.allocated_pages.insert(page_id) {
                    ...
                    // This should never happen with fetch_add, but if it does,
                    // it means there's a serious bug. Log and retry.
                    // In production, this would warrant an error log.
                    continue;
                }
            }
        }
