...
// Export the unified cache as the main cache interface
pub use unified_cache::{UnifiedCache, UnifiedCacheConfig};
...
/// Optimized cached page for read-heavy workloads using Cow
#[derive(Debug)]
pub struct OptimizedCachedPage {
    pub page: Cow<'static, Page>,
    pub access_count: AtomicUsize,
    pub last_access: AtomicUsize,
    pub is_dirty: AtomicUsize,
}

impl OptimizedCachedPage {
    pub fn new(page: Page) -> Self {
        Self {
            page: Cow::Owned(page),
            access_count: AtomicUsize::new(0),
    ...
    pub fn from_borrowed(page: &'static Page) -> Self {
        Self {
    ...
    /// Get page data efficiently - no allocation for reads
    pub fn data(&self) -> &[u8] {
        self.page.data()
    }

    /// Clone to owned if needed (for writes)
    pub fn to_owned_page(&mut self) -> &mut Page {
        if let Cow::Borrowed(_) = self.page {
            self.page = Cow::Owned(self.page.as_ref().clone());
        }
        self.page.to_mut()
    }
}
