...
impl FallbackIo {
    pub fn new(queue_depth: u32) -> Self {
        Self {
            ...
            completed: Arc::new(Mutex::new(VecDeque::with_capacity(queue_depth as usize * 2))),
    ...
    fn handle_write(
        ...
        let result = match request.buffer {
            Some(IoBuffer::Standard(ref buf)) => {
                match file_guard.write_all(buf.as_slice()) {
                    Ok(()) => buf.len() as i32,
                    Err(e) => -(e.raw_os_error().unwrap_or(EIO)),
                }
            },
...
impl ZeroCopyIo for FallbackIo {
    ...
    fn sq_space_left(&self) -> usize {
        ...
        self.queue_depth as usize - pending.len()
    }
