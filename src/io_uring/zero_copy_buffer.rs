...
impl AlignedBuffer {
    ...
    pub fn new(size: usize, alignment: BufferAlignment) -> Result<Self> {
        ...
        if ptr.is_null() {
            return Err(Error::new(ErrorKind::OutOfMemory, "Failed to allocate buffer"));
        }
    ...
    pub fn split_at(&mut self, mid: usize) -> Result<AlignedBuffer> {
        if mid > self.len {
            return Err(Error::new(ErrorKind::InvalidInput, "Split index out of bounds"));
        }
        ...
        Ok(unsafe {
            AlignedBuffer::from_raw_parts(second_ptr, second_len, self.alignment)
        })
    }
...
impl BufferPool {
    ...
    pub fn clear(&self) {
        for pool in &self.pools {
            ...
            self.stats.current_buffers.fetch_sub(count, Ordering::Relaxed);
        }
    }
}
...
impl RegisteredBufferManager {
    ...
    pub fn register_buffers(&mut self, buffers: Vec<AlignedBuffer>) -> Vec<Arc<RegisteredBuffer>> {
        ...
        for (idx, buffer) in buffers.into_iter().enumerate() {
            self.buffers.push(Arc::new(RegisteredBuffer::new(buffer, idx as u32)));
        }
