...
impl DirectIoFile {
    ...
    fn detect_block_size(fd: RawFd) -> Result<usize> {
        #[cfg(target_os = "linux")]
        {
            ...
            let result = unsafe {
                libc::ioctl(fd, BLKSSZGET as libc::c_ulong, &mut block_size)
            };
    ...
    pub fn advise(&self, offset: u64, len: u64, advice: PosixAdvice) -> Result<()> {
        #[cfg(target_os = "linux")]
        {
            let result = unsafe {
                libc::posix_fadvise(
...
impl DirectIoReader {
    ...
    pub fn read(&mut self, len: usize) -> Result<Vec<u8>> {
        ...
        let mut buffer = self.buffer_pool.acquire(aligned_len, self.file.alignment())?;
