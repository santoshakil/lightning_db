...
impl AccessPattern {
    ...
    pub fn record_read(&mut self, latency_us: f64) {
        ...
        self.avg_latency = (self.avg_latency * (self.read_count - 1) as f64 + latency_us) / self.read_count as f64;
    ...
    pub fn record_write(&mut self, latency_us: f64) {
        ...
        self.avg_latency = (self.avg_latency * (total_ops - 1) as f64 + latency_us) / total_ops as f64;
...
impl TieredSSTable {
    ...
    pub fn update_placement_score(&mut self, tier_config: &TierConfig) {
        ...
        let tier_match_score = if hotness >= tier_config.min_hotness && hotness <= tier_config.max_hotness {
            100.0
...
impl MigrationJob {
    ...
    pub fn execute(&self, tiered_manager: &TieredStorageManager) -> Result<()> {
        ...
        // Update metadata
        tiered_manager.update_sstable_tier(
            Arc::clone(&self.sstable),
            self.to_tier,
            target_path,
        )?;
...
impl TieredStorageManager {
    ...
    fn evaluate_and_schedule_migrations(
        ...
        for tier_sstables in sstables_write.values_mut() {
            for tiered_sstable in tier_sstables.iter_mut() {
                let file_name = tiered_sstable.sstable.metadata().file_path
                    .file_name()
                    .unwrap()
                    .to_string_lossy();
            ...
            for tiered_sstable in tier_sstables {
                ...
                if let Some(hotter_tier) = current_tier.previous_tier() {
                    if let Some(hotter_config) = tiers_read.get(&hotter_tier) {
                        if hotness >= hotter_config.min_hotness && hotter_config.has_space(tiered_sstable.sstable.metadata().file_size) {
                ...
                if let Some(colder_tier) = current_tier.next_tier() {
                    if let Some(colder_config) = tiers_read.get(&colder_tier) {
                        if hotness <= colder_config.max_hotness && colder_config.has_space(tiered_sstable.sstable.metadata().file_size) {
                            let job_id = next_job_id.fetch_add(1, Ordering::SeqCst);
                            migrations.push(MigrationJob::new(
                                job_id,
                                Arc::clone(&tiered_sstable.sstable),
    ...
    pub fn get_tier_stats(&self) -> HashMap<StorageTier, TierStats> {
        ...
        for (tier, config) in tiers.iter() {
            ...
            let total_accesses = sstables.get(tier)
                .map(|sstables| {
                    sstables.iter()
            ...
            result.insert(*tier, TierStats {
                num_sstables: tier_sstables as u64,
                total_size: config.current_size,
                max_size: config.max_size,
                utilization: config.utilization(),
                total_accesses,
                avg_hotness: sstables.get(tier)
                    .map(|sstables| {
                        if sstables.is_empty() {
                            0.0
                        } else {
                            sstables.iter().map(|ts| ts.access_pattern.hotness_score()).sum::<f64>() / sstables.len() as f64
                        }
                    })
                    .unwrap_or(0.0),
                migrations_in: 0,
