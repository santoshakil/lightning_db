use crate::statistics::metrics::{DatabaseMetrics, ComponentMetrics, MetricsSnapshot};
...
impl MetricsCollector {
    ...
    pub fn register_component(&self, name: &str) -> Arc<ComponentMetrics> {
        ...
        self.component_metrics.write().insert(name.to_string(), Arc::clone(&component));
        component
    }
    ...
    pub fn get_window_metrics(&self, window: Duration) -> Option<WindowMetrics> {
        ...
        let duration = window_snapshots[window_snapshots.len() - 1].timestamp
        ...
        Some(WindowMetrics {
            ...
            reads_per_sec,
            writes_per_sec,
            avg_read_latency_us: if count > 0 { total_read_latency / count as u64 } else { 0 },
            avg_write_latency_us: if count > 0 { total_write_latency / count as u64 } else { 0 },
            ...
            error_rate: if last.reads + last.writes > first.reads + first.writes {
                ((last.read_errors + last.write_errors) - (first.read_errors + first.write_errors)) as f64
