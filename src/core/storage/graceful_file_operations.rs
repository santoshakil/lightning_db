...
mod tests {
    ...
    fn test_read_only_fallback() {
        ...
        // Create file first
        std::fs::write(&file_path, b"existing data").expect("Failed to write test file");
    ...
    fn test_write_permission_detection() {
        let temp_dir = TempDir::new().expect("Failed to create temp directory for permission test");
