use crate::features::encryption::EncryptionManager;
use crate::core::error::Result;
use super::{OptimizedPageManager, Page, PageManager};
    ...
    pub fn get_page(&self, page_id: u32) -> Result<Page> {
        match self {
            ...
            PageManagerWrapper::Encrypted {
                ...
                let mut decrypted_data = [0u8; crate::core::storage::PAGE_SIZE];
    ...
    pub fn write_page(&self, page: &Page) -> Result<()> {
        match self {
            ...
            PageManagerWrapper::Encrypted {
                ...
                let mut encrypted_data = [0u8; crate::core::storage::PAGE_SIZE];

                // Ensure encrypted data fits in page size
                if encrypted.len() > crate::core::storage::PAGE_SIZE {
                    return Err(crate::core::error::Error::Encryption(format!(
                        "Encrypted data ({} bytes) exceeds page size ({} bytes)",
                        encrypted.len(),
                        crate::core::storage::PAGE_SIZE
                    )));
                }
