...
impl CheckpointManager {
        ...
        for entry in std::fs::read_dir(base_path)
            ...
            if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                if filename.starts_with("checkpoint_") && filename.ends_with(".chk") {
                    if let Some(lsn_str) = filename.strip_prefix("checkpoint_").and_then(|s| s.strip_suffix(".chk")) {
    ...
    pub fn create_checkpoint<F>(&self, checkpoint_lsn: u64, mut get_state: F) -> Result<CheckpointInfo>
    where
        F: FnMut() -> Result<(u64, Vec<u64>)>, // Returns (committed_lsn, active_transactions)
    {
        ...
        let mut metadata = CheckpointMetadata::new(checkpoint_lsn, committed_lsn, active_transactions);
        ...
        let checkpoint_path = self.base_path.join(format!("checkpoint_{:016}.chk", checkpoint_lsn));
        ...
        info!("Checkpoint created successfully at LSN {} -> {:?}", checkpoint_lsn, checkpoint_path);
    ...
    pub fn load_checkpoint(&self, checkpoint_lsn: u64) -> Result<CheckpointInfo> {
        ...
        if metadata.checkpoint_lsn != checkpoint_lsn {
            return Err(Error::CheckpointCorrupted {
                checkpoint_lsn,
                reason: format!("LSN mismatch: expected {}, found {}", checkpoint_lsn, metadata.checkpoint_lsn),
            });
        }
    ...
    pub fn find_latest_checkpoint(&self) -> Result<Option<CheckpointInfo>> {
        ...
        for info in sorted_checkpoints {
            match self.load_checkpoint(info.checkpoint_lsn) {
                ...
                Err(e) => {
                    warn!("Failed to load checkpoint at LSN {}: {}", info.checkpoint_lsn, e);
                }
            }
        }
                    ...
                    if let Some(lsn_str) = filename.strip_prefix("checkpoint_").and_then(|s| s.strip_suffix(".chk")) {
                        if let Ok(checkpoint_lsn) = lsn_str.parse::<u64>() {
                            match self.read_checkpoint_metadata(&path) {
                                ...
                                Err(e) => {
                                    warn!("Failed to read checkpoint metadata from {:?}: {}", path, e);
                                }
                            }
                        }
                    }
    ...
    fn verify_checkpoint(&self, checkpoint_path: &Path) -> Result<()> {
        ...
        if metadata.committed_lsn > metadata.checkpoint_lsn {
            return Err(Error::CheckpointCorrupted {
                checkpoint_lsn: metadata.checkpoint_lsn,
                reason: format!("Committed LSN {} > checkpoint LSN {}", 
                    metadata.committed_lsn, metadata.checkpoint_lsn),
            });
        }
        
        debug!("Checkpoint verification successful: LSN {}", metadata.checkpoint_lsn);
        Ok(())
    }
    ...
    fn cleanup_old_checkpoints(&self) -> Result<()> {
        ...
        for checkpoint in checkpoints.into_iter().skip(self.config.keep_checkpoint_count) {
            info!("Removing old checkpoint at LSN {}", checkpoint.checkpoint_lsn);
            if let Err(e) = std::fs::remove_file(&checkpoint.file_path) {
                warn!("Failed to remove old checkpoint {:?}: {}", checkpoint.file_path, e);
            }
        }
...
mod tests {
    ...
    fn test_checkpoint_creation_and_loading() {
        ...
        // Create a checkpoint
        let checkpoint_info = manager.create_checkpoint(100, || Ok((95, vec![1, 2, 3])))
        ...
        // Load the checkpoint
        let loaded = manager.load_checkpoint(100).expect("Failed to load checkpoint");
