...
impl WALRecoveryContext {
    ...
    pub fn recover_with_fixes<P, F>(&mut self, wal_path: P, mut apply_fn: F) -> Result<()>
    where
        P: AsRef<Path>,
            ...
            match self.read_entry_safe(&mut file, offset, file_size) {
                Ok(Some(entry)) => {
                    ...
                    if self.strict_ordering && entry.lsn <= last_valid_lsn {
                        warn!(
                            "Out-of-order LSN {} (last was {})",
                            entry.lsn, last_valid_lsn
                        );
                        continue;
                    }
    ...
    fn read_entry_safe(
        ...
        if offset + 4 + len as u64 > file_size {
            warn!(
                "Incomplete entry at offset {} (needs {} bytes)",
                offset, len
            );
            return Ok(None);
        }
        ...
        match bincode::decode_from_slice::<WALEntry, _>(&entry_bytes, bincode::config::standard()) {
            ...
            Err(deserialization_error) => {
                ...
                Err(crate::core::error::Error::WalBinaryCorruption {
                    offset,
...
mod tests {
    ...
    fn test_torn_write_recovery() {
        ...
        // Create a WAL with torn write
        {
            ...
            // Write header
            file.write_all(&[0x57, 0x41, 0x4C, 0x21, 0x01, 0x00, 0x00, 0x00])
                .unwrap();
