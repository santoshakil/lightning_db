...
impl UnifiedTransactionManager {
    ...
    pub fn get(&self, tx_id: u64, key: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let visible_value = {
            let visibility = self.visibility_tracker.read();
            self.version_store
                .get_visible(key, &snapshot, &visibility)?
        };
    ...
    pub fn acquire_read_lock(&self, tx_id: u64, key: &[u8]) -> Result<()> {
        // Check for write locks - but allow reads for snapshot isolation
        if let Some(write_lock) = self.write_locks.get(key) {
            if write_lock.tx_id != tx_id {
                // For snapshot isolation, reads are allowed even with write locks
                // The version store will handle visibility correctly
                // Just track the potential conflict but don't fail
                self.stats.conflict_count.fetch_add(1, Ordering::Relaxed);
            }
            return Ok(());
        }
    ...
    fn cleanup_expired_locks(&self) {
        ...
        // Cleanup write locks
        self.write_locks
            .retain(|_, lock_info| now.duration_since(lock_info.timestamp) <= timeout);
    ...
    pub fn get_detailed_stats(&self) -> TransactionStats {
        TransactionStats {
            active_transactions: AtomicU64::new(
                self.stats.active_transactions.load(Ordering::Relaxed),
            ),
...
impl VisibilityTracker {
    ...
    fn update_min_active_timestamp(&mut self) {
        self.min_active_timestamp = self
            .active_transactions
            .iter()
            .min()
            .copied()
...
impl UnifiedVersionStore {
    ...
    fn complete_reserved_write(
        ...
        // Add actual version
        self.put_version(key.to_vec(), value.map(|v| v.to_vec()), timestamp, tx_id);
    }
    ...
    pub fn cleanup_old_versions(&self, before_timestamp: u64, min_versions_to_keep: usize) {
        ...
        for mut entry in self.versions.iter_mut() {
            ...
            versions.retain(|v| v.timestamp >= before_timestamp || !v.is_reserved);

            // Ensure we keep at least min_versions_to_keep
            while versions.len() > min_versions_to_keep
                && versions
                    .last()
                    .is_some_and(|v| v.timestamp < before_timestamp)
            {
                versions.pop();
            }
        ...
        if total_cleaned > 0 {
            self.garbage_collected
                .fetch_add(total_cleaned as u64, Ordering::Relaxed);
        }
    }
            ...
            for version in versions.value() {
                if !version.is_reserved {
                    return version
                        .value
                        .as_ref()
                        .map(|v| (v.clone(), version.timestamp));
                }
            }
        }
        None
...
mod tests {
    ...
    fn test_priority_deadlock_prevention() {
        ...
        let tx1 = manager
            .begin_with_priority(TransactionPriority::High)
            .unwrap();
        let tx2 = manager
            .begin_with_priority(TransactionPriority::Low)
            .unwrap();
