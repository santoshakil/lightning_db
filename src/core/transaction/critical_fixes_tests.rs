...
mod tests {
    ...
    fn test_timestamp_overflow_protection() {
        ...
        manager.next_timestamp.store(u64::MAX - 500, Ordering::SeqCst);
    ...
    fn test_batch_commit_sequential_timestamps() {
        ...
        for _ in 0..10 {
            let tx_id = manager.begin().unwrap();
            manager.put(tx_id, b"key", b"value").unwrap();
            tx_ids.push(tx_id);
        }
    ...
    fn test_concurrent_transaction_safety() {
        ...
        let total_transactions = stats.commit_count.load(Ordering::Relaxed) 
            + stats.abort_count.load(Ordering::Relaxed);
        ...
        println!("Conflicts: {}", stats.conflict_count.load(Ordering::Relaxed));
    ...
    fn test_transaction_retry_on_conflict() {
                ...
                for attempt in 0..10 {
                    match mgr.begin() {
                        Ok(tx_id) => {
                            ...
                            // Read current value
                            let current = mgr.get(tx_id, key).unwrap()
        ...
        if let Ok(Some(final_value)) = manager.get(tx_id, b"shared_counter") {
            ...
            println!("Successful transactions: {}", success_count.load(Ordering::Relaxed));
            assert!(count > 0);
        }
