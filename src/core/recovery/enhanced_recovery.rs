...
use crate::core::storage::{Page, PageId};
use crate::core::wal::{LogSequenceNumber, WalEntry};
...
impl DoubleWriteBuffer {
    ...
    pub fn write_pages(
        &self,
        pages: &[(PageId, &Page)],
        write_fn: impl Fn(PageId, &Page) -> Result<()>,
    ) -> Result<()> {
        ...
        {
            let mut buffer = self.buffer.lock().map_err(|_| Error::Io("Double-write buffer lock poisoned".to_string()))?;
...
impl RedundantMetadata {
    ...
    pub fn write_header(&self, header: &Header) -> Result<()> {
        ...
        for backup_path in &self.backup_paths {
            if let Err(e) = self.write_header_to_file(backup_path, &header_bytes) {
                eprintln!(
                    "Warning: Failed to write backup header to {:?}: {}",
                    backup_path, e
                );
    ...
    pub fn read_header(&self) -> Result<Header> {
        ...
        Err(Error::InvalidFormat(
            "All metadata copies are corrupted".to_string(),
        ))
    }
...
impl RecoveryProgress {
    ...
    pub fn get_progress(&self) -> (u64, u64, String, std::time::Duration) {
        ...
        let phase = self.current_phase.lock().map(|p| p.clone()).unwrap_or_else(|_| "Unknown".to_string());
        ...
        F: FnMut(WalEntry) -> Result<()>,
    {
        ...
        for (idx, segment_path) in segments.iter().enumerate() {
            ...
            match self.recover_segment(segment_path, checkpoint_lsn, &mut apply_fn) {
                ...
                Err(e) => {
                    eprintln!(
                        "Warning: Failed to recover segment {:?}: {}",
                        segment_path, e
                    );
