use super::{BTreeNode, KeyEntry, NodeType, MAX_KEYS_PER_NODE};
use crate::core::error::{Error, Result};
use crate::core::storage::{Page, PageManagerWrapper};
...
impl<'a> SplitHandler<'a> {
    ...
    fn split_internal_node(
        ...
        right_node.entries = node.entries.drain((mid + 1)..).collect();
        ...
        right_node.children = node.children.drain((mid + 1)..).collect();
