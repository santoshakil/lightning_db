...
impl BatchEvictingArcCache {
    ...
    pub fn batch_evict(&self) -> Result<Vec<u32>> {
        ...
        if self.evicting.compare_exchange(false, true, Ordering::Acquire, Ordering::Relaxed).is_err() {
        ...
        for victim in victims {
            if let Some((_, cached_page)) = self.cache.remove(&victim) {
                ...
                if cached_page.is_dirty() {
                    ...
                    self.eviction_stats.dirty_pages_written.fetch_add(1, Ordering::Relaxed);
                }
        ...
        // Update statistics
        self.eviction_stats.total_batches.fetch_add(1, Ordering::Relaxed);
        self.eviction_stats.pages_evicted.fetch_add(evicted.len(), Ordering::Relaxed);
        self.eviction_stats.eviction_time_ms.fetch_add(
        ...
        *self.eviction_stats.last_eviction.lock() = Some(start);
        ...
        debug!("Batch evicted {} pages in {:?}", evicted.len(), start.elapsed());
