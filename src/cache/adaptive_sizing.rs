...
use crate::performance_tuning::{
    WorkloadType, WorkloadProfiler, HardwareDetector, HardwareInfo,
};
use std::sync::Arc;
...
impl WorkingSetEstimator {
    ...
    fn record_access(&mut self, page_id: u64) {
        ...
        *self.access_frequency.entry(page_id).or_insert(0) += 1;
        ...
        self.estimated_size = self.access_frequency
            .values()
            .filter(|&&count| count >= 2)
            .count() * 4096; // Assume 4KB pages
    }
...
impl PerformancePredictor {
    ...
    fn record_performance(&mut self, cache_size: usize, hit_rate: f64) {
        self.size_performance_history.push_back((cache_size, hit_rate));
    ...
    fn get_confidence(&self) -> f64 {
        ...
        let mean_hit_rate: f64 = self.size_performance_history
            .iter()
            .map(|(_, hr)| hr)
            .sum::<f64>() / n as f64;
        ...
        let predictions: Vec<f64> = self.size_performance_history
            .iter()
            .map(|(size, _)| {
        ...
        for ((_, actual_hit_rate), predicted) in self.size_performance_history.iter().zip(predictions) {
...
impl AdaptiveCacheSizer {
    ...
    pub fn initialize(&self) -> Result<()> {
        if let Ok(hw_info) = self.hardware_detector.detect() {
    ...
    pub fn update_performance(&self, metrics: &CachePerformanceMetrics) -> Result<Option<usize>> {
        ...
        self.performance_predictor.lock().record_performance(current_size, metrics.hit_rate);
    ...
    pub fn adjust_cache_size(&self, new_size: usize, reason: SizingReason) -> Result<bool> {
        ...
        if let Some(hw_info) = self.hardware_info.read().as_ref() {
            let memory_usage_ratio = new_size as f64 / (hw_info.total_memory_mb * 1024 * 1024) as f64;
            if memory_usage_ratio > self.config.memory_pressure_threshold {
                return Err(Error::Config("Cache size would exceed memory pressure threshold".into()));
            }
        }
    ...
    pub fn get_statistics(&self) -> AdaptiveSizingStats {
        ...
        let average_hit_rate = if performance_history.is_empty() {
            ...
            performance_history.iter().map(|m| m.hit_rate).sum::<f64>() / performance_history.len() as f64
        };
    ...
    fn calculate_optimal_size(&self, metrics: &CachePerformanceMetrics) -> Result<Option<usize>> {
        ...
        let max_increase = (current_size as f64 * (1.0 + self.config.max_adjustment_percent)) as usize;
        let max_decrease = (current_size as f64 * (1.0 - self.config.max_adjustment_percent)) as usize;
    ...
    fn calculate_temporal_locality(&self) -> f64 {
        ...
        let average_hit_rate = recent_hit_rates.iter().sum::<f64>() / recent_hit_rates.len() as f64;
    ...
    fn estimate_scan_intensity(&self) -> f64 {
        ...
        let avg_eviction_rate = recent_metrics.iter().map(|m| m.eviction_rate).sum::<f64>() / recent_metrics.len() as f64;
        let avg_hit_rate = recent_metrics.iter().map(|m| m.hit_rate).sum::<f64>() / recent_metrics.len() as f64;
...
mod tests {
    ...
    fn test_workload_pattern_analysis() {
        let config = AdaptiveSizingConfig::default();
        let sizer = AdaptiveCacheSizer::new(config).unwrap();
        
        // Simulate read-heavy workload
        sizer.analyze_workload(5000.0, 0.9).unwrap();
