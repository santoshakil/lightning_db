...
use crate::performance_tuning::{HardwareDetector, HardwareInfo, WorkloadProfiler, WorkloadType};
...
impl AdaptiveCacheSizer {
    ...
    fn estimate_scan_intensity(&self) -> f64 {
        ...
        let avg_eviction_rate = recent_metrics.iter().map(|m| m.eviction_rate).sum::<f64>()
            / recent_metrics.len() as f64;
        let avg_hit_rate =
            recent_metrics.iter().map(|m| m.hit_rate).sum::<f64>() / recent_metrics.len() as f64;
