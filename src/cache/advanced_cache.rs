...
impl<K: Clone + Hash + Eq + Send + Sync + 'static, V: Clone + Send + Sync + 'static> ClockProCache<K, V> {
                ...
                if let Some(key) = hot_pages.get(hand) {
                    if let Some(entry) = entries.get(key) {
                        if entry.reference_bit.swap(0, Ordering::Relaxed) == 0 {
                            // Found a victim
                            self.clock_hand_hot.store((hand + 1) % hot_pages.len(), Ordering::Relaxed);
                ...
                if let Some(key) = cold_pages.get(hand) {
                    if let Some(entry) = entries.get(key) {
                        if entry.test_bit.load(Ordering::Relaxed) == 0 {
                            // Found a victim
                            self.clock_hand_cold.store((hand + 1) % cold_pages.len(), Ordering::Relaxed);
                            return Some(key.clone());
                        } else {
                            // Promote to hot
...
impl<K: Clone + Hash + Eq + Send + Sync + 'static, V: Clone + Send + Sync + 'static> AdvancedCache<K, V> for ClockProCache<K, V> {
    fn get(&self, key: &K) -> Option<V> {
        let entries = self.entries.read();
...
impl<K: Clone + Hash + Eq + Send + Sync + 'static, V: Clone + Send + Sync + 'static> WTinyLFUCache<K, V> {
...
impl<K: Clone + Hash + Eq + Send + Sync + 'static, V: Clone + Send + Sync + 'static> AdvancedCache<K, V> for WTinyLFUCache<K, V> {
    fn get(&self, key: &K) -> Option<V> {
        // Increment frequency
...
pub fn create_cache<K: Clone + Hash + Eq + Send + Sync + 'static, V: Clone + Send + Sync + 'static>(
