...
impl PatternDetector {
    ...
    fn detect_sequential_patterns(&mut self) {
        ...
        for i in 0..self.access_history.len().saturating_sub(1) {
            let page_diff = self.access_history[i + 1].page_id as i64 - self.access_history[i].page_id as i64;
        ...
        self.detected_patterns.insert(PatternType::Sequential, patterns);
        ...
        self.pattern_confidence.insert(PatternType::Sequential, confidence);
    }
    ...
    fn detect_strided_patterns(&mut self) {
        ...
        for i in 0..self.access_history.len().saturating_sub(2) {
            let stride1 = self.access_history[i + 1].page_id as i64 - self.access_history[i].page_id as i64;
            let stride2 = self.access_history[i + 2].page_id as i64 - self.access_history[i + 1].page_id as i64;
            
            if stride1 == stride2 && stride1 > 1 {
                stride_patterns.entry(stride1)
        ...
        self.detected_patterns.insert(PatternType::Strided, patterns);
        ...
        self.pattern_confidence.insert(PatternType::Strided, confidence);
    }
    ...
    fn detect_clustered_patterns(&mut self) {
        ...
        for (center, pages) in &clusters {
            if pages.len() >= 5 {
                ...
                let min_page = *sorted_pages.first().unwrap();
                let max_page = *sorted_pages.last().unwrap();
        ...
        self.detected_patterns.insert(PatternType::Clustered, patterns);
        ...
        self.pattern_confidence.insert(PatternType::Clustered, confidence);
    }
    ...
    fn detect_temporal_patterns(&mut self) {
        ...
        self.pattern_confidence.insert(PatternType::Temporal, confidence);
        
        self.detected_patterns.insert(PatternType::Temporal, patterns);
    }
    ...
    fn detect_hotspot_patterns(&mut self) {
        ...
        let hotspots: Vec<u64> = sorted_pages.into_iter()
            .take(hotspot_count)
            .map(|(page_id, _)| page_id)
            .collect();
        
        if !hotspots.is_empty() {
            let confidence = (hotspot_count as f64 / self.access_history.len() as f64 * 5.0).min(1.0);
            ...
            self.detected_patterns.insert(PatternType::Hotspot, vec![pattern]);
            self.pattern_confidence.insert(PatternType::Hotspot, confidence);
        }
    }
    
    fn predict_next_accesses(&self, current_page: u64, max_predictions: usize) -> Vec<PrefetchRequest> {
...
impl CachePrewarmer {
    ...
    pub fn new(config: PrewarmingConfig) -> Self {
        Self {
            pattern_detector: Arc::new(Mutex::new(
                PatternDetector::new(config.pattern_learning_window)
            )),
    ...
    pub fn record_prefetch_hit(&self, page_id: u64) {
    ...
    pub fn record_prefetch_miss(&self, page_id: u64) {
    ...
    pub fn warm_for_workload(&self, workload_pattern: &WorkloadPattern, allocation: &CacheAllocation) -> Result<()> {
    ...
    fn spawn_warming_worker(&self, name: String, priority: WarmingPriority) -> Result<thread::JoinHandle<()>> {
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                while is_running.load(Ordering::Relaxed) {
                    ...
                    if let Some(request) = queue.write().pop_front() {
    ...
    fn start_pattern_analyzer(&self) -> Result<()> {
        ...
        self.active_warmers.write().insert("pattern_analyzer".to_string(), handle);
        Ok(())
    }
    ...
    fn start_prefetch_scheduler(&self) -> Result<()> {
        ...
        self.active_warmers.write().insert("prefetch_scheduler".to_string(), handle);
        Ok(())
    }
    
    fn trigger_predictive_prefetch(&self, current_page: u64) {
        let predictions = self.pattern_detector.lock()
    ...
    fn get_queue_for_priority(&self, priority: WarmingPriority) -> Arc<RwLock<VecDeque<PrefetchRequest>>> {
        match priority {
    ...
    fn generate_time_series_pages(&self, size: usize) -> Vec<u64> {
        ...
        (0..size as u64).map(|i| now - i).collect()
    }
    ...
    fn generate_recent_time_pages(&self, size: usize) -> Vec<u64> {
        ...
        (0..size as u64).map(|i| now - i * 3600).collect() // Last N hours
    }
...
mod tests {
    ...
    fn test_pattern_detector() {
        ...
        assert!(detector.detected_patterns.contains_key(&PatternType::Sequential));
    }
    ...
    fn test_warming_strategies() {
        ...
        let allocation = crate::cache::adaptive_sizing::CacheAllocation::new(
            512 * 1024 * 1024,
            &workload_pattern,
        );
