...
use crate::index::{IndexKey, JoinType, IndexManager};
...
impl QueryPlanner {
        ...
        match condition {
            QueryCondition::Equals { field, value: _ } => {
                if let Some(index_name) = self.find_best_index_for_field(field) {
                    ...
                    Err(Error::Generic(format!("No index available for field: {}", field)))
                }
            }
            ...
            _ => {
                ...
                Err(Error::Generic("Condition type not yet implemented".to_string()))
            }
        }
    }
    ...
    fn count_index_lookups(&self, steps: &[ExecutionStep]) -> usize {
        steps.iter().filter(|step| matches!(step, ExecutionStep::IndexLookup { .. })).count()
    }
    
    /// Count range scans in the execution plan
    fn count_range_scans(&self, steps: &[ExecutionStep]) -> usize {
        steps.iter().filter(|step| matches!(step, ExecutionStep::RangeScan { .. })).count()
    }
    
    /// Execute a planned query
    pub fn execute_plan(&self, plan: &ExecutionPlan, index_manager: &IndexManager) -> Result<Vec<Vec<u8>>> {
        let mut intermediate_results: HashMap<usize, Vec<Vec<u8>>> = HashMap::new();
        let mut final_results = Vec::new();
        
        for (step_id, step) in plan.steps.iter().enumerate() {
            match step {
                ExecutionStep::IndexLookup { index_name, condition, .. } => {
                    let results = self.execute_index_lookup(index_name, condition, index_manager)?;
                    intermediate_results.insert(step_id, results);
                }
                ExecutionStep::RangeScan { index_name, start_key, end_key, .. } => {
                    let results = self.execute_range_scan(index_name, start_key, end_key, index_manager)?;
                    intermediate_results.insert(step_id, results);
                }
                ExecutionStep::Join { left_step, right_step, join_type, .. } => {
                    if let (Some(left_results), Some(right_results)) = 
                        (intermediate_results.get(left_step), intermediate_results.get(right_step)) {
                        let join_results = self.execute_join(left_results, right_results, join_type)?;
                        intermediate_results.insert(step_id, join_results);
                    }
                }
                ExecutionStep::Filter { input_step, condition, .. } => {
    ...
    fn execute_index_lookup(&self, index_name: &str, condition: &QueryCondition, index_manager: &IndexManager) -> Result<Vec<Vec<u8>>> {
        if let Some(index) = index_manager.get_index(index_name) {
            match condition {
    ...
    fn execute_range_scan(&self, index_name: &str, start_key: &IndexKey, end_key: &IndexKey, index_manager: &IndexManager) -> Result<Vec<Vec<u8>>> {
        if let Some(index) = index_manager.get_index(index_name) {
            let results = index.range(start_key, end_key)?;
            Ok(results.into_iter().map(|(_, primary_key)| primary_key).collect())
        } else {
            Err(Error::Generic("Index not found".to_string()))
        }
    }
    
    fn execute_join(&self, left_results: &[Vec<u8>], right_results: &[Vec<u8>], _join_type: &JoinType) -> Result<Vec<Vec<u8>>> {
    ...
    fn execute_filter(&self, input_results: &[Vec<u8>], _condition: &QueryCondition) -> Result<Vec<Vec<u8>>> {
