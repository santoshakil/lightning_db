...
async fn test_comprehensive_security_integration() {
    ...
    // Test authentication
    let auth_manager =
        AuthenticationManager::new("test_secret".to_string(), Duration::from_secs(3600));
    ...
    let token = auth_manager
        .authenticate("testuser", "securepassword123", None)
        .unwrap();
    ...
    let _memory_guard = resource_manager
        .allocate_memory(10, "test".to_string())
        .unwrap();
    ...
    security_monitor
        .report_failed_authentication(None, Some("testuser".to_string()))
        .await
        .unwrap();
