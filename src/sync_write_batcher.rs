...
impl SyncWriteBatcher {
    pub fn new(db: Arc<Database>, batch_size: usize, max_delay_ms: u64) -> Arc<Self> {
        ...
        thread::spawn(move || {
            BatchWorker::new(db, receiver, batch_size, max_delay_ms, worker_shutdown, worker_stats).run();
        });
    ...
    pub fn put(&self, key: Vec<u8>, value: Vec<u8>) -> Result<()> {
        ...
        self.sender.send(BatchCommand::Write {
            key,
            value,
            result_sender,
    ...
    pub fn flush(&self) -> Result<()> {
        self.sender.send(BatchCommand::Flush)
            .map_err(|_| Error::Generic("Write batcher channel closed".to_string()))
    }
...
impl BatchWorker {
    ...
    fn run(mut self) {
        while !self.shutdown.load(Ordering::Relaxed) {
            ...
            match self.receiver.recv_timeout(Duration::from_millis(10)) {
                Ok(BatchCommand::Write { key, value, result_sender }) => {
    ...
    fn execute_batch(&mut self) -> Result<()> {
        ...
        self.stats.writes_completed.fetch_add(count, Ordering::Relaxed);
        Ok(())
    }
}
