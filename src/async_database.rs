use crate::async_storage::{AsyncStorage, AsyncWAL, AsyncTransaction, AsyncIOConfig, AsyncBatchProcessor, AsyncBatchResult};
use crate::async_page_manager::AsyncPageManager;
use crate::async_wal::AsyncWriteAheadLog;
...
impl AsyncDatabase {
    ...
    pub async fn create<P: AsRef<Path>>(
        ...
        let storage = Arc::new(AsyncPageManager::create(
            storage_path,
    ...
    pub async fn open<P: AsRef<Path>>(
        ...
        let page_manager = Arc::new(RwLock::new(PageManager::open(
            &db_path.join("btree.db"),
        )?));
    ...
    pub async fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        // Write to WAL first
        self.wal.append(WALOperation::Put {
            key: key.to_vec(),
            value: value.to_vec(),
        }).await?;
        ...
        tokio::task::spawn_blocking(move || {
            btree.write().insert(&key, &value)
        }).await.map_err(|e| crate::error::Error::Generic(e.to_string()))??;
...
impl AsyncBatchProcessor for AsyncDatabaseBatchProcessor {
    async fn process_write_batch(&self, writes: Vec<(Vec<u8>, Vec<u8>)>) -> Result<()> {
        ...
        if writes.len() > MAX_BATCH_SIZE {
            return Err(crate::error::Error::Generic(
                format!("Batch size {} exceeds maximum {}", writes.len(), MAX_BATCH_SIZE)
            ));
        }
    ...
    async fn process_read_batch(&self, keys: Vec<Vec<u8>>) -> Result<Vec<Option<Vec<u8>>>> {
        ...
        if keys.len() > MAX_BATCH_SIZE {
            return Err(crate::error::Error::Generic(
                format!("Batch size {} exceeds maximum {}", keys.len(), MAX_BATCH_SIZE)
            ));
        }
