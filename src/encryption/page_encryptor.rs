...
use super::{EncryptionAlgorithm, EncryptionMetadata, generate_nonce, constant_time_eq};
...
impl PageEncryptor {
    ...
    fn deserialize_encrypted_page(&self, data: &[u8]) -> Result<(EncryptionMetadata, Vec<u8>)> {
        if data.len() < 4 {
            return Err(Error::Encryption("Invalid encrypted page format".to_string()));
        }
        ...
        let (metadata, _): (EncryptionMetadata, _) = bincode::decode_from_slice(&data[4..4 + metadata_len], bincode::config::standard())
...
mod tests {
    ...
    fn create_test_encryptor() -> (PageEncryptor, Arc<KeyManager>) {
        ...
        let encryptor = PageEncryptor::new(key_manager.clone(), config.algorithm).unwrap();
    ...
    fn test_page_encryption_decryption() {
        ...
        // Encrypt
        let encrypted = encryptor.encrypt_page(page_id, plaintext).unwrap();
        assert_ne!(encrypted, plaintext);
        
        // Decrypt
        let decrypted = encryptor.decrypt_page(page_id, &encrypted).unwrap();
    ...
    fn test_different_pages_different_ciphertexts() {
        ...
        let encrypted1 = encryptor.encrypt_page(1, plaintext).unwrap();
        let encrypted2 = encryptor.encrypt_page(2, plaintext).unwrap();
        
        // Same plaintext should produce different ciphertexts for different pages
        assert_ne!(encrypted1, encrypted2);
        
        // Both should decrypt correctly
        assert_eq!(encryptor.decrypt_page(1, &encrypted1).unwrap(), plaintext);
        assert_eq!(encryptor.decrypt_page(2, &encrypted2).unwrap(), plaintext);
    }
    ...
    fn test_tampered_ciphertext_fails() {
        ...
        let mut encrypted = encryptor.encrypt_page(page_id, plaintext).unwrap();
    ...
    fn test_wrong_page_id_fails() {
        ...
        let encrypted = encryptor.encrypt_page(100, plaintext).unwrap();
