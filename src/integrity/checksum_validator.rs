...
use crate::storage::{Page, PageManager, PageManagerAsync, PageType};
...
impl ChecksumValidator {
    ...
    pub async fn validate_page(&self, page_id: u64) -> Result<bool> {
        ...
        if page.page_type == PageType::Data {
            if !self.validate_key_values(&page, page_id).await {
                return Ok(false);
            }
        }
    ...
    /// Validate page data checksum
    fn validate_page_data(&self, page: &Page, page_id: u64) -> bool {
    ...
    fn validate_metadata_page(&self, page: &Page, page_id: u64) -> bool {
