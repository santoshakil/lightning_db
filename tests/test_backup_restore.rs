use lightning_db::{BackupConfig, BackupManager, Database, LightningDbConfig};
use std::thread;
...
fn test_incremental_backup() {
    ...
    let full_metadata = backup_manager
        .create_backup(&db_path, &full_backup_path)
        .unwrap();

    println!(
        "Full backup created: {} files, {} bytes",
...
fn test_compressed_backup() {
    ...
    let uncompressed_metadata = manager
        .create_backup(&db_path, &uncompressed_backup)
        .unwrap();
    ...
    println!(
        ...
        uncompressed_metadata.total_size
    );
    ...
    manager
        .restore_backup(&compressed_backup, &restore_path)
        .unwrap();
