use lightning_db::storage::{OptimizedPageManager, MmapConfig, Page};
...
fn test_optimized_page_manager_performance() {
    ...
    println!("  Write time: {:?} ({:.2} pages/sec)", 
        write_duration, NUM_PAGES as f64 / write_duration.as_secs_f64());
    println!("  Sync time: {:?}", sync_duration);
    println!("  Random read time: {:?} ({:.2} pages/sec)", 
        read_duration, NUM_PAGES as f64 / read_duration.as_secs_f64());
...
fn test_database_with_optimized_mmap() {
    ...
    println!("  Write time: {:?} ({:.2} ops/sec)", 
        write_duration, NUM_KEYS as f64 / write_duration.as_secs_f64());
    println!("  Read time: {:?} ({:.2} ops/sec)", 
        read_duration, NUM_KEYS as f64 / read_duration.as_secs_f64());
}