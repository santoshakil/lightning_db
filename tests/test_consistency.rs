use lightning_db::{ConsistencyConfig, ConsistencyLevel, Database, LightningDbConfig};
...
fn test_consistency_levels() {
    ...
    db.put_with_consistency(b"key1", b"value1", ConsistencyLevel::Eventual)
        .unwrap();
    let result = db
        .get_with_consistency(b"key1", ConsistencyLevel::Eventual)
        .unwrap();
    assert_eq!(result, Some(b"value1".to_vec()));

    // Test strong consistency
    db.put_with_consistency(b"key2", b"value2", ConsistencyLevel::Strong)
        .unwrap();
    let result = db
        .get_with_consistency(b"key2", ConsistencyLevel::Strong)
        .unwrap();
    assert_eq!(result, Some(b"value2".to_vec()));

    // Test linearizable consistency
    db.put_with_consistency(b"key3", b"value3", ConsistencyLevel::Linearizable)
        .unwrap();
    let result = db
        .get_with_consistency(b"key3", ConsistencyLevel::Linearizable)
        .unwrap();
...
fn test_session_consistency() {
    ...
    db.put_with_consistency(b"session_key", b"session_value", ConsistencyLevel::Session)
        .unwrap();

    // Immediate read should see the write
    let result = db
        .get_with_consistency(b"session_key", ConsistencyLevel::Session)
        .unwrap();
    ...
    // Reader thread with eventual consistency
    {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..10 {
                let key = format!("concurrent_key_{}", i);
                if let Ok(Some(_)) =
                    db_clone.get_with_consistency(key.as_bytes(), ConsistencyLevel::Eventual)
                {
                    found_count += 1;
                }
            }
    ...
    // Reader thread with strong consistency
    {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..10 {
                let key = format!("concurrent_key_{}", i);
                if let Ok(Some(_)) =
                    db_clone.get_with_consistency(key.as_bytes(), ConsistencyLevel::Strong)
                {
                    found_count += 1;
                }
            }
...
fn test_consistency_timeout() {
    ...
    db.put_with_consistency(b"timeout_key", b"timeout_value", ConsistencyLevel::Strong)
        .unwrap();
