...
fn test_heavy_concurrent_load() {
    ...
    assert!(
        total_errors < 10,
        "Too many errors during concurrent operations"
    );
    assert!(
        total_successes > (num_writers * operations_per_thread),
...
fn test_memory_pressure() {
    ...
    if let Some(stats_str) = db.cache_stats() {
        ...
        assert!(
            stats_str.contains("hit") || stats_str.contains("miss"),
...
fn test_transaction_conflicts() {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..increments_per_thread {
                ...
                loop {
                    ...
                    // Read current value
                    let current = db_clone
                        .get_tx(tx_id, key.as_bytes())
                        .unwrap()
                    ...
                    db_clone
                        .put_tx(tx_id, key.as_bytes(), new_value.as_bytes())
                        .unwrap();
                    ...
                    match db_clone.commit_transaction(tx_id) {
                        ...
                        Err(_) => {
                            ...
                            let backoff =
                                std::cmp::min(100 * (1 << std::cmp::min(retries, 6)), 1000);
    ...
    assert_eq!(
        total_count, expected_total as i32,
        "Incorrect total count after concurrent increments"
    );
}
...
fn test_crash_recovery_simulation() {
    ...
    // Phase 1: Normal operations followed by abrupt termination
    {
        ...
        for i in 0..20 {
            db.put_tx(
                tx_id,
                format!("uncommitted_{}", i).as_bytes(),
                b"should_not_exist",
            )
            .unwrap();
        }
...
fn test_long_running_with_checkpoints() {
    ...
    assert!(
        final_checkpoints >= 5,
        "Should have multiple checkpoints during long-running operations"
    );
...
fn test_graceful_shutdown() {
    ...
    // Create database and perform operations
    {
        ...
        for i in 0..100 {
            db.put(
                format!("shutdown_test_{}", i).as_bytes(),
                format!("value_{}", i).as_bytes(),
            )
            .unwrap();
        }
