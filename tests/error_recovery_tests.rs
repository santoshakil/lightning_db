...
fn test_wal_corrupted_header() {
    ...
    match result {
        Ok(recovered_db) => {
            ...
            recovered_db.put(b"test_after_recovery", b"test_value").unwrap();
            assert_eq!(recovered_db.get(b"test_after_recovery").unwrap(), Some(b"test_value".to_vec()));
        }
        Err(_) => {
...
fn test_wal_invalid_checksums() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(format!("{:?}", e).contains("checksum") || format!("{:?}", e).contains("Corruption"));
        }
    }
}
...
fn test_simultaneous_btree_lsm_corruption() {
    ...
    match result {
        Ok(recovered_db) => {
            ...
            recovered_db.put(b"dual_corruption_recovery", b"success").unwrap();
        }
        Err(e) => {
...
fn test_memory_corruption_during_flush() {
    ...
    for thread_id in 0..5 {
        let db_clone = Arc::clone(&db);
        let handle = thread::spawn(move || -> std::result::Result<(), Box<dyn std::error::Error + Send + Sync>> {
            for i in 0..100 {
    ...
    assert_eq!(db.get(b"post_flush_stress").unwrap(), Some(b"functional".to_vec()));
}
...
fn test_cascading_failures_across_components() {
    ...
    match result {
        Ok(recovered_db) => {
            ...
            recovered_db.put(b"cascading_recovery_test", b"works").unwrap();
        }
        Err(e) => {
            println!("Cascading failure appropriately handled: {:?}", e);
            // Should be a specific corruption error
            let error_str = format!("{:?}", e).to_lowercase();
            assert!(error_str.contains("corrupt") || error_str.contains("invalid") || error_str.contains("checksum"));
        }
    }
}
    ...
    // Phase 3: Recovery and verification
    {
        ...
        for i in 0..100 {
            ...
            assert_eq!(
                ...
                "Data consistency failed for key {}", key
            );
        }
        ...
        for i in 0..20 {
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Transaction consistency failed for key {}", key
            );
        }
...
fn test_transaction_atomicity_after_crash() {
    ...
    // Start transaction and crash before commit
    {
        ...
        for i in 0..10 {
            ...
            db.put_tx(tx_id, key.as_bytes(), new_value.as_bytes()).unwrap();
        }
    ...
    // Recovery: transaction should be rolled back
    {
        ...
        for i in 0..10 {
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Transaction atomicity violated: key {} was modified", key
            );
        }
        ...
        for i in 10..20 {
            ...
            assert_eq!(
                value, None,
                "Transaction atomicity violated: key {} exists from uncommitted transaction", key
            );
        }
    }
}
...
fn test_phantom_reads_prevention_after_recovery() {
    ...
    let initial_range_size = {
        ...
        // Read initial range  
        let initial_range = db.range(Some(b"range_key_020"), Some(b"range_key_080")).unwrap();
    ...
    // After recovery, range should be unchanged
    {
        ...
        let recovered_range = recovered_db.range(Some(b"range_key_020"), Some(b"range_key_080")).unwrap();
        
        assert_eq!(
            recovered_range.len(), initial_range_size,
            "Phantom reads detected: range size changed from {} to {}", 
            initial_range_size, recovered_range.len()
        );
        ...
        for i in (21..79).step_by(2) {
            ...
            assert_eq!(
                value, None,
                "Phantom read detected: key {} exists after recovery", key
            );
        }
    ...
    // Recovery should rebuild indexes consistently
    {
        ...
        for i in 0..50 {
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Index inconsistency detected for key {}", key
            );
        }
        
        // Test range operations (require consistent indexing)
        let range_result = recovered_db.range(Some(b"indexed_key_010"), Some(b"indexed_key_040")).unwrap();
        assert!(
            range_result.len() >= 20,
...
fn test_recovery_during_active_transactions() {
    ...
    for idx in (0..5).step_by(2) {
        for i in 0..20 {
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Committed transaction data missing for key {}", key
            );
        }
    }
    ...
    for idx in (1..5).step_by(2) {
        for i in 0..20 {
            ...
            assert_eq!(
                value, None,
                "Uncommitted transaction data found for key {}", key
            );
        }
    }
}
...
fn test_recovery_with_pending_compactions() {
    ...
    for i in 0..1000 {
        ...
        assert_eq!(
            actual_value,
            Some(expected_value.into_bytes()),
            "Data lost during compaction recovery for key {}", key
        );
    }
    
    // Database should be functional after recovery
    recovered_db.put(b"post_compaction_recovery", b"works").unwrap();
    assert_eq!(
...
fn test_recovery_with_partial_writes() {
    ...
    match result {
        Ok(recovered_db) => {
            ...
            recovered_db.put(b"partial_write_recovery_test", b"success").unwrap();
            ...
            assert!(surviving_keys > 0, "No data survived partial write recovery");
        }
        Err(e) => {
...
fn test_recovery_from_disk_full_conditions() {
    ...
    recovered_db.put(b"disk_full_recovery_test", b"works").unwrap();
    assert_eq!(
...
fn test_recovery_from_memory_leaks() {
    ...
    for i in 0..50 {
        ...
        assert_eq!(
            actual_value,
            Some(expected_value.into_bytes()),
            "Memory leak recovery failed for key {}", key
        );
    }
    
    // Check transaction count is reasonable
    let stats = db.stats();
    assert_eq!(stats.active_transactions, 0, "Memory leak: active transactions not cleaned up");
}
...
fn test_recovery_from_lock_contention() {
    ...
    for thread_id in 0..5 { // Check subset to avoid timeout
        for i in 0..10 {
            let key = format!("contention_key_{:02}_{:02}", thread_id, i);
    ...
    println!("Lock contention recovery: {} successful writes", successful_writes);
    // Should have some successful writes despite contention
    assert!(successful_writes > 0, "No writes succeeded during lock contention test");
}
...
fn test_recovery_from_cache_corruption() {
    ...
    for i in 0..100 {
        ...
        assert_eq!(
            actual_value,
            Some(expected_value.into_bytes()),
            "Cache corruption affected data integrity for key {}", key
        );
    }
    ...
    for i in 100..150 {
        ...
        assert_eq!(
            actual_value,
            Some(expected_value.into_bytes()),
            "Cache rebuild failed for key {}", key
        );
    }
}
...
fn test_recovery_from_extreme_fragmentation() {
    ...
    for cycle in 0..5 {
        for i in (1..100).step_by(2) { // Only odd keys should remain
            let key = format!("frag_key_{:02}_{:03}", cycle, i);
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Fragmentation recovery failed for key {}", key
            );
        }
    }
    ...
    for cycle in (0..3).step_by(2) {
        for i in 0..50 {
            ...
            assert_eq!(
                actual_value,
                Some(expected_value.into_bytes()),
                "Transaction fragmentation recovery failed for key {}", key
            );
        }
    }
    ...
    for cycle in (1..3).step_by(2) {
        for i in 0..50 {
            ...
            assert_eq!(
                value, None,
                "Aborted transaction data found during fragmentation test for key {}", key
            );
        }
    }
    ...
    println!("Fragmentation performance - Write: {:?}, Read: {:?}", write_duration, read_duration);
    
    // Operations should complete in reasonable time despite fragmentation
    assert!(
        write_duration < std::time::Duration::from_secs(5),
        "Write performance severely degraded by fragmentation: {:?}", write_duration
    );
    assert!(
        read_duration < std::time::Duration::from_secs(2),
        "Read performance severely degraded by fragmentation: {:?}", read_duration
    );
}
...
fn test_recovery_from_resource_exhaustion() {
    ...
    for i in 0..10 {
        ...
        assert_eq!(
            actual_value,
            Some(expected_value.into_bytes()),
            "Resource exhaustion recovery failed for key {}", key
        );
    }
    ...
    println!("Resource exhaustion recovery: {} committed transactions survived", surviving_committed);
    
    if resource_limit_hit {
        // If we actually hit limits, at least some committed data should survive
        assert!(surviving_committed > 0, "No committed transactions survived resource exhaustion");
    }
}