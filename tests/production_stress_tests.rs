...
fn test_sustained_high_write_throughput_1m() {
    ...
    for batch_idx in 0..(TOTAL_OPS / BATCH_SIZE) {
        ...
        if ops_completed % 50_000 == 0 {
            ...
            println!("Progress: {}/{} ops ({:.0} ops/sec)", ops_completed, TOTAL_OPS, ops_per_sec);
        }
    }
    ...
    println!("Completed 1M writes in {:.2}s ({:.0} ops/sec)", 
             total_duration.as_secs_f64(), final_ops_per_sec);
    ...
    println!("Verified 1M entries in {:.2}s ({:.0} ops/sec)", 
             verify_duration.as_secs_f64(), verify_ops_per_sec);
    
    assert!(final_ops_per_sec > 50_000.0, "Write throughput too low: {:.0}", final_ops_per_sec);
    assert!(verify_ops_per_sec > 100_000.0, "Read verification too slow: {:.0}", verify_ops_per_sec);
}
...
fn test_sustained_high_read_throughput_cache_pressure() {
    ...
    println!("Starting {} random reads with cache pressure...", READ_ITERATIONS);
    ...
    for i in 0..READ_ITERATIONS {
        ...
        if read_duration.as_nanos() < 1_000 { // < 1μs = likely cache hit
            cache_hits += 1;
        } else {
            cache_misses += 1;
        }
        ...
        if i % 100_000 == 0 && i > 0 {
            ...
            println!("Progress: {}/{} reads ({:.0} ops/sec, {:.1}% hit ratio)", 
                     i, READ_ITERATIONS, ops_per_sec, hit_ratio * 100.0);
        }
    }
    ...
    println!("Completed {} reads in {:.2}s ({:.0} ops/sec, {:.1}% hit ratio)", 
             READ_ITERATIONS, total_duration.as_secs_f64(), final_ops_per_sec, final_hit_ratio * 100.0);
    
    assert!(final_ops_per_sec > 100_000.0, "Read throughput under pressure too low: {:.0}", final_ops_per_sec);
}
...
fn test_mixed_workload_patterns() {
    ...
    for (read_pct, write_pct, pattern_name) in test_patterns {
            ...
            let handle = thread::spawn(move || {
                ...
                while completed.load(Ordering::Relaxed) < TOTAL_OPS {
                    ...
                    if op_choice < read_pct {
                        ...
                        let key = format!("mixed_key_{:08}", key_idx);
                        let value = format!("updated_value_{}_{}", key_idx, completed.load(Ordering::Relaxed));
                        db.put(key.as_bytes(), value.as_bytes()).unwrap();
        ...
        println!("{}: {} ops in {:.2}s ({:.0} ops/sec, {} reads, {} writes)",
                 pattern_name, total_completed, duration.as_secs_f64(), 
                 ops_per_sec, total_reads, total_writes);
        
        assert!(ops_per_sec > 10_000.0, "{} pattern too slow: {:.0} ops/sec", pattern_name, ops_per_sec);
    }
}
...
fn test_large_batch_operations() {
    ...
    for &batch_size in BATCH_SIZES {
        ...
        assert!(batch_ops_per_sec > 20_000.0, "Batch {} too slow: {:.0} ops/sec", batch_size, batch_ops_per_sec);
    }
}
...
fn test_100_concurrent_readers() {
    ...
    println!("Completed {} concurrent readers: {} reads in {:.2}s ({:.0} reads/sec, {} errors)",
             NUM_READERS, total_reads, duration.as_secs_f64(), reads_per_sec, errors);
    
    assert_eq!(errors, 0, "Should have no read errors");
    assert!(reads_per_sec > 50_000.0, "Concurrent read performance too low: {:.0}", reads_per_sec);
...
fn test_50_concurrent_writers() {
    ...
    println!("Completed {} concurrent writers: {} writes in {:.2}s ({:.0} writes/sec, {} errors)",
             NUM_WRITERS, total_writes, duration.as_secs_f64(), writes_per_sec, errors);
    ...
    assert!(writes_per_sec > 10_000.0, "Concurrent write performance too low: {:.0}", writes_per_sec);
    assert_eq!(total_writes, NUM_WRITERS * WRITES_PER_WRITER, "Missing writes");
    assert_eq!(verified, NUM_WRITERS * WRITES_PER_WRITER, "Data integrity check failed");
}
...
fn test_reader_writer_contention() {
    ...
    println!("Setting up {} initial keys for contention test...", INITIAL_KEYS);
    ...
    println!("Starting reader-writer contention test: {} readers, {} writers for {:?}...",
    ...
    println!("Contention test completed in {:.2}s:", duration.as_secs_f64());
    println!("  Reads: {} ({:.0}/sec, {} errors)", total_reads, reads_per_sec, read_errs);
    println!("  Writes: {} ({:.0}/sec, {} errors)", total_writes, writes_per_sec, write_errs);
    println!("  Total ops: {} ({:.0}/sec)", 
             total_reads + total_writes,
             (total_reads + total_writes) as f64 / duration.as_secs_f64());
    
    assert!(reads_per_sec > 1_000.0, "Read performance under contention too low: {:.0}", reads_per_sec);
    assert!(writes_per_sec > 500.0, "Write performance under contention too low: {:.0}", writes_per_sec);
    assert!((read_errs as f64 / total_reads as f64) < 0.01, "Too many read errors: {}/{}", read_errs, total_reads);
    assert!((write_errs as f64 / total_writes as f64) < 0.01, "Too many write errors: {}/{}", write_errs, total_writes);
}
...
fn test_transaction_isolation_extreme_concurrency() {
    ...
    println!("Testing transaction isolation with {} concurrent transactions...", NUM_CONCURRENT_TXS);
                            ...
                            match db.get(key.as_bytes()) {
                                Ok(Some(current_value)) => {
                                    ...
                                    if current_value != initial_value {
                                        ...
                                        if !String::from_utf8_lossy(&current_value).starts_with(&our_value) {
                                            isolation_violations.fetch_add(1, Ordering::Relaxed);
                                        }
                                    }
                                }
    ...
    println!("Transaction isolation test completed in {:.2}s:", duration.as_secs_f64());
    println!("  Successful commits: {} ({:.1}%)", successful, success_rate * 100.0);
    println!("  Failed commits: {}", failed);
    println!("  Isolation violations: {}", violations);
    
    assert_eq!(successful + failed, NUM_CONCURRENT_TXS, "Missing transactions");
    assert_eq!(violations, 0, "Transaction isolation violations detected");
    assert!(success_rate > 0.8, "Transaction success rate too low: {:.1}%", success_rate * 100.0);
}
...
fn test_memory_pressure_cache_evictions() {
    ...
    println!("Testing memory pressure with {}KB cache and {}MB data...", 
             cache_size / 1024, (NUM_KEYS * VALUE_SIZE) as u64 / MB);
    
    // Phase 1: Fill cache and force evictions
    println!("Phase 1: Writing {} keys to force cache evictions...", NUM_KEYS);
    ...
    println!("Completed writes in {:.2}s ({:.0} keys/sec)", write_duration.as_secs_f64(), write_rate);
    ...
    for i in 0..READ_ITERATIONS {
        ...
        for (idx, &byte) in value.iter().enumerate() {
            ...
            assert_eq!(byte, expected, "Data corruption at key {} index {}", key_idx, idx);
        }
        
        // Classify as cache hit/miss based on access time
        if access_time.as_nanos() < 10_000 { // < 10μs = likely cache hit
            cache_hits += 1;
        } else {
            cache_misses += 1;
        }
        ...
        if i % 20_000 == 0 && i > 0 {
            ...
            let rate = i as f64 / elapsed.as_secs_f64();
            let hit_ratio = cache_hits as f64 / (cache_hits + cache_misses) as f64;
            println!("  Accessed {} keys ({:.0} keys/sec, {:.1}% hit ratio)", 
                     i, rate, hit_ratio * 100.0);
        }
    }
    ...
    println!("Completed {} reads in {:.2}s ({:.0} keys/sec, {:.1}% hit ratio)",
             READ_ITERATIONS, read_duration.as_secs_f64(), read_rate, final_hit_ratio * 100.0);
    ...
    println!("Sequential access: 1000 keys in {:.2}ms ({:.0} keys/sec)",
             seq_duration.as_millis(), seq_rate);
    
    // Assertions
    assert!(write_rate > 1_000.0, "Write performance under memory pressure too low: {:.0}", write_rate);
    assert!(read_rate > 10_000.0, "Read performance under memory pressure too low: {:.0}", read_rate);
    assert!(final_hit_ratio > 0.05, "Cache hit ratio too low, cache may not be working: {:.1}%", final_hit_ratio * 100.0);
    assert!(seq_rate > 10_000.0, "Sequential access too slow: {:.0}", seq_rate);
}
...
fn test_disk_space_exhaustion_handling() {
    ...
    println!("Testing disk space exhaustion handling (max {} attempts)...", MAX_ATTEMPTS);
        ...
        match db.put(key.as_bytes(), &value) {
            Ok(_) => {
                ...
                if i % 10_000 == 0 && i > 0 {
                    ...
                    println!("  Written {} keys ({} MB, {:.0} keys/sec)", 
    ...
    println!("  Successful writes: {} ({} MB)", successful_writes, mb_written);
    println!("  Duration: {:.2}s ({:.0} keys/sec)", duration.as_secs_f64(), final_rate);
    
    if let Some(failure_point) = first_failure {
        println!("  First failure at: {} ({:.1}% through test)", 
                 failure_point, failure_point as f64 / MAX_ATTEMPTS as f64 * 100.0);
    } else {
    ...
    assert!(successful_writes > 100, "Should have succeeded some writes: {}", successful_writes);
    assert_eq!(verified, verify_count, "Database integrity compromised after disk exhaustion");
}
...
fn test_file_descriptor_limits() {
    ...
    println!("Testing file descriptor limits with {} concurrent transactions...", MAX_TRANSACTIONS);
    ...
    println!("Successfully created {} concurrent transactions", successful_begins);
    
    // Phase 2: Test database is still functional
    println!("Testing database functionality with {} active transactions...", active_transactions.len());
    ...
    println!("Database remained functional: 100 ops in {:.2}ms", test_duration.as_millis());
    ...
    println!("Transaction cleanup completed in {:.2}s: {} committed, {} rolled back",
             cleanup_duration.as_secs_f64(), committed, rollbacks);
    ...
    assert!(successful_begins > 100, "Should create many transactions: {}", successful_begins);
    assert!(committed + rollbacks == successful_begins, "Transaction count mismatch");
}
...
fn test_thread_pool_saturation() {
    ...
    println!("Testing thread pool saturation with {} threads...", NUM_THREADS);
    ...
    println!("Thread pool saturation test completed in {:.2}s:", duration.as_secs_f64());
    println!("  Total operations: {} ({:.0} ops/sec)", total_ops, ops_per_sec);
    println!("  Errors: {} ({:.2}% error rate)", errors, error_rate * 100.0);
    println!("  Expected operations: {}", NUM_THREADS * OPS_PER_THREAD);
    
    // Verify some operations completed successfully
    assert!(total_ops > NUM_THREADS * OPS_PER_THREAD / 2, 
            "Too few operations completed: {} < {}", total_ops, NUM_THREADS * OPS_PER_THREAD / 2);
    assert!(error_rate < 0.1, "Error rate too high: {:.2}%", error_rate * 100.0);
    assert!(ops_per_sec > 1_000.0, "Throughput too low under thread saturation: {:.0}", ops_per_sec);
}
...
fn test_continuous_crash_recovery_cycles() {
    ...
    println!("Testing {} crash/recovery cycles with {} ops each...", NUM_CYCLES, OPS_PER_CYCLE);
    
    let mut total_written = 0;
    let mut recovery_times = Vec::new();
    
    for cycle in 0..NUM_CYCLES {
        println!("Cycle {}/{}: Creating database and writing data...", cycle + 1, NUM_CYCLES);
        ...
        if cycle > 0 {
            ...
            println!("  Write: {:.2}ms, Verify current: {:.2}ms ({}), Verify previous: {:.2}ms ({})",
                     write_time.as_millis(), verify_time.as_millis(), verified_this_cycle,
                     full_verify_time.as_millis(), verified_previous);
        } else {
            println!("  Write: {:.2}ms, Verify: {:.2}ms ({})",
                     write_time.as_millis(), verify_time.as_millis(), verified_this_cycle);
        }
        ...
        if cycle % 10 == 9 {
            println!("Completed {} cycles, {} total keys written", cycle + 1, total_written);
        }
    }
    ...
    println!("Final recovery time: {:.2}ms", final_recovery_time.as_millis());
    ...
    for cycle in 0..NUM_CYCLES {
        ...
        if cycle % 10 == 9 {
            println!("  Verified through cycle {} ({} keys)", cycle + 1, final_verified);
        }
    }
    
    let final_verify_time = final_verify_start.elapsed();
    
    let avg_recovery_time = recovery_times.iter().skip(1).sum::<Duration>() / (recovery_times.len() - 1) as u32;
    ...
    println!("  Final verification: {} keys in {:.2}ms", final_verified, final_verify_time.as_millis());
    println!("  Average recovery time: {:.2}ms", avg_recovery_time.as_millis());
    println!("  Maximum recovery time: {:.2}ms", max_recovery_time.as_millis());
    
    assert_eq!(total_written, TOTAL_EXPECTED_KEYS, "Incorrect number of keys written");
    assert_eq!(final_verified, TOTAL_EXPECTED_KEYS, "Data loss detected in crash recovery");
    assert!(avg_recovery_time.as_millis() < 1000, "Average recovery time too slow: {}ms", avg_recovery_time.as_millis());
    assert!(max_recovery_time.as_millis() < 5000, "Maximum recovery time too slow: {}ms", max_recovery_time.as_millis());
}
...
fn test_recovery_under_load() {
    ...
    println!("Testing recovery under load: {} background threads, {} recovery cycles...", 
    ...
    for cycle in 0..RECOVERY_CYCLES {
                ...
                while !stop_flag.load(Ordering::Relaxed) && local_ops < OPS_PER_THREAD {
                    ...
                    match op_type {
                        0..=70 => {
                            ...
                            let value = format!("recovery_value_{}_{}_cycle_{}", thread_id, local_ops, cycle);
                        ...
                        _ => {
                            ...
                            if let Ok(tx_id) = db.begin_transaction() {
                                ...
                                let value = format!("recovery_tx_value_{}_{}", thread_id, local_ops);
        ...
        assert!(final_ops > 100, "Too few operations completed under load: {}", final_ops);
    }
    ...
    println!("Final recovery completed in {:.2}ms", final_recovery_time.as_millis());
    ...
    println!("Final verification: {} total keys found in {:.2}ms", 
             total_keys, final_verify_time.as_millis());
    
    assert!(total_keys > 1000, "Too few keys survived recovery under load: {}", total_keys);
    assert!(final_recovery_time.as_millis() < 2000, "Final recovery too slow: {}ms", final_recovery_time.as_millis());
}
...
fn test_24_hour_continuous_operation() {
    ...
    let memory_handle = thread::spawn(move || {
        ...
        while !memory_stop.load(Ordering::Relaxed) {
            ...
            if memory_samples.len() % 12 == 0 { // Every hour
            ...
            while !stop_flag.load(Ordering::Relaxed) {
                ...
                let result = match op_type {
                    ...
                    801..=900 => {
                        ...
                        for i in 0..batch_size {
                            let key = format!("stability_batch_{}_{}_{}", worker_id, operation_counter, i);
                            let value = format!("batch_value_{}_{}_{}", worker_id, operation_counter, i);
    ...
    let monitoring_handle = thread::spawn(move || {
        ...
        while !monitoring_stop.load(Ordering::Relaxed) {
            ...
            println!("Hour {}: {} ops total ({} this hour), {} errors ({:.3}%), {} keys",
    ...
    println!("Final integrity check: {} keys verified in {:.2}ms",
             final_range.len(), integrity_time.as_millis());
    
    // Assertions for test success
    assert!(final_duration >= TEST_DURATION - Duration::from_secs(60), "Test ended too early");
    assert!(final_ops > 1_000_000, "Too few operations in 24 hours: {}", final_ops);
    assert!((final_errors as f64 / final_ops as f64) < 0.001, "Error rate too high: {:.4}%", 
            final_errors as f64 / final_ops as f64 * 100.0);
...
fn test_memory_leak_detection() {
    ...
    println!("Testing memory leak detection over {} cycles...", TEST_CYCLES);
    ...
    for cycle in 0..TEST_CYCLES {
        ...
        for i in 0..OPS_PER_CYCLE {
            ...
            // Read back randomly
            if i % 10 == 0 {
                let read_key = format!("leak_test_key_{}_{}", cycle, rand::thread_rng().gen_range(0..i + 1));
            ...
            if i % 50 == 0 {
                ...
                db.put_tx(tx_id, tx_key.as_bytes(), tx_value.as_bytes()).unwrap();
    ...
    let max_keys = memory_usage_samples.iter().map(|(_, keys)| *keys).max().unwrap_or(0);
    let min_keys = memory_usage_samples.iter().map(|(_, keys)| *keys).min().unwrap_or(0);
    
    let avg_growth = memory_differences.iter().sum::<i64>() as f64 / memory_differences.len() as f64;
    ...
    let growth_trend = (final_keys as f64 - initial_keys as f64) / memory_usage_samples.len() as f64;
    let excessive_growth_samples = memory_differences.iter()
        .filter(|&&diff| diff > 1000)
        .count();
    
    println!("  Growth trend per sample: {:.2}", growth_trend);
    println!("  Samples with excessive growth (>1000): {}", excessive_growth_samples);
    ...
    println!("Memory cleanup test: {} -> {} keys (freed {})",
             pre_cleanup_count, post_cleanup_count, 
             pre_cleanup_count - post_cleanup_count);
    
    // Assertions
    assert!(growth_trend < 10.0, "Memory growth trend too high: {:.2}", growth_trend);
    assert!(excessive_growth_samples < memory_usage_samples.len() / 10, 
            "Too many samples with excessive growth: {}", excessive_growth_samples);
    assert!(post_cleanup_count <= pre_cleanup_count, 
            "Memory not reclaimed after deletions: {} -> {}", pre_cleanup_count, post_cleanup_count);
...
fn test_maximum_key_value_sizes() {
    ...
    for &key_size in &key_sizes {
        ...
        match result {
            Ok(_) => {
                println!("  Successfully stored {}-byte key in {:.2}ms", key_size, duration.as_millis());
                ...
                assert_eq!(retrieved.as_deref(), Some(value.as_bytes()));
                println!("  Successfully retrieved in {:.2}ms", retrieve_duration.as_millis());
            }
            Err(e) => {
    ...
    for &value_size in &value_sizes {
        println!("Testing value size: {} bytes ({:.1} MB)", value_size, value_size as f64 / 1_048_576.0);
        ...
        match result {
            Ok(_) => {
                ...
                println!("  Successfully stored {:.1}MB value in {:.2}ms ({:.1} MB/s)", 
                         value_size as f64 / 1_048_576.0, duration.as_millis(), throughput);
                ...
                let retrieve_throughput = value_size as f64 / retrieve_duration.as_secs_f64() / 1_048_576.0;
                println!("  Successfully retrieved in {:.2}ms ({:.1} MB/s)", 
                         retrieve_duration.as_millis(), retrieve_throughput);
            }
            Err(e) => {
                println!("  Failed to store {:.1}MB value: {:?}", value_size as f64 / 1_048_576.0, e);
    ...
    match combo_result {
        Ok(_) => {
            println!("Successfully stored large key + large value in {:.2}ms", combo_duration.as_millis());
...
fn test_billions_of_tiny_keys() {
    ...
    println!("Testing {} tiny keys ({}B key, {}B value)...", TOTAL_KEYS, KEY_SIZE, VALUE_SIZE);
    ...
    for batch_idx in 0..(TOTAL_KEYS / BATCH_SIZE) {
        ...
        if batch_idx % 100 == 0 {
            ...
            let estimated_total_time = elapsed.as_secs_f64() * TOTAL_KEYS as f64 / total_written as f64;
            
            println!("Batch {}/{}: {} keys total ({:.0} keys/sec, batch: {:.2}ms, ETA: {:.1}min)",
                     batch_idx + 1, TOTAL_KEYS / BATCH_SIZE, total_written, keys_per_sec,
                     batch_duration.as_millis(), (estimated_total_time - elapsed.as_secs_f64()) / 60.0);
        }
    }
    ...
    println!("  {} keys written in {:.2}s", TOTAL_KEYS, write_duration.as_secs_f64());
    ...
    println!("  {} successful reads out of {} attempts", successful_reads, READ_SAMPLES);
    println!("  {} errors", read_errors);
    println!("  {:.0} reads/sec", read_keys_per_sec);
    println!("  Average latency: {:.2}μs", read_duration.as_nanos() as f64 / READ_SAMPLES as f64 / 1000.0);
    ...
    let scan_result: Result<Vec<(Vec<u8>, Vec<u8>)>, lightning_db::core::error::Error> = Ok(scan_result.unwrap().into_iter().take(1000).collect::<Vec<_>>());
    let scan_duration = scan_start.elapsed();
    
    match scan_result {
        Ok(entries) => {
            println!("Range scan: {} entries in {:.2}ms ({:.0} entries/sec)",
                     entries.len(), scan_duration.as_millis(),
    ...
    println!("  Data integrity: {:.2}%", actual_keys as f64 / TOTAL_KEYS as f64 * 100.0);
    
    // Assertions
    assert_eq!(actual_keys, TOTAL_KEYS, "Key count mismatch");
    assert!(write_keys_per_sec > 50_000.0, "Write performance too slow: {:.0} keys/sec", write_keys_per_sec);
    assert!(read_keys_per_sec > 100_000.0, "Read performance too slow: {:.0} keys/sec", read_keys_per_sec);
    assert_eq!(read_errors, 0, "Should have no read errors for existing keys");
...
fn test_deeply_nested_transactions() {
    ...
    println!("Testing deeply nested transactions (depth {})...", MAX_NESTING_DEPTH);
    ...
    println!("  {} chains completed in {:.2}ms", chains_completed, concurrent_duration.as_millis());
    ...
    for rollback_depth in [5, 15, 25, 35, 45] {
        ...
        println!("  Rollback depth {}: {:.2}ms", rollback_depth, rollback_tx_time.as_millis());
    }
    
    let rollback_duration = rollback_start.elapsed();
    println!("Rollback tests completed in {:.2}ms", rollback_duration.as_millis());
    
    println!("Deeply nested transaction test completed successfully");
    
    // Assertions
    assert_eq!(verified_keys, MAX_NESTING_DEPTH * OPERATIONS_PER_LEVEL, "Not all nested transaction data committed");
    assert_eq!(chains_completed, CONCURRENT_CHAINS, "Not all concurrent chains completed");
    assert!(tx_ops_completed > 0, "No transaction operations completed in concurrent test");
    assert!(total_time.as_millis() < 10_000, "Nested transaction performance too slow: {}ms", total_time.as_millis());
}
...
fn test_rapid_connection_cycles() {
    ...
    for cycle in 0..NUM_CYCLES {
        ...
        if cycle % 100 == 99 {
            ...
            println!("Cycle {}: avg {:.2}ms/cycle ({:.0} cycles/sec)",
                     cycle + 1, avg_cycle_time.as_millis(), cycles_per_sec);
        }
    }
    ...
    println!("  {} cycles in {:.2}s ({:.0} cycles/sec)", NUM_CYCLES, total_duration.as_secs_f64(), cycles_per_sec);
    println!("  {} operations total ({:.0} ops/sec)", total_operations, ops_per_sec);
    println!("  Cycle times: avg {:.2}ms, min {:.2}ms, max {:.2}ms",
             avg_cycle_time.as_millis(), min_cycle_time.as_millis(), max_cycle_time.as_millis());
    ...
    let mut verified_keys = 0;
    for cycle in (0..NUM_CYCLES).step_by(50) { // Check every 50th cycle
        for op in (0..OPS_PER_CYCLE).step_by(10) { // Check every 10th operation
            let key = format!("rapid_cycle_{}_op_{}", cycle, op);
    ...
    // Assertions
    assert!(cycles_per_sec > 10.0, "Connection cycle rate too slow: {:.0} cycles/sec", cycles_per_sec);
    assert!(ops_per_sec > 1000.0, "Operation rate too slow during rapid cycles: {:.0} ops/sec", ops_per_sec);
    assert!(avg_cycle_time.as_millis() < 500, "Average cycle time too slow: {}ms", avg_cycle_time.as_millis());
    assert!(max_cycle_time.as_millis() < 2000, "Maximum cycle time too slow: {}ms", max_cycle_time.as_millis());
    assert!(final_key_count > NUM_CYCLES * OPS_PER_CYCLE / 2, "Too much data loss during rapid cycles");
    assert!(verified_keys > 0, "No spot-checked keys found after rapid cycles");
