...
use lightning_db::property_testing::{PropertyTester, PropertyTestConfig};
use std::sync::Arc;
...
fn test_acid_properties() {
    ...
    assert!(report.success_rate >= 95.0, 
           "ACID property tests should have high success rate, got {:.1}%", report.success_rate);
    
    // Ensure we ran a reasonable number of tests
    assert!(report.total_tests >= 500, "Should run comprehensive test suite");
}
...
fn test_concurrent_data_integrity() {
    ...
    assert!(total_keys >= expected_keys * 8 / 10, // Allow some variance
           "Expected around {} keys, found {}", expected_keys, total_keys);
}
...
fn test_transaction_atomicity() {
    ...
    assert_eq!(db.get(b"tx_key1").unwrap().as_deref(), Some(b"tx_value1".as_ref()));
    assert_eq!(db.get(b"tx_key2").unwrap().as_deref(), Some(b"tx_value2".as_ref()));
    assert_eq!(db.get(b"tx_key3").unwrap().as_deref(), Some(b"tx_value3".as_ref()));
...
fn test_edge_cases() {
    ...
    assert_eq!(db.get(b"").unwrap().as_deref(), Some(b"empty_key_value".as_ref()));

    // Empty value
    db.put(b"empty_value_key", b"").unwrap();
    assert_eq!(db.get(b"empty_value_key").unwrap().as_deref(), Some(b"".as_ref()));

    // Large key
    let large_key = vec![b'K'; 1024];
    db.put(&large_key, b"large_key_value").unwrap();
    assert_eq!(db.get(&large_key).unwrap().as_deref(), Some(b"large_key_value".as_ref()));

    // Large value
    let large_value = vec![b'V'; 64 * 1024]; // 64KB
    db.put(b"large_value_key", &large_value).unwrap();
    assert_eq!(db.get(b"large_value_key").unwrap().as_deref(), Some(large_value.as_ref()));

    // Special characters in keys
    let special_key = b"key\x00\xFF\x01\x7F";
    db.put(special_key, b"special_chars").unwrap();
    assert_eq!(db.get(special_key).unwrap().as_deref(), Some(b"special_chars".as_ref()));

    // Unicode in values (treated as bytes)
    let unicode_value = "Hello 世界! 🌍".as_bytes();
    db.put(b"unicode_key", unicode_value).unwrap();
    assert_eq!(db.get(b"unicode_key").unwrap().as_deref(), Some(unicode_value));
...
fn test_durability_and_recovery() {
    ...
    // Phase 1: Write data
    {
        ...
        let tx_id = db.begin_transaction().unwrap();
        db.put_tx(tx_id, b"tx_recovery", b"tx_recovery_value").unwrap();
        db.commit_transaction(tx_id).unwrap();
    ...
    // Phase 2: Reopen and verify data
    {
        ...
        assert_eq!(db.get(b"tx_recovery").unwrap().as_deref(), Some(b"tx_recovery_value".as_ref()));
...
fn test_performance_properties() {
    ...
    assert!(write_ops_per_sec > 1000.0, "Write performance should exceed 1K ops/sec");
    assert!(read_ops_per_sec > 10000.0, "Read performance should exceed 10K ops/sec");
    ...
    assert!(avg_write_latency < Duration::from_millis(10), "Write latency should be < 10ms");
    assert!(avg_read_latency < Duration::from_millis(1), "Read latency should be < 1ms");
    ...
    for (key, expected_value) in &expected_state {
        ...
        match (expected_value, actual_value) {
            (Some(expected), Some(actual)) => {
                assert_eq!(actual, expected.as_bytes(), "Value mismatch for key: {}", key);
            }
            (None, None) => {
                // Both expect and actual are None - consistent
            }
            (Some(expected), None) => {
                panic!("Expected value '{}' for key '{}', but key not found", expected, key);
            }
            (None, Some(actual)) => {
                panic!("Expected key '{}' to be deleted, but found value: {:?}", key, actual);
            }
        }
    }

    println!("  ✅ Data consistency maintained across {} operations", expected_state.len());
}
...
fn test_comprehensive_property_suite() {
    ...
    assert!(report.total_tests >= 1000, "Should run comprehensive test suite");
    assert!(report.success_rate >= 98.0, "Should have very high success rate for production database");
    assert!(report.failed_tests < report.total_tests / 50, "Failed tests should be minimal");
    ...
    assert_eq!(critical_violations, 0, "No critical data integrity or consistency violations allowed");
