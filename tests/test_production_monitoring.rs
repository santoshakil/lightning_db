...
use lightning_db::monitoring::production_hooks::{
    MonitoringEvent, MonitoringHook, OperationType, HealthStatus
};
...
fn test_monitoring_hooks() {
    ...
    let slow_ops: Vec<_> = events.iter()
...
fn test_custom_monitoring_events() {
    ...
    // Check event types
    let has_health_check = events.iter().any(|e| matches!(e, MonitoringEvent::HealthCheck { .. }));
    let has_memory_usage = events.iter().any(|e| matches!(e, MonitoringEvent::MemoryUsage { .. }));
    let has_cache_eviction = events.iter().any(|e| matches!(e, MonitoringEvent::CacheEviction { .. }));
...
fn test_concurrent_monitoring() {
    ...
    // Check events
    let events = test_hook.get_events();
    println!("Recorded {} events during concurrent operations", events.len());
    
    // Should have recorded many events
    assert!(events.len() > 0, "Should have recorded events during concurrent operations");
    
    // Count slow operations
    let slow_ops = events.iter()
        .filter(|e| matches!(e, MonitoringEvent::SlowOperation { .. }))
        .count();
    
    println!("Found {} slow operations during concurrent execution", slow_ops);
}