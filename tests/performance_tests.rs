...
fn test_write_performance() {
    let test_db = TestDatabase::new(TestDbConfig {
        use_memory: false, // Use disk for realistic performance testing
        ..Default::default()
    });
    let db = &test_db.db;

    const NUM_OPS: usize = MEDIUM_TEST_SIZE; // Reduced from 10k to 1k for faster tests
    ...
    assert!(
        ...
        "Write performance too low: {:.0}",
        writes_per_sec
    );
    assert!(
        reads_per_sec > 10_000.0,
        "Read performance too low: {:.0}",
        reads_per_sec
    );
    assert!(
        items_per_sec > 100_000.0,
        "Scan performance too low: {:.0}",
        items_per_sec
    );
}
...
fn test_transaction_performance() {
    ...
    const NUM_TXS: usize = 100; // Reduced from 1000
    const OPS_PER_TX: usize = 5; // Reduced from 10
    ...
    assert!(
        txs_per_sec > 1_000.0,
        "Transaction performance too low: {:.0}",
        txs_per_sec
    );
    assert!(
        ops_per_sec > 10_000.0,
        "Operations performance too low: {:.0}",
        ops_per_sec
    );
}
...
fn test_concurrent_read_write_performance() {
    let test_db = TestDatabase::new_fast();
    let db = Arc::new(test_db.db); // Note: This moves ownership, but test is short
    ...
    let handles: Vec<_> = (0..4)
        .map(|thread_id| {
            let db = Arc::clone(&db);
            thread::spawn(move || {
                for i in 0..250 { // Reduced from 2500
    ...
    println!(
        "Concurrent write performance: {:.0} ops/sec",
        write_ops_per_sec
    );
    ...
    println!(
        "Concurrent read performance: {:.0} ops/sec",
        read_ops_per_sec
    );
...
fn test_batch_operation_performance() {
    ...
    let keys: Vec<_> = (0..NUM_BATCHES)
        .flat_map(|batch_id| {
            (0..BATCH_SIZE).map(move |i| format!("batch_{}_key_{:04}", batch_id, i).into_bytes())
        })
        .collect();
    ...
    println!(
        "Batch read performance: {:.0} ops/sec",
        batch_read_ops_per_sec
    );
    ...
    assert!(
        batch_ops_per_sec > 10_000.0,
        "Batch write performance too low"
    );
    assert!(
        batch_read_ops_per_sec > 50_000.0,
        "Batch read performance too low"
    );
}
...
fn test_memory_pressure_performance() {
    ...
    println!(
        "\nAverage write time under memory pressure: {:?}",
        avg_write_time
    );
    ...
    println!(
        "Average read time under memory pressure: {:?}",
        avg_read_time
    );
...
fn test_compression_performance() {
    ...
    for i in 0..100 {
        ...
        db_no_compression
            .put(key.as_bytes(), compressible_data.as_bytes())
            .unwrap();
    }
    ...
    println!(
        "Read time without compression: {:?}",
        no_compression_read_time
    );
    ...
    println!(
        ...
        no_compression_stats.page_count
    );
...
fn test_scan_performance() {
    ...
    let range_results = db
        .range(Some(b"scan_key_001000"), Some(b"scan_key_002000"))
        .unwrap();
