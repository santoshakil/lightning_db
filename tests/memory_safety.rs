...
fn test_concurrent_transactions() {
    ...
    for thread_id in 0..3 {
        ...
        let handle = thread::spawn(move || {
            ...
            for _ in 0..50 {
                ...
                let value: i32 = std::str::from_utf8(&current).unwrap().parse().unwrap();
    ...
    for handle in handles {
        ...
        println!(
            "Thread {}: {} commits, {} conflicts",
            thread_id, success, conflicts
        );
    ...
    assert!(
        count <= total_success,
        "Counter {} should not exceed successful commits {}",
        count,
        total_success
    );
