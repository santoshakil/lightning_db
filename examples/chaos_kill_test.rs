...
fn run_kill_tests() {
    ...
    let test_cases = vec![
        ...
        ("transaction", "before_commit", "Kill before transaction commit"),
        ("transaction", "during_commit", "Kill during transaction commit"),
        ("transaction", "after_commit", "Kill after transaction commit"),
...
fn verify_database_recovery(db_path: &std::path::Path) -> Result<(usize, bool), Box<dyn std::error::Error>> {
    ...
    if let Ok(report) = lightning_db::integrity::verify_database_integrity(db_path) {
        let has_critical_errors = report.errors.iter()
...
fn print_test_summary(results: &[(&str, TestResult)]) {
    ...
    if failed > 0 {
        ...
        for (desc, result) in results.iter().filter(|(_, r)| !r.success) {
            println!("   - {}: {}", desc, result.error.as_ref().unwrap_or(&"Unknown error".to_string()));
        }
    }
    ...
    if failed == 0 {
        ...
        println!("   {} scenarios need attention for full kill -9 resilience.", failed);
    } else {
