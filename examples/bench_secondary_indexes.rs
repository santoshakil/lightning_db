use lightning_db::{
    Database, LightningDbConfig, IndexKey, IndexQuery, 
    SimpleRecord, WalSyncMode
};
use std::sync::Arc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 2: Insert data with automatic indexing
    {
        ...
        for i in 0..count {
            ...
            record.set_field("name".to_string(), format!("User_{}", i % 1000).into_bytes());
            record.set_field("email".to_string(), format!("user{}@example.com", i).into_bytes());
            record.set_field("age".to_string(), format!("{}", 20 + (i % 50)).into_bytes());
            record.set_field("department".to_string(), format!("Dept_{}", i % 10).into_bytes());
        ...
        println!("  • Inserted {} records in {:.2}s", count, duration.as_secs_f64());
        println!("  • Indexed writes: {:.0} ops/sec", ops_per_sec);
        println!("  • Status: {}", if ops_per_sec >= 1_000.0 { "✅ GOOD" } else { "⚠️ SLOW" });
    }
    ...
    // Test 3: Single key lookups
    {
        ...
        println!("  • Name lookup found {} results in {:.2}μs", results.len(), duration.as_micros());
        ...
        println!("  • Email lookup in {:.2}μs: {}", duration.as_micros(), 
        ...
        println!("  • Composite lookup found {} results in {:.2}μs", results.len(), duration.as_micros());
    }
    ...
    // Test 4: Range queries
    {
        ...
        let duration = start.elapsed();
        println!("  • Age range [25-35] found {} results in {:.2}ms", 
                 results.len(), duration.as_millis());
        println!("  • Range query performance: {}", 
                 if duration.as_millis() < 10 { "🚀 EXCELLENT" } 
                 else if duration.as_millis() < 50 { "✅ GOOD" } 
                 else { "⚠️ SLOW" });
    }
    ...
    // Test 5: Bulk lookup performance
    {
        ...
        println!("  • {} lookups found {} results in {:.2}ms", 
                 lookup_count, found_count, duration.as_millis());
        println!("  • Index lookup rate: {:.0} lookups/sec", lookups_per_sec);
        println!("  • Status: {}", 
                 if lookups_per_sec >= 100_000.0 { "🚀 EXCELLENT" }
                 else if lookups_per_sec >= 10_000.0 { "✅ GOOD" }
    ...
    // Test 6: Update with index maintenance
    {
        ...
        for i in 0..update_count {
            ...
            old_record.set_field("name".to_string(), format!("User_{}", i % 1000).into_bytes());
            old_record.set_field("email".to_string(), format!("user{}@example.com", i).into_bytes());
            ...
            new_record.set_field("name".to_string(), format!("User_{}", i % 1000).into_bytes());
            new_record.set_field("email".to_string(), format!("user{}@example.com", i).into_bytes());
        ...
        println!("  • {} indexed updates in {:.2}s", update_count, duration.as_secs_f64());
        println!("  • Update rate: {:.0} ops/sec", updates_per_sec);
        println!("  • Status: {}", 
                 if updates_per_sec >= 1_000.0 { "✅ GOOD" } else { "⚠️ SLOW" });
    }
    ...
    // Test 7: Index statistics
    {
        ...
        println!("  • Index consistency: {}", 
                 if consistent { "✅ VERIFIED" } else { "❌ INCONSISTENT" });
    }
