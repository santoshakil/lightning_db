use lightning_db::lock_free::{
    LockFreeMetricsCollector, OperationTimer, OperationType, ShardedCache,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Writes: {} ({:.0} ops/sec)",
        metrics_snapshot.writes, metrics_snapshot.writes_per_sec
    );
    println!(
        "  Reads:  {} ({:.0} ops/sec)",
        metrics_snapshot.reads, metrics_snapshot.reads_per_sec
    );
    println!(
        "  Deletes: {} ({:.0} ops/sec)",
        metrics_snapshot.deletes, metrics_snapshot.deletes_per_sec
    );
    println!(
        "  Total:  {} ({:.0} ops/sec)",
        metrics_snapshot.total_ops, metrics_snapshot.ops_per_sec
    );
    println!();
    ...
    println!(
        "  Write: {:.1} μs avg",
        metrics_snapshot.avg_write_latency_us
    );
    println!(
        "  Read:  {:.1} μs avg",
        metrics_snapshot.avg_read_latency_us
    );
    println!(
        "  Delete: {:.1} μs avg",
        metrics_snapshot.avg_delete_latency_us
    );
    println!();
