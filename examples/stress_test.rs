...
fn test_mixed_workload(db: Arc<Database>) {
    ...
    println!(
        "  Writes: {} ({:.1}%)",
        total_writes,
        (total_writes as f64 / total_ops as f64) * 100.0
    );
    println!("  Duration: {:?}", elapsed);
    println!(
...
fn test_large_transactions(db: Arc<Database>) {
    ...
    for tx_id in 0..num_transactions {
        ...
        for i in 0..entries_per_transaction {
            ...
            db.put_tx(tx, key.as_bytes(), &value)
                .expect("Put in transaction failed");
        }
...
fn test_memory_pressure(db: Arc<Database>) {
    ...
    let elapsed = start.elapsed();
    println!(
        "  Successfully stored and retrieved {} large values",
        hit_count
    );
    println!("  Duration: {:?}", elapsed);
    println!("  Cache stats: {:?}\n", db.cache_stats());
...
fn test_crash_recovery(db_path: &str) {
    ...
    println!(
        "  Recovered {}/{} entries after crash",
        recovered, num_entries
    );
    println!(
        "  Recovery rate: {:.1}%\n",
...
fn test_long_running(db: Arc<Database>) {
        ...
        let handle = thread::spawn(move || {
            ...
            while start.elapsed() < duration {
                ...
                if local_ops % 10_000 == 0 {
                    ...
                    local_ops = 0;
                    println!(
                        "  Thread {} checkpoint: {} total ops",
                        thread_id,
                        ops_count_clone.load(Ordering::Relaxed)
                    );
                }
            }
    ...
    println!("  Duration: {:?}", elapsed);
    println!(
        "  Sustained throughput: {:.2} ops/sec",
        total_ops as f64 / elapsed.as_secs_f64()
    );
