use lightning_db::{
    observability::{HealthStatus, Metrics},
...
fn monitor_metrics(metrics: Arc<Metrics>) {
    ...
    loop {
        ...
        let write_rate =
            (snapshot.operations.writes - last_snapshot.operations.writes) as f64 / 5.0;
        let error_rate =
            (snapshot.errors.total_errors - last_snapshot.errors.total_errors) as f64 / 5.0;

        // Print update
        println!(
            "\n📊 [{}s] Metrics Update:",
            snapshot.health.uptime.as_secs()
        );
        println!(
            "   Throughput: {:.0} reads/s, {:.0} writes/s",
        ...
        println!(
            ...
            snapshot.cache.hit_rate_percent, snapshot.cache.hits, snapshot.cache.misses
        );
        println!(
            "   Memory: {:.1} MB, CPU: {:.1}%",
        ...
        if error_rate > 10.0 {
            ...
            metrics.update_health(HealthStatus::Degraded(format!(
                "High error rate: {:.1}/s",
                error_rate
            )));
...
fn print_final_report(metrics: &Arc<Metrics>) {
    ...
    println!(
        ...
        snapshot.performance.throughput_ops_sec
    );
    ...
    if error_rate < 0.001
        && snapshot.cache.hit_rate_percent > 70.0
