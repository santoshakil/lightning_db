use lightning_db::{Database, LightningDbConfig, Key, KeyBatch};
...
fn test_performance_comparison(db: &Database) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let zero_copy_keys: Vec<Key> = byte_keys.iter()
        .map(|k| Key::from(k.as_slice()))
        .collect();
    ...
    println!("    Traditional PUT: {:.0} ops/sec ({:.2} ms)", 
             put_ops_per_sec_traditional, traditional_put_time.as_millis());
    println!("    Zero-copy PUT:   {:.0} ops/sec ({:.2} ms)", 
             put_ops_per_sec_zero_copy, zero_copy_put_time.as_millis());
    println!("    Traditional GET: {:.0} ops/sec ({:.2} ms)", 
             get_ops_per_sec_traditional, traditional_get_time.as_millis());
    println!("    Zero-copy GET:   {:.0} ops/sec ({:.2} ms)", 
             get_ops_per_sec_zero_copy, zero_copy_get_time.as_millis());
...
fn test_memory_usage(_db: &Database) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Large batch - Keys: {}, Size: {} bytes", 
             large_batch.len(), large_batch.total_size());
    ...
    println!("    Inline keys: {}/{} ({:.1}%)", 
             inline_count, inline_keys.len(), 
