...
fn main() {
                ...
                if local_ops % 10 == 0 {
                    ...
                    if let Ok(tx_id) = db_clone.begin_transaction() {
                        for i in 0..5 {
                            ...
                            let value =
                                format!("worker_{}_tx_value_{}_{}", worker_id, i, local_tx_ops);
    ...
    if memory_samples.len() >= 2 {
        ...
        println!("\n💾 MEMORY ANALYSIS:");
        println!(
            "  Initial Memory: {} KB ({:.1} MB)",
            initial_memory,
            initial_memory as f64 / 1024.0
        );
        println!(
            "  Final Memory: {} KB ({:.1} MB)",
            final_memory,
            final_memory as f64 / 1024.0
        );
        println!(
            "  Peak Memory: {} KB ({:.1} MB)",
            peak_memory,
    ...
    match db.begin_transaction() {
        Ok(tx_id) => {
            let tx_result = db
                .put_tx(tx_id, b"tx_test", b"tx_value")
