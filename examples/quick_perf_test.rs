...
fn main() {
    ...
    println!(
        "  10,000 sequential writes: {:?} ({:.0} ops/sec)\n",
        elapsed, throughput
    );
    ...
    println!(
        "  100 transactions (10 ops each): {:?} ({:.0} tx/sec)\n",
        elapsed, throughput
    );
    ...
    println!(
        "  Range scan {} entries: {:?} ({:.0} entries/sec)\n",
    ...
    let throughput = 1000.0 / elapsed.as_secs_f64();
    println!(
        "  1,000 mixed operations: {:?} ({:.0} ops/sec)\n",
        elapsed, throughput
    );
