...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  btree.db: {} bytes ({:.2} MB)", btree_size, btree_size as f64 / 1_048_576.0);
    ...
    if wal_dir.exists() {
        ...
        println!("  WAL total: {} bytes ({:.2} MB)", wal_total_size, wal_total_size as f64 / 1_048_576.0);
    }
    ...
    // Test 2: Reopen and verify all data
    {
        ...
        println!("  Regular entries: {} verified, {} missing", verified, missing);
    ...
    println!("\nFinal btree.db size: {} bytes ({:.2} MB)", 
             final_btree_size, final_btree_size as f64 / 1_048_576.0);
