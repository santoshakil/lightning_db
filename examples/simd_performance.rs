use lightning_db::simd::{
    simd_batch_hash, simd_compare_keys, simd_crc32, simd_find_key, simd_is_zero, simd_prefix_match,
};
...
fn generate_test_keys(rng: &mut StdRng, count: usize, key_size: usize) -> Vec<Vec<u8>> {
    let mut keys: Vec<Vec<u8>> = (0..count)
        .map(|_| (0..key_size).map(|_| rng.random::<u8>()).collect())
        .collect();
...
fn test_batch_hash(keys: &[&[u8]]) {
    // Scalar hash
    let start = Instant::now();
    let _scalar_hashes: Vec<u64> = keys
        .iter()
        .map(|k| {
...
fn test_prefix_matching(keys: &[&[u8]]) {
    ...
    // Scalar prefix match
    let start = Instant::now();
    let scalar_matches: Vec<bool> = keys.iter().map(|k| k.starts_with(prefix)).collect();
    let scalar_duration = start.elapsed();
...
fn test_checksum(keys: &[Vec<u8>]) {
    ...
    // Scalar CRC32
    let start = Instant::now();
    let scalar_crcs: Vec<u32> = test_data
        .iter()
        .map(|data| {
    ...
    // SIMD CRC32
    let start = Instant::now();
    let simd_crcs: Vec<u32> = test_data.iter().map(|data| simd_crc32(data, 0)).collect();
    let simd_duration = start.elapsed();
...
fn test_zero_detection() {
    ...
    for size in sizes {
        ...
        println!(
            "    Zero page - Scalar: {:?}, SIMD: {:?}, Speedup: {:.2}x",
            scalar_duration,
            simd_duration,
            scalar_duration.as_secs_f64() / simd_duration.as_secs_f64()
        );
        ...
        println!(
            "    Non-zero page - Scalar: {:?}, SIMD: {:?}, Speedup: {:.2}x",
            scalar_duration,
            simd_duration,
            scalar_duration.as_secs_f64() / simd_duration.as_secs_f64()
        );
