...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.create_index("idx_multi", vec!["category".to_string(), "brand".to_string()])?;
    ...
    println!("  • Avg latency: {:.2}μs", duration.as_micros() as f64 / iterations as f64);
    println!("  • Status: {}", if queries_per_sec >= 10_000.0 { "✅ EXCELLENT" } else if queries_per_sec >= 5_000.0 { "✅ GOOD" } else { "❌ NEEDS IMPROVEMENT" });
    ...
    println!("  • Avg latency: {:.2}ms", duration.as_millis() as f64 / range_iterations as f64);
    println!("  • Status: {}", if range_queries_per_sec >= 1_000.0 { "✅ EXCELLENT" } else if range_queries_per_sec >= 500.0 { "✅ GOOD" } else { "❌ NEEDS IMPROVEMENT" });
    ...
    println!("  • Avg latency: {:.2}μs", duration.as_micros() as f64 / multi_iterations as f64);
    println!("  • Status: {}", if multi_queries_per_sec >= 8_000.0 { "✅ EXCELLENT" } else if multi_queries_per_sec >= 4_000.0 { "✅ GOOD" } else { "❌ NEEDS IMPROVEMENT" });
    ...
    println!("  • Avg latency: {:.2}ms", duration.as_millis() as f64 / join_iterations as f64);
    println!("  • Status: {}", if joins_per_sec >= 500.0 { "✅ EXCELLENT" } else if joins_per_sec >= 200.0 { "✅ GOOD" } else { "❌ NEEDS IMPROVEMENT" });
    ...
    println!("  • Avg planning time: {:.2}μs", duration.as_micros() as f64 / plan_iterations as f64);
    println!("  • Status: {}", if plans_per_sec >= 50_000.0 { "✅ EXCELLENT" } else if plans_per_sec >= 20_000.0 { "✅ GOOD" } else { "❌ NEEDS IMPROVEMENT" });
    
    // Summary
    println!("\n🎯 SECONDARY INDEX PERFORMANCE SUMMARY:\n");
    println!("✅ Single index lookups: {:.0} queries/sec", queries_per_sec);
    println!("✅ Range queries: {:.0} queries/sec", range_queries_per_sec);
    println!("✅ Multi-column index: {:.0} queries/sec", multi_queries_per_sec);
    ...
    if let Some(value) = db.get(verify_key)? {
        println!("✅ Data verified: {:?}", std::str::from_utf8(&value).unwrap_or("<binary>"));
    } else {
