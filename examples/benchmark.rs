...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let best_read = results1.get_ops_per_sec.max(results2.get_ops_per_sec).max(results3.get_ops_per_sec);
    let best_write = results1.put_ops_per_sec.max(results2.put_ops_per_sec).max(results3.put_ops_per_sec);
    let best_read_latency = results1.get_latency_us.min(results2.get_latency_us).min(results3.get_latency_us);
    let best_write_latency = results1.put_latency_us.min(results2.put_latency_us).min(results3.put_latency_us);
    
    println!("\n🎯 Performance vs Targets:");
    println!("  Read throughput:  {:>10.0} ops/sec (target: 1M+) {}",
        best_read,
        if best_read >= 1_000_000.0 { "✅" } else { "❌" }
    );
    println!("  Write throughput: {:>10.0} ops/sec (target: 100K+) {}",
        best_write,
        if best_write >= 100_000.0 { "✅" } else { "❌" }
    );
    println!("  Read latency:     {:>10.2} μs (target: <1μs) {}",
        best_read_latency,
        if best_read_latency < 1.0 { "✅" } else { "❌" }
    );
    println!("  Write latency:    {:>10.2} μs (target: <10μs) {}",
        best_write_latency,
        if best_write_latency < 10.0 { "✅" } else { "❌" }
    );
    ...
    if best_write > 50_000.0 {
        println!("  • Good write performance ({}+ ops/sec)", (best_write as u64 / 1000) * 1000);
    }
    if best_read > 100_000.0 {
        println!("  • Excellent read performance ({}+ ops/sec)", (best_read as u64 / 1000) * 1000);
    }
    if results1.compression_ratio > 1.0 {
        println!("  • Compression working ({:.1}x reduction)", results1.compression_ratio);
    }
...
fn run_benchmark(path: &std::path::Path, config: LightningDbConfig) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
    ...
    for _ in 0..num_keys {
        let key: String = (0..key_size).map(|_| {
            let c: u8 = rng.random_range(b'a'..=b'z');
            c as char
        }).collect();
        let value: String = (0..value_size).map(|_| {
            let c: u8 = rng.random_range(b'a'..=b'z');
            c as char
        }).collect();
    ...
    results.batch_put_ops_per_sec = (batch_count * batch_size) as f64 / batch_put_duration.as_secs_f64();
    ...
    results.batch_get_ops_per_sec = (batch_count * batch_size) as f64 / batch_get_duration.as_secs_f64();
...
fn print_results(name: &str, results: &BenchmarkResults) {
    println!("\n📈 {} Results:", name);
    println!("  Write Performance:  {:>10.0} ops/sec ({:.2} μs/op)", 
        results.put_ops_per_sec, results.put_latency_us);
    println!("  Read Performance:   {:>10.0} ops/sec ({:.2} μs/op)", 
        results.get_ops_per_sec, results.get_latency_us);
    println!("  Delete Performance: {:>10.0} ops/sec ({:.2} μs/op)", 
        results.delete_ops_per_sec, results.delete_latency_us);
    println!("  Scan Performance:   {:>10.0} scans/sec", results.scan_ops_per_sec);
    println!("  Transaction Perf:   {:>10.0} tx/sec", results.transaction_ops_per_sec);
    println!("  Batch Put Perf:     {:>10.0} ops/sec", results.batch_put_ops_per_sec);
    println!("  Batch Get Perf:     {:>10.0} ops/sec", results.batch_get_ops_per_sec);
    println!("  Mixed Workload:     {:>10.0} ops/sec", results.mixed_ops_per_sec);
