...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ⏱️  Database created in: {:.2}s", start.elapsed().as_secs_f64());
    ...
    println!("   ⏱️  Begin transaction in: {:.3}s", start.elapsed().as_secs_f64());
    
    let start = Instant::now();
    db.put_tx(tx_id, b"tx_key", b"tx_value")?;
    println!("   ⏱️  Put in transaction in: {:.3}s", start.elapsed().as_secs_f64());
    
    let start = Instant::now();
    db.commit_transaction(tx_id)?;
    println!("   ⏱️  Commit transaction in: {:.3}s", start.elapsed().as_secs_f64());
