...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Generated migration: {}",
        add_column_migration.metadata.name
    );
    ...
    let mut ctx = MigrationContext::new(db_path.to_string(), current_version, target_version)
        .with_dry_run(true) // Safe mode for example
        .with_batch_size(1000);

    println!(
        "Migration context created: {} -> {}",
    ...
    let mut tracker = ProgressTracker::new(progress.clone());
    ...
    for i in 1..=3 {
        ...
        tracker.update_migration(version, &format!("Executing migration {}", i), 100);

        let snapshot = tracker.get_progress_snapshot();
        println!(
            "Progress: {}% - {}",
            snapshot.progress_percentage, snapshot.current_step
        );
    }

    // 12. Generate rollback plan
    println!("\n=== Rollback Planning ===");
    let _rollback_manager = RollbackManager::new();
