...
use lightning_db::{
    config_management::{
        ConfigBuilder, ConfigFormat, ConfigManager, ConfigSource, EnvironmentConfig, ResourceLimits,
    },
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load from file
    config_manager.load_from_source(ConfigSource::File("demo_config.json".into()))?;

    let loaded_config = config_manager.get_config();
    println!("Loaded configuration from file:");
    println!(
        "  Cache size: {} MB",
        loaded_config.cache_size / 1024 / 1024
    );
    println!(
        "  Max transactions: {}",
        loaded_config.max_active_transactions
    );
    println!();
    ...
    println!(
        "  Cache size: {} GB",
        perf_config.cache_size / 1024 / 1024 / 1024
    );
    ...
    println!(
        "  Cache size: {} GB",
        env_config.cache_size / 1024 / 1024 / 1024
    );
    ...
    let invalid_config = ConfigBuilder::new().cache_size(0).build();
...
fn production_setup() -> Result<ConfigManager, Box<dyn std::error::Error>> {
    ...
    config_manager.load_from_source(ConfigSource::File("/etc/lightning_db/config.json".into()))?;
    ...
    if args.len() > 1 {
        config_manager.load_from_source(ConfigSource::CommandLine(args[1..].to_vec()))?;
    }
    ...
    config_manager.on_change(|new_config| {
        ...
        println!(
            "  New cache size: {} MB",
            new_config.cache_size / 1024 / 1024
        );
    });
