...
use lightning_db::distributed_tracing::exporter::{MultiExporter, FileExporter, FileFormat, BatchExporter, MetricsExporter, LoggingExporter, LogLevel as ExporterLogLevel};
...
fn main() -> Result<()> {
    ...
    let database = Database::create("example_traced.db", lightning_db::LightningDbConfig::default())?;
...
fn trace_basic_operations(database: &Database) -> Result<()> {
    ...
    match database.put(key, value) {
        ...
        Err(e) => {
            span.record_error(&format!("Failed to create user: {}", e), Some("DB_WRITE_ERROR"));
        }
    }
    
    // Finish the span (automatically done when span goes out of scope)
    let completed_span = span.finish();
    println!("   ✓ Basic operation traced - Duration: {}μs", completed_span.duration_micros());
...
fn trace_nested_operations(database: &Database) -> Result<()> {
    ...
    with_context(parent_context.clone(), || -> Result<()> {
        ...
        parent_span.log(LogLevel::Info, "Starting user profile retrieval".to_string());
        ...
        parent_span.log(LogLevel::Info, "User profile retrieved successfully".to_string());
        
        let completed = parent_span.finish();
        println!("   ✓ Nested operations traced - Total duration: {}μs", completed.duration_micros());
...
fn trace_cross_service_request(database: &Database) -> Result<()> {
    ...
    with_context(context.clone(), || -> Result<()> {
        ...
        span.log(LogLevel::Info, "Processing cross-service request".to_string());
        
        // Simulate updating user data
        let key = b"user:123";
        let value = br#"{"id": 123, "name": "Alice Updated", "last_modified": "2024-01-15T10:30:00Z"}"#;
        ...
        match database.put(key, value) {
            ...
            Err(e) => {
                span.record_error(&format!("Failed to update user: {}", e), Some("UPDATE_ERROR"));
            }
        }
        ...
        println!("   ✓ Cross-service request traced - Trace ID: {}", context.trace_id);
...
fn trace_transaction_operations(database: &Database) -> Result<()> {
    ...
    with_context(context.clone(), || -> Result<()> {
        ...
        tx_span.log(LogLevel::Info, "Starting money transfer transaction".to_string());
        ...
        for (key, value) in operations {
            ...
            match database.put(key.as_bytes(), value) {
                ...
                Err(e) => {
                    op_span.record_error(&format!("Failed to update {}: {}", key, e), Some("TX_ERROR"));
                    tx_span.record_error(&format!("Transaction failed at {}", key), Some("TX_ROLLBACK"));
        ...
        println!("   ✓ Transaction traced - TX ID: 12345, Duration: {}μs", completed.duration_micros());
...
fn trace_error_scenarios(database: &Database) -> Result<()> {
    ...
    with_context(context.clone(), || -> Result<()> {
        ...
        span.record_success();
        let completed = span.finish();
        println!("   ✓ Error scenarios traced - Duration: {}μs", completed.duration_micros());
...
fn trace_performance_monitoring(database: &Database) -> Result<()> {
    let mut context = TraceContext::new_root();
    context.set_baggage("performance_test".to_string(), "batch_operations".to_string());
    
    with_context(context.clone(), || -> Result<()> {
        let mut batch_span = DatabaseSpans::batch(&context, "PUT", 100).build();
        
        batch_span.set_tag("batch.size".to_string(), "100".to_string());
        batch_span.set_tag("test.type".to_string(), "performance_monitoring".to_string());
        
        batch_span.log(LogLevel::Info, "Starting batch performance test".to_string());
        ...
        for i in 0..100 {
            ...
            if i % 10 == 0 {
                ...
                match database.put(key.as_bytes(), value.as_bytes()) {
                    ...
                    Err(e) => op_span.record_error(&format!("Batch operation failed: {}", e), Some("BATCH_ERROR")),
                }
        ...
        println!("   ✓ Performance monitoring traced - {} operations in {}μs", 
                100, completed.duration_micros());
        println!("     Throughput: {:.2} MB/s", 
                (total_bytes as f64 / 1024.0 / 1024.0) / (completed.duration_micros() as f64 / 1_000_000.0));
