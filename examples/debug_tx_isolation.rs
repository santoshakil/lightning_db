...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Tx1 read: {:?}", val1.as_ref().map(|v| String::from_utf8_lossy(v)));
    
    let val2 = db.get_tx(tx2, b"shared_key")?;
    println!("Tx2 read: {:?}", val2.as_ref().map(|v| String::from_utf8_lossy(v)));
    ...
    println!("Tx2 sees: {:?}", val2_after.as_ref().map(|v| String::from_utf8_lossy(v)));
    ...
    println!("Final value: {:?}", final_val.as_ref().map(|v| String::from_utf8_lossy(v)));
