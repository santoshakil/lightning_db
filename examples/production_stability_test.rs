...
impl StabilityTester {
                    ...
                    let result = match op_type {
                        0..=40 => {
                            ...
                            let value = format!(
                                "value_{}_data_{}",
                                thread_id,
                                "x".repeat(rng.random_range(100..1000))
                            );
                        ...
                        41..=70 => {
                            ...
                            let key = format!(
                                "key_{:08}_{:08}",
                                thread_id,
                                rng.random_range(0..local_op_count.max(1))
                            );
                        ...
                        81..=90 => {
                            ...
                            let tx_result = db.begin_transaction().and_then(|tx_id| {
                                for i in 0..10 {
                                    let key =
                                        format!("tx_key_{}_{}_{}", thread_id, local_op_count, i);
    ...
    fn generate_report(
        ...
        // Find peak metrics
        let peak_metrics = metrics_history
            .iter()
        ...
        if end_metrics.fd_count > start_metrics.fd_count + 100 {
            anomalies.push(format!(
                ...
                start_metrics.fd_count, end_metrics.fd_count
            ));
        }
        ...
        if end_metrics.thread_count > start_metrics.thread_count + 10 {
            anomalies.push(format!(
                "Thread leak: {} -> {}",
                start_metrics.thread_count, end_metrics.thread_count
            ));
        }

        // Check for performance degradation
        if metrics_history.len() > 10 {
            let early_latencies: Vec<_> = metrics_history
                .iter()
                .take(10)
                .map(|m| m.operation_latency)
                .collect();
            let late_latencies: Vec<_> = metrics_history
                .iter()
                .rev()
                .take(10)
                .map(|m| m.operation_latency)
    ...
    fn print_report(&self, report: &StabilityReport) {
        ...
        println!(
            "🕐 Start Time: {:?}",
            report.start_metrics.timestamp.elapsed()
        );
        ...
        println!(
            "📈 Error Rate: {:.4}%",
            report.errors_encountered as f64 / report.operations_performed.max(1) as f64 * 100.0
        );
        ...
        println!(
            "   Initial RSS: {:.2} MB",
            report.start_metrics.memory_rss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Final RSS: {:.2} MB",
            report.end_metrics.memory_rss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Peak RSS: {:.2} MB",
            report.peak_metrics.memory_rss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Initial VSS: {:.2} MB",
            report.start_metrics.memory_vss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Final VSS: {:.2} MB",
            report.end_metrics.memory_vss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Peak VSS: {:.2} MB",
            report.peak_metrics.memory_vss as f64 / 1024.0 / 1024.0
        );
        println!(
            "   Growth: {:.2} MB ({:.2}%)",
            (report.end_metrics.memory_rss as f64 - report.start_metrics.memory_rss as f64)
                / 1024.0
                / 1024.0,
            (report.end_metrics.memory_rss as f64 / report.start_metrics.memory_rss.max(1) as f64
                - 1.0)
                * 100.0
        );
        ...
        println!(
            ...
            if report.end_metrics.fd_count > report.start_metrics.fd_count {
                format!(
                    "+{}",
                    report.end_metrics.fd_count - report.start_metrics.fd_count
                )
            } else {
                "0".to_string()
            }
        ...
        println!(
            ...
            if report.end_metrics.thread_count > report.start_metrics.thread_count {
                format!(
                    "+{}",
                    report.end_metrics.thread_count - report.start_metrics.thread_count
                )
            } else {
                "0".to_string()
            }
        ...
        println!(
            "   Avg latency: {:?}",
            Duration::from_nanos(
                (report.duration.as_nanos() / report.operations_performed.max(1) as u128) as u64
            )
        );
...
fn main() {
    ...
    let duration_mins = args.get(1).and_then(|s| s.parse::<u64>().ok()).unwrap_or(5); // Default 5 minutes
