...
impl BenchmarkResults {
    fn print_comparison(&self, name: &str, baseline: Option<&BenchmarkResults>) {
        println!("\n{} Results:", name);
        println!("  Sequential Write: {:.2}ms", self.sequential_write.as_secs_f64() * 1000.0);
        println!("  Random Write:     {:.2}ms", self.random_write.as_secs_f64() * 1000.0);
        println!("  Sequential Read:  {:.2}ms", self.sequential_read.as_secs_f64() * 1000.0);
        println!("  Random Read:      {:.2}ms", self.random_read.as_secs_f64() * 1000.0);
        println!("  Mixed Workload:   {:.2}ms", self.mixed_workload.as_secs_f64() * 1000.0);
        println!("  Transaction:      {:.2}ms", self.transaction_overhead.as_secs_f64() * 1000.0);
        println!("  Bulk Insert:      {:.2}ms", self.bulk_insert.as_secs_f64() * 1000.0);
        println!("  Bulk Delete:      {:.2}ms", self.bulk_delete.as_secs_f64() * 1000.0);
        
        if let Some(baseline) = baseline {
            println!("\nSpeedup vs baseline:");
            println!("  Sequential Write: {:.2}x", baseline.sequential_write.as_secs_f64() / self.sequential_write.as_secs_f64());
            println!("  Random Write:     {:.2}x", baseline.random_write.as_secs_f64() / self.random_write.as_secs_f64());
            println!("  Sequential Read:  {:.2}x", baseline.sequential_read.as_secs_f64() / self.sequential_read.as_secs_f64());
            println!("  Random Read:      {:.2}x", baseline.random_read.as_secs_f64() / self.random_read.as_secs_f64());
            println!("  Mixed Workload:   {:.2}x", baseline.mixed_workload.as_secs_f64() / self.mixed_workload.as_secs_f64());
            println!("  Transaction:      {:.2}x", baseline.transaction_overhead.as_secs_f64() / self.transaction_overhead.as_secs_f64());
            println!("  Bulk Insert:      {:.2}x", baseline.bulk_insert.as_secs_f64() / self.bulk_insert.as_secs_f64());
            println!("  Bulk Delete:      {:.2}x", baseline.bulk_delete.as_secs_f64() / self.bulk_delete.as_secs_f64());
        }
    }
}

fn benchmark_lightning_db(num_operations: usize) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
    let dir = tempdir()?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (size_name, num_ops) in test_sizes {
        ...
        println!("\nOverall Performance:");
        println!("  Baseline total time: {:.2}ms", baseline_total.as_secs_f64() * 1000.0);
        println!("  Lightning DB total time: {:.2}ms", lightning_total.as_secs_f64() * 1000.0);
        println!("  Overall speedup: {:.2}x", baseline_total.as_secs_f64() / lightning_total.as_secs_f64());
    }
