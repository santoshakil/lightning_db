...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "1. Small key writes ({}B key, {}B value):",
        small_key.len(),
        small_value.len()
    );
    ...
    if us_per_op < 10.0 {
        println!("   ✅ Meets write latency target!");
    }
    ...
    println!("   {:>10.0} ops/sec ({:.3} μs/op)", ops_per_sec, us_per_op);
    if us_per_op < 1.0 {
        println!("   ✅ Meets read latency target!");
    }
