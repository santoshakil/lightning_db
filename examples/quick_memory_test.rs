...
fn main() {
        ...
        let handle = thread::spawn(move || {
            ...
            while worker_start.elapsed() < worker_duration {
                ...
                if local_ops % 5 == 0 {
                    ...
                    let value = format!("w{}_v{}_{}", worker_id, local_ops, 
        ...
        .output()
    {
        if let Ok(memory_str) = String::from_utf8(output.stdout) {
            if let Ok(memory_kb) = memory_str.trim().parse::<u64>() {
                println!("  Current Memory: {} KB ({:.1} MB)", memory_kb, memory_kb as f64 / 1024.0);
    ...
    match db.begin_transaction() {
        Ok(tx_id) => {
            let tx_result = db.put_tx(tx_id, b"test_tx", b"test_val")
