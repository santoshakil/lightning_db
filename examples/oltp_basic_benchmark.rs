...
fn benchmark_batch_ops(db: &Database, num_batches: u64, batch_size: u64) -> BenchmarkMetrics {
    println!(
        "Benchmarking batch operations ({} batches x {} ops)...",
...
fn generate_report(metrics: &[BenchmarkMetrics]) {
    ...
    let avg_throughput: f64 =
        metrics.iter().map(|m| m.ops_per_sec).sum::<f64>() / metrics.len() as f64;

    let avg_p50: u64 = metrics.iter().map(|m| m.p50_latency_us).sum::<u64>() / metrics.len() as u64;

    let avg_p99: u64 = metrics.iter().map(|m| m.p99_latency_us).sum::<u64>() / metrics.len() as u64;
