...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n🎯 PERFORMANCE VS TARGETS:");
    let best_read = performance_results
        .iter()
        .filter(|(op, _, _)| op.contains("Read") || op.contains("Cache"))
        .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
    let best_write = performance_results
        .iter()
    ...
    if let Some((op, ops, lat)) = best_read {
        ...
        println!(
            "  Read:  {} ops/sec ({:.2} μs) - {} (Target: 1M+ ops/sec)",
            ops,
            lat,
            if meets_target {
                "✅ ACHIEVED"
    ...
    if let Some((op, ops, lat)) = best_write {
        ...
        println!(
            "  Write: {} ops/sec ({:.2} μs) - {} (Target: 100K+ ops/sec)",
            ops,
            lat,
            if meets_target {
                "✅ ACHIEVED"
