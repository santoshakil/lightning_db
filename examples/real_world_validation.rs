        ...
        let handle = thread::spawn(move || {
            ...
            for _ in 0..operations_per_thread {
                ...
                if rng.random::<f64>() < 0.9 {
                    ...
                    // Write operation
                    let value =
                        format!("value:{}:ts:{}", key_id, Instant::now().elapsed().as_secs());
...
fn validate_queue_pattern() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _consumer_id in 0..consumers {
        ...
        let handle = thread::spawn(move || {
            ...
            loop {
                ...
                if !found
                    && consumed_clone.load(std::sync::atomic::Ordering::Relaxed)
    ...
    println!(
        ...
        (consumed.load(std::sync::atomic::Ordering::Relaxed) as f64 / total_messages as f64)
            * 100.0
    );
