            ...
            loop {
                ...
                for key in keys {
                    ...
                    match db.get(&key) {
                        Ok(Some(stored_value)) => {
                            if let Some(record) =
                                DataRecord::from_stored_value(key.clone(), &stored_value)
                            {
                                if !record.verify() {
                                    let msg = format!(
                                        "Background check: checksum mismatch for key: {:?}",
                                        key
                                    );
                        ...
                        Err(e) => {
                            let msg =
                                format!("Background check: read error for key {:?}: {}", key, e);
                            anomalies.lock().unwrap().push(msg);
                        }
                    }
                }
...
fn main() {
    ...
    let total_errors = write_errors
        + read_errors
        + integrity_report.errors.len()
        + integrity_report.corrupted_keys;
