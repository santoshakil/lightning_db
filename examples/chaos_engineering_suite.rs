...
impl ChaosEngineer {
        ...
        #[cfg(unix)]
        {
            ...
            if let Ok(stats) = statfs::statfs(temp_dir.path()) {
                ...
                if let Ok(mut file) = fs::File::create(&fill_path) {
                    ...
                    for _ in 0..chunks_to_write.min(100) {
                        // Limit to 100MB for test
    ...
    fn test_memory_pressure(&self) -> ChaosResult {
        ...
        ChaosResult {
            ...
            error_message: if error_count > success_count {
                Some(format!(
                    "Too many failures under memory pressure: {} errors, {} successes",
    ...
    fn test_concurrent_stress(&self) -> ChaosResult {
        ...
        ChaosResult {
            ...
            error_message: if total_errors > total_success {
                Some(format!(
                    "High error rate under stress: {} errors, {} successes",
    ...
    fn test_random_failures(&self) -> ChaosResult {
        ...
        ChaosResult {
            ...
            error_message: if failures > successes {
                Some(format!(
                    "Too many random failures: {} vs {} successes",
                    failures, successes
                ))
            } else {
                None
    ...
    fn print_summary(&self) {
        ...
        if integrity_issues > 0 {
            println!(
                "\n⚠️  DATA INTEGRITY ISSUES: {} scenarios affected data integrity!",
                integrity_issues
            );
        } else {
