use lightning_db::{Database, LightningDbConfig, resource_limits::{ResourceLimits, ResourceEnforcer}};
...
fn main() {
    ...
    println!("\n📋 Resource Limits:");
    println!("   Max memory: {} MB", limits.max_memory_bytes / 1024 / 1024);
    println!("   Max concurrent operations: {}", limits.max_concurrent_operations);
    println!("   Max write throughput: {} MB/s", limits.max_write_throughput_bytes_per_sec / 1024 / 1024);
    println!("   Max read throughput: {} MB/s", limits.max_read_throughput_bytes_per_sec / 1024 / 1024);
    println!("   Max disk usage: {} MB", limits.max_disk_bytes / 1024 / 1024);
...
fn test_concurrent_operations(db: &Arc<Database>, enforcer: &Arc<ResourceEnforcer>) {
    ...
    println!("   ✅ Concurrent operations limited: {} rejected", total_violations);
}
...
fn test_write_throughput(db: &Arc<Database>, enforcer: &Arc<ResourceEnforcer>) {
    ...
    println!("   Written: {:.1} MB in {:?}", total_written as f64 / 1024.0 / 1024.0, elapsed);
    println!("   Actual throughput: {:.1} MB/s", throughput);
    println!("   Throttled {} times for total {:?}", throttled_count, total_throttle_time);
...
fn test_mixed_workload(db: &Arc<Database>, enforcer: &Arc<ResourceEnforcer>) {
    ...
    thread::spawn(move || {
        while start_time.elapsed() < test_duration {
            ...
            println!("      Write throughput: {:.1}%", percentages.write_throughput_percent);
            println!("      Read throughput: {:.1}%", percentages.read_throughput_percent);
        }
    });
    ...
    println!("      Operations rejected: {}", final_stats.operations_rejected);
}
...
fn print_enforcement_report(enforcer: &Arc<ResourceEnforcer>) {
    ...
    if violations.is_empty() {
        ...
        for violation in &violations {
            *violation_counts.entry(format!("{:?}", violation.violation_type)).or_insert(0) += 1;
        }
    ...
    println!("   Concurrent operations: {} ({:.1}%)",
             usage.concurrent_operations,
             percentages.operations_percent);
    ...
    if violations.is_empty() {
        ...
        println!("   📈 ACTIVE - Resource limits actively enforced ({} violations)", violations.len());
    }
