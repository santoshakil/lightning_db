...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Write throughput: {} ({:.0} / 100K ops/sec)",
        if write_target_met {
            "✅ ACHIEVED"
        } else {
            "❌ NOT MET"
    ...
    println!(
        "  Write latency:    {} ({:.2} / 10 μs)",
        if write_latency_met {
            "✅ ACHIEVED"
        } else {
            "❌ NOT MET"
        },
