...
fn test_configuration(name: &str, config: LightningDbConfig) {
    ...
    // Print results
    println!("Sequential Write: {:.2} μs/op ({:.0} ops/sec)", 
             write_latency, 1_000_000.0 / write_latency);
    println!("Sequential Read: {:.2} μs/op ({:.0} ops/sec)", 
             read_latency, 1_000_000.0 / read_latency);
    println!("Concurrent Write: {:.0} ops/sec", concurrent_write_throughput);
    println!("Concurrent Read: {:.0} ops/sec", concurrent_read_throughput);
    println!("Mixed Workload: {:.0} ops/sec ({:.1}% reads)", 
             mixed_stats.0, mixed_stats.1);
    println!("Range Scan: {:.0} entries/sec", scan_throughput);
    
    // Validate against targets
    validate_performance(name, write_latency, read_latency, 
                        concurrent_write_throughput, concurrent_read_throughput);
...
fn mixed_workload_test(db: Arc<Database>) -> (f64, f64) {
    ...
    for _ in 0..NUM_THREADS {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..1000 {
                if rng.random_bool(0.8) { // 80% reads
...
fn validate_performance(
    ...
    println!("  Write latency: {} (target: <{:.0}μs)", 
             if write_pass { "✓ PASS" } else { "✗ FAIL" }, write_target);
    
    // Target: 1M+ reads/sec = <1μs latency  
    let read_target = 1.0;
    let read_pass = read_latency <= read_target;
    println!("  Read latency: {} (target: <{:.0}μs)", 
             if read_pass { "✓ PASS" } else { "✗ FAIL" }, read_target);
    
    // Target: 100K+ concurrent writes/sec
    let cwrite_target = 100_000.0;
    let cwrite_pass = concurrent_write >= cwrite_target;
    println!("  Concurrent writes: {} (target: >{:.0} ops/sec)", 
             if cwrite_pass { "✓ PASS" } else { "✗ FAIL" }, cwrite_target);
    
    // Target: 1M+ concurrent reads/sec
    let cread_target = 1_000_000.0;
    let cread_pass = concurrent_read >= cread_target;
    println!("  Concurrent reads: {} (target: >{:.0} ops/sec)", 
             if cread_pass { "✓ PASS" } else { "✗ FAIL" }, cread_target);
    
    let all_pass = write_pass && read_pass && cwrite_pass && cread_pass;
    println!("  Overall: {}", if all_pass { "✓ ALL TESTS PASS" } else { "✗ SOME TESTS FAIL" });
}