...
fn test_concurrent_conflicts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db = Arc::new(Database::create("./edge_test_db/conflicts", LightningDbConfig::default())?);
    ...
    match (result1, result2) {
        (Ok(_), Ok(_)) => println!("  ⚠️  Both conflicting transactions succeeded (last-write-wins)"),
...
fn test_resource_limits() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..20 {
        match db.begin_transaction() {
            ...
            Err(e) => {
                println!("  ✓ Transaction limit enforced at {} transactions: {}", i, e);
                break;
            }
        }
    }
