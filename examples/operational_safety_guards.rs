use lightning_db::{safety_guards::SafetyGuards, Database, LightningDbConfig};
use std::sync::Arc;
use std::thread;
...
fn test_retention_guard(_db: &Arc<Database>, guards: &Arc<SafetyGuards>) {
    ...
    guards
        .retention_guard
        .protect_key_pattern("system_".to_string());
    guards
        .retention_guard
        .protect_key_pattern("config_".to_string());
...
fn test_emergency_procedures(_db: &Arc<Database>, guards: &Arc<SafetyGuards>) {
    ...
    guards
        .corruption_guard
        .report_corruption("page_123", "Checksum mismatch");
