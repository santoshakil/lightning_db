...
fn test_basic_functionality_stress() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    for i in 0..operations {
            ...
            0..=1 => {
                ...
                if let Err(e) = db.put(key.as_bytes(), value.as_bytes()) {
                    ...
                    if errors > 100 { // Too many errors
                        return Ok(result.failure(start.elapsed(), &format!("Too many write errors: {}", e)));
                    }
                }
            }
            2..=8 => {
                // 70% reads
                if i > 1000 { // Only read after some data exists
                    let read_key = format!("stress_key_{:08}", i % 1000);
                    if let Err(e) = db.get(read_key.as_bytes()) {
                        errors += 1;
                        if errors > 100 {
                            return Ok(result.failure(start.elapsed(), &format!("Too many read errors: {}", e)));
                        }
                    }
                }
            }
            9 => {
                ...
                if i > 1000 {
                    ...
                    if let Err(e) = db.delete(delete_key.as_bytes()) {
                        errors += 1;
                        if errors > 100 {
                            return Ok(result.failure(start.elapsed(), &format!("Too many delete errors: {}", e)));
                        }
                    }
                }
            }
        ...
        if i % 10000 == 0 {
            print!("  Progress: {}/{} ({:.1}%) - Errors: {}\r", 
                   i, operations, (i as f64 / operations as f64) * 100.0, errors);
        }
    }
    ...
    let details = format!("{} ops in {:.2}s ({:.0} ops/sec), {} errors", 
                         operations, duration.as_secs_f64(), ops_per_sec, errors);
    
    if errors < 10 && ops_per_sec > 10000.0 {
        Ok(result.success(duration, &details))
    } else {
        Ok(result.failure(duration, &format!("High error rate or low performance: {}", details)))
    }
}
...
fn test_concurrent_operations() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    let details = format!("{} threads, {} ops total, {:.0} ops/sec, {} errors", 
...
fn test_large_dataset() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    println!("  Writing {} records ({} MB)...", num_records, (num_records * value_size) / (1024 * 1024));
    ...
    let details = format!("{}MB written ({:.0} ops/sec), {} random reads ({:.0} ops/sec), {} read errors", 
                         (num_records * value_size) / (1024 * 1024),
                         write_ops_per_sec, read_count, read_ops_per_sec, read_errors);
...
fn test_memory_pressure() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    for batch in 0..num_batches {
        ...
        for i in 0..keys_per_batch {
            ...
            if let Err(e) = db.put(key.as_bytes(), large_value.as_bytes()) {
                return Ok(result.failure(start.elapsed(), &format!("Memory pressure write failed: {}", e)));
            }
        }
    ...
    let details = format!("{} keys, {}MB total data, {:.2}s", 
                         total_keys, total_data / (1024 * 1024), duration.as_secs_f64());
...
fn test_crash_recovery() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    let duration = start.elapsed();
    let details = format!("Recovery: {:.2}ms, Missing committed: {}, Found uncommitted: {}", 
                         recovery_time.as_millis(), missing_committed, found_uncommitted);
...
fn test_data_integrity_stress() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    while stress_start.elapsed() < stress_duration {
        ...
        match operations % 5 {
            0..=2 => { let _ = db.put(key.as_bytes(), value.as_bytes()); }
            3 => { let _ = db.get(key.as_bytes()); }
            4 => { let _ = db.delete(key.as_bytes()); }
            _ => unreachable!()
        }
    ...
    let details = format!("{} integrity checks, {} stress ops, {} errors in {}s", 
                         num_keys, stress_ops, errors, stress_duration.as_secs());
    
    if errors == 0 {
        Ok(result.success(duration, &details))
    } else {
        Ok(result.failure(duration, &format!("Data integrity compromised: {}", details)))
    }
}
...
fn test_performance_sustainability() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    while start.elapsed() < test_duration {
        ...
        print!("    Time: {:.1}min, Ops/sec: {:.0}, Total ops: {}\r",
               start.elapsed().as_secs_f64() / 60.0, ops_per_sec, total_operations);
    ...
    let avg_performance: f64 = performance_samples.iter().sum::<f64>() / performance_samples.len() as f64;
    let first_half_avg: f64 = performance_samples[..performance_samples.len()/2].iter().sum::<f64>() 
                              / (performance_samples.len()/2) as f64;
    let second_half_avg: f64 = performance_samples[performance_samples.len()/2..].iter().sum::<f64>() 
                               / (performance_samples.len()/2) as f64;
    
    let degradation = ((first_half_avg - second_half_avg) / first_half_avg) * 100.0;
    
    let details = format!("{} ops total, {:.0} avg ops/sec, {:.1}% performance degradation", 
                         total_operations, avg_performance, degradation);
    
    if degradation < 20.0 && avg_performance > 5000.0 {
        Ok(result.success(duration, &details))
    } else {
        Ok(result.failure(duration, &format!("Performance not sustainable: {}", details)))
    }
}
...
fn test_transaction_consistency() -> Result<TestResults, Box<dyn std::error::Error>> {
        ...
        let handle = thread::spawn(move || {
            ...
            for _ in 0..transfers_per_thread {
                ...
                match transfer_money(&db_clone, from_account, to_account, transfer_amount) {
                    Ok(_) => { completed_clone.fetch_add(1, Ordering::Relaxed); }
    ...
    let duration = start.elapsed();
    let details = format!("{} transfers, {} errors, balance error: {}", 
                         completed, errors, balance_error);
    
    if balance_error == 0 && errors < completed / 10 {
        Ok(result.success(duration, &details))
    } else {
        Ok(result.failure(duration, &format!("Transaction consistency issues: {}", details)))
    }
}
...
fn test_edge_cases() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    if edge_case_errors <= 1 { // Allow 1 failure for unsupported edge cases
        Ok(result.success(duration, &details))
    } else {
...
fn test_resource_cleanup() -> Result<TestResults, Box<dyn std::error::Error>> {
    ...
    let duration = start.elapsed();
    let details = format!("10 DB cycles, {} data items found after cleanup", found_data);
    
    if found_data > 500 { // Should find most of the data
        Ok(result.success(duration, &details))
    } else {
...
fn transfer_money(db: &Database, from: usize, to: usize, amount: i32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.put_tx(tx_id, from_key.as_bytes(), new_from_balance.to_string().as_bytes())?;
    db.put_tx(tx_id, to_key.as_bytes(), new_to_balance.to_string().as_bytes())?;
