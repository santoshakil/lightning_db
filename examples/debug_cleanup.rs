...
fn test_cleanup_scenario(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Database created in: {:.3}s", start.elapsed().as_secs_f64());
    ...
    let start = Instant::now();
    drop(db);
    println!("  Database dropped in: {:.3}s", start.elapsed().as_secs_f64());
...
fn test_auto_batcher_cleanup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    batcher.shutdown();
    drop(batcher);
    println!("  Auto batcher dropped in: {:.3}s", start.elapsed().as_secs_f64());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 2: LSM database cleanup
    {
        ...
        println!("  LSM Database created in: {:.3}s", start.elapsed().as_secs_f64());
        ...
        let start = Instant::now();
        drop(db);
        println!("  LSM Database dropped in: {:.3}s", start.elapsed().as_secs_f64());
    ...
    for i in 0..3 {
        ...
        println!("  Full cycle {} took: {:.3}s", i, start.elapsed().as_secs_f64());
    }
