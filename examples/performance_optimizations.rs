...
fn benchmark_memory_layout() {
    ...
    println!(
        "Cache-aligned allocation: {:?} (10k allocations)",
        alloc_time
    );
    ...
    println!(
        "Object pool operations: {:?} (10k get/return cycles)",
        pool_time
    );
...
fn benchmark_cache_friendly_algorithms() {
    ...
    for size in sizes {
        let mut data: Vec<u32> = (0..size).map(|_| rand::rng().random()).collect();
    ...
    println!(
        "Binary search with prefetch: {:?} (10k searches)",
        search_time
    );
    ...
    println!(
        "Boyer-Moore search: {:?} (found {} matches)",
        search_time,
        matches.len()
    );
...
fn benchmark_optimization_manager() {
    ...
    println!(
        "Object pool operations: {:?} (1k get/return cycles)",
        pool_time
    );
    ...
    for (name, workload_type) in workloads {
        ...
        match workload_type {
            ...
            WorkloadType::AnalyticalScan => {
                ...
                let data: Vec<u32> = (0..100000).collect();
                let _results =
                    CacheFriendlyAlgorithms::sequential_scan_optimized(&data, |x| *x % 1000 == 0);
            }
        }
        ...
        println!(
            "  Pool utilization: {:.2}%",
            stats.btree_node_pool_stats.utilization * 100.0
        );
    }
}
