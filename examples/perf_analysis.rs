...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configs = vec![
        ...
        ("Periodic WAL (10ms)", WalSyncMode::Periodic { interval_ms: 10 }, false),
        ("Async + No compression", WalSyncMode::Async, true),
    ];
    
    println!("Testing individual puts ({} operations):\n", test_count);
    println!("{:<25} {:>15} {:>15} {:>15}", "Configuration", "Writes/sec", "μs/op", "Status");
    println!("{}", "-".repeat(75));
    ...
    for (name, sync_mode, disable_compression) in configs {
        ...
        let status = if ops_sec >= 100_000.0 { "✅" } else if ops_sec >= 50_000.0 { "⚠️" } else { "❌" };
        
        println!("{:<25} {:>15.0} {:>15.2} {:>15}", name, ops_sec, us_per_op, status);
    }
    ...
    println!("{:<25} {:>15.0} {:>15.2} {:>15}", "AutoBatcher", ops_sec, us_per_op, status);
    ...
    let status = if ops_sec >= 1_000_000.0 { "✅" } else if ops_sec >= 500_000.0 { "⚠️" } else { "❌" };
    
    println!("{:<25} {:>15} {:>15} {:>15}", "Configuration", "Reads/sec", "μs/op", "Status");
    println!("{}", "-".repeat(75));
    println!("{:<25} {:>15.0} {:>15.2} {:>15}", "Cached reads", ops_sec, us_per_op, status);
