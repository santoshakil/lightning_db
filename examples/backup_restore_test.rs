...
fn create_backup(
    ...
    println!(
        "  ✓ Backup complete: {} MB in {:.2}s ({:.1} MB/s)",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=========================");
    {
        ...
        println!(
            "  ✓ Verified {}/100 sample entries after compressed restore",
            verified
        );
    }
    ...
    println!("====================================");
    {
        ...
        println!(
            "  ✓ Database modified (checksum changed: {})",
    ...
    println!("==================================");
    {
        ...
        match Database::open(&restore_dir, LightningDbConfig::default()) {
            Ok(db) => match db.verify_integrity() {
                Ok(report) => {
    ...
    println!("==================================");
    {
        ...
        for (name, compress) in &[("Uncompressed", false), ("Compressed", true)] {
            let backup_dir = temp_dir
                .path()
                .join(format!("perf_test_{}", name.to_lowercase()));
            fs::create_dir_all(&backup_dir)?;
            ...
            let restore_dir = temp_dir
                .path()
                .join(format!("perf_restore_{}", name.to_lowercase()));
            let start = Instant::now();
