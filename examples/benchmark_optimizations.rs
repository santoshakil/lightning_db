...
fn benchmark_wal_optimizations(dir: &tempfile::TempDir, value: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
...
fn benchmark_thread_local_writes(dir: &tempfile::TempDir, value: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n2. Thread-Local Write Benchmarks:");
    
    let config = LightningDbConfig::default();
    let db = Arc::new(Database::create(dir.path().join("thread_local.db"), config)?);
...
fn benchmark_memory_optimizations(dir: &tempfile::TempDir, value: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test with standard page manager
    {
        ...
        let duration = start.elapsed();
        println!("   Standard page manager: {:.2} ms", duration.as_secs_f64() * 1000.0);
    }
    ...
    // Test with optimized page manager
    {
        ...
        let db = Arc::new(Database::create(dir.path().join("optimized_pm.db"), config)?);
        let count = 1000;
        let start = Instant::now();
        ...
        let duration = start.elapsed();
        println!("   Optimized page manager: {:.2} ms", duration.as_secs_f64() * 1000.0);
    }
