...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ⏱️  Database created in: {:.3}s", start.elapsed().as_secs_f64());
    
    let start = Instant::now();
    let batcher = Database::create_auto_batcher(db.clone());
    println!("   ⏱️  Auto batcher created in: {:.3}s", start.elapsed().as_secs_f64());
    ...
    println!("   📊 Submitted: {}, Completed: {}, Batches: {}, Errors: {}", 
    ...
    match batcher.wait_for_completion() {
        Ok(_) => {
            println!("   ⏱️  Wait completed in: {:.3}s", start.elapsed().as_secs_f64());
        }
        Err(e) => {
            println!("   ❌ Wait failed after: {:.3}s - {}", start.elapsed().as_secs_f64(), e);
        }
    }
