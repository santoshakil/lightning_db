...
impl DatabaseMonitor {
    ...
    pub fn update_performance_metrics(
        ...
        self.cache_hit_rate
            .store((cache_hit_rate * 10000.0) as u64, Ordering::Relaxed); // Store as basis points
        self.active_transactions
            .store(active_txns, Ordering::Relaxed);
...
fn display_alerts(monitor: &DatabaseMonitor) {
    ...
    for alert in recent_alerts {
        ...
        println!(
            "{} [{}] {}: {} (value: {:.1}, threshold: {:.1})",
            severity_emoji, timestamp, alert.id, alert.message, alert.metric_value, alert.threshold
        );
    }
}
