...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if results.len() == 4 {
        ...
        println!("  ❌ Range scan returned wrong count: {} (expected 4)", results.len());
    }
    ...
    println!("  Write: {:.0} ops/sec ({:.2} μs/op)", write_ops, write_elapsed.as_micros() as f64 / 10000.0);
    ...
    println!("  Read: {:.0} ops/sec ({:.2} μs/op)", read_ops, read_elapsed.as_micros() as f64 / 10000.0);
