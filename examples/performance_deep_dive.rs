...
fn analyze_transaction_overhead() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Single-op transactions: {:.0} ops/sec ({:.1}x slower)",
        single_tx_throughput,
        direct_throughput / single_tx_throughput
    );
    println!(
        "   Batched transactions ({}): {:.0} ops/sec ({:.1}x faster than single)",
        batch_size,
        batch_tx_throughput,
        batch_tx_throughput / single_tx_throughput
    );
...
fn analyze_memory_patterns() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &working_set in &working_sets {
        ...
        println!(
            "     Working set {}: {:.0} ops/sec",
            working_set, throughput
        );
    }
...
fn analyze_concurrent_scaling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &num_threads in &thread_counts {
        ...
        println!(
            "     {} threads: {:.0} ops/sec ({:.1}x)",
            num_threads,
            throughput,
            throughput / (ops_per_thread as f64)
        );
    }
