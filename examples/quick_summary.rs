...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Write: {:.0} ops/sec ({:.2} μs) - {}",
        write_ops_sec,
        write_latency_us,
        if write_ops_sec > 100_000.0 {
            "✅ EXCEEDS TARGET"
        } else {
            "Working towards target"
        }
