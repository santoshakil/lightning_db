...
fn test_write_patterns() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    let db = Database::create("./perf_test_db/write_patterns", LightningDbConfig::default())?;
    ...
    if seq_ops > rand_ops * 1.5 {
        observations.push("Sequential writes significantly faster than random (good B+Tree locality)".to_string());
    } else {
        observations.push("Sequential/random write performance similar (possible optimization opportunity)".to_string());
    }
...
fn test_cache_effectiveness() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    if cache_speedup > 10.0 {
        observations.push(format!("Excellent cache effectiveness: {:.1}x speedup", cache_speedup));
    } else if cache_speedup > 5.0 {
        observations.push(format!("Good cache effectiveness: {:.1}x speedup", cache_speedup));
    } else {
        observations.push(format!("Poor cache effectiveness: {:.1}x speedup (needs optimization)", cache_speedup));
    }
    
    // Check metrics
    let metrics = db.get_metrics();
    observations.push(format!("Cache hit rate: {:.2}%", metrics.cache_hit_rate * 100.0));
...
fn test_transaction_overhead() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    observations.push(format!("Single transaction overhead: {:.1}%", single_overhead));
    
    if tx_batch_ops > tx_single_ops * 1.5 {
        observations.push("Batched transactions significantly faster (good amortization)".to_string());
    } else {
        observations.push("Limited benefit from transaction batching (optimization opportunity)".to_string());
    }
...
fn test_compression_impact() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    let write_overhead = (comp_write.as_secs_f64() - no_comp_write.as_secs_f64()) / no_comp_write.as_secs_f64() * 100.0;
    let read_overhead = (comp_read.as_secs_f64() - no_comp_read.as_secs_f64()) / no_comp_read.as_secs_f64() * 100.0;
    
    observations.push(format!("Compression write overhead: {:.1}%", write_overhead));
    ...
    if comp_size > 0 && no_comp_size > 0 {
        let compression_ratio = (no_comp_size as f64 - comp_size as f64) / no_comp_size as f64 * 100.0;
...
fn test_concurrent_patterns() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    let db = Arc::new(Database::create("./perf_test_db/concurrent", LightningDbConfig::default())?);
    let mut observations = Vec::new();
    ...
    observations.push(format!("Read-heavy workload: {:.0} ops/sec with 8 threads", ops_per_sec));
        ...
        handles.push(thread::spawn(move || {
            for i in 0..1000 {
                ...
                match db_clone.put(key.as_bytes(), value.as_bytes()) {
                    ...
                    Err(_) => { contention.fetch_add(1, Ordering::Relaxed); }
                }
            }
        }));
    }
    ...
    if contentions > 0 {
        observations.push(format!("Write contention detected: {} failures", contentions));
    } else {
        observations.push(format!("No write contention with {} concurrent writes", successes));
    }
...
fn test_value_size_impact() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    observations.push(format!("Performance degradation from 10B to 1MB: {:.1}%", degradation));
    
    for (label, write_ops, _read_ops, _) in &results {
        if write_ops < &100000.0 {
            observations.push(format!("{} values: {:.0} writes/sec (below target)", label, write_ops));
        }
    }
...
fn test_iterator_performance() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    observations.push(format!("Range scan rate: {:.0} entries/sec ({} entries)", range_rate, range_count));
    ...
    if prefix_count != 10000 {
        observations.push(format!("Prefix scan returned incorrect count: {} (expected 10000)", prefix_count));
    }
...
fn test_memory_pressure() -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    ...
    for _ in 0..10000 {
        ...
        if latency > 10.0 { // Likely cache miss
            cache_misses += 1;
        }
    }
    
    let final_metrics = db.get_metrics();
    let eviction_rate = (initial_metrics.cache_evictions as f64 - final_metrics.cache_evictions as f64) / 10000.0;
    
    observations.push(format!("Cache miss rate: {:.1}%", cache_misses as f64 / 100.0));
    observations.push(format!("Eviction rate: {:.2} per operation", eviction_rate));
    observations.push(format!("Final cache hit rate: {:.2}%", final_metrics.cache_hit_rate * 100.0));
...
fn print_analysis(results: &[PerformanceResult]) {
    ...
    // Performance table
    println!("\n⚡ Performance Metrics:\n");
    println!("{:<25} | {:>12} | {:>10} | {:>10} | {:>10}", 
             "Test", "Ops/sec", "P50 (μs)", "P99 (μs)", "P99.9 (μs)");
    println!("{}", "-".repeat(80));
    
    for result in results {
        println!("{:<25} | {:>12.0} | {:>10.2} | {:>10.2} | {:>10.2}",
                 result.test_name,
                 result.ops_per_sec,
    ...
    for result in results {
        if result.test_name.contains("Cache") && result.ops_per_sec > 1_000_000.0 {
            ...
            println!("  ✅ Read target (1M ops/sec): {:.1}M ops/sec", result.ops_per_sec / 1_000_000.0);
        }
        if result.test_name.contains("Write") && result.ops_per_sec > 100_000.0 {
            meets_write_target = true;
            println!("  ✅ Write target (100K ops/sec): {:.0}K ops/sec", result.ops_per_sec / 1_000.0);
        }
    }
