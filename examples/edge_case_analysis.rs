...
fn test_empty_database() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Empty database get: {:?} in {:.2}μs", 
             result.is_none(), 
    ...
    println!("   First write: {:.2}μs", first_write_time.as_nanos() as f64 / 1000.0);
    ...
    println!("   Second write: {:.2}μs", second_write_time.as_nanos() as f64 / 1000.0);
    ...
    println!("   Full scan ({} items): {:.2}μs", count, scan_time.as_nanos() as f64 / 1000.0);
...
fn test_key_size_limits() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &key_sizes {
        ...
        println!("     {} bytes: Write {:.2}μs ({}), Read {:.2}μs ({})",
                 size,
...
fn test_value_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Empty value: Write {:.2}μs, Read {:.2}μs, Retrieved: {}",
...
fn test_transaction_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Empty transaction: {:.2}μs", empty_tx_time.as_nanos() as f64 / 1000.0);
    ...
    println!("   Large transaction (1000 ops): Prepare {:.2}ms, Commit {:.2}ms",
             prepare_time.as_millis(), commit_time.as_millis());
    ...
    println!("   Transaction abort (100 ops): {:.2}μs, Data exists: {}",
             abort_time.as_nanos() as f64 / 1000.0, aborted_exists);
...
fn test_concurrent_write_patterns() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (pattern_name, same_key) in patterns {
        ...
        println!("   {} pattern: {}/{} successful, {:.0} ops/sec",
...
fn test_delete_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Verification: Deleted exists: {}, Undeleted exists: {}", 
...
fn test_database_reopen() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Verification ({} samples): {:.2}ms", verified, verify_time.as_millis());
...
fn test_memory_limits() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     Read {} items: {:.2}ms", read_count, read_time.as_millis());
    
    let stats = db.stats();
    println!("     Final stats: {} pages, {} free pages", 
