...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  - Batch deleted: {} items",
        delete_results.iter().filter(|&&r| r).count()
    );
    ...
    println!(
        ...
        String::from_utf8_lossy(&persistent.unwrap())
    );
    ...
    if let Some(lsm_stats) = db2.lsm_stats() {
        ...
        println!(
            "  - Cache hit rate: {:.2}%",
            lsm_stats.cache_hit_rate * 100.0
        );
    }
