...
fn test_memory_leaks() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    for cycle in 0..cycles {
        let db = Database::create(
            temp_dir.path().join(format!("cycle_{}", cycle)),
            config.clone(),
        )?;
            ...
            Some(actual_value) => {
                if actual_value != expected_value.as_bytes() {
                    ...
                    if corruption_errors <= 5 {
                        println!(
                            "      Corruption detected: key={}, expected={}, got={}",
                            key,
                            expected_value,
...
fn test_crash_recovery() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    if !success {
        println!(
            "    Recovery issues: {} missing committed, {} found uncommitted",
...
fn test_concurrent_safety() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    println!(
        "    {} threads, {} ops, {:.0} ops/sec, {} errors, {} slow ops",
...
fn test_performance_load() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    println!(
        "    {} ops in {:.2}s, {:.0} ops/sec, {:.2}μs avg latency",
        operations,
...
fn test_transaction_consistency() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    println!(
        "    {} transfers, {} errors, total balance: {} (expected: {}), error: {}",
...
fn test_large_dataset() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    println!(
        "    {}MB written, {:.1} MB/s write throughput, {}/{} reads successful",
        data_size_mb,
        write_throughput,
        read_count - read_errors,
        read_count
    );
