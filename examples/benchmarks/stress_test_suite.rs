...
fn test_transaction_conflicts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..NUM_THREADS {
        ...
        let handle = thread::spawn(move || {
            ...
            for _ in 0..TRANSFERS_PER_THREAD {
                ...
                if db_clone
                    .put_tx(tx_id, from_key.as_bytes(), &new_from.to_le_bytes())
                    .is_err()
                {
                    let _ = db_clone.abort_transaction(tx_id);
                    conflicts.fetch_add(1, Ordering::Relaxed);
                ...
                if db_clone
                    .put_tx(tx_id, to_key.as_bytes(), &new_to.to_le_bytes())
                    .is_err()
                {
                    let _ = db_clone.abort_transaction(tx_id);
                    conflicts.fetch_add(1, Ordering::Relaxed);
    ...
    results.failed_operations =
        conflict_count.load(Ordering::Relaxed) + error_count.load(Ordering::Relaxed);
    results.duration = duration;
...
fn test_large_batch_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for batch_id in 0..NUM_BATCHES {
        ...
        match db.write_batch(&batch) {
            ...
            Err(e) => {
                ...
                results
                    .errors
                    .push(format!("Batch {} failed: {}", batch_id, e));
            }
        }
    ...
    results.throughput_ops_per_sec =
        results.total_operations as f64 / results.duration.as_secs_f64();
    results.avg_latency_us = results.duration.as_micros() as f64 / NUM_BATCHES as f64;

    // Verify data
    let mut verify_errors = 0;
    for batch_id in 0..NUM_BATCHES.min(10) {
        // Spot check first 10 batches
