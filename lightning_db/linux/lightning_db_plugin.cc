...
#include "lightning_db_plugin_private.h"
...
static void lightning_db_plugin_handle_method_call(
    LightningDbPlugin* self,
...
static void lightning_db_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(lightning_db_plugin_parent_class)->dispose(object);
}

static void lightning_db_plugin_class_init(LightningDbPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = lightning_db_plugin_dispose;
}
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  LightningDbPlugin* plugin = LIGHTNING_DB_PLUGIN(user_data);
  lightning_db_plugin_handle_method_call(plugin, method_call);
}
