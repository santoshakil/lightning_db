...
use lightning_db::{
    ...
    encryption::{EncryptionConfig, EncryptionAlgorithm, KeyDerivationFunction, EncryptionManager},
};
use rand::Rng;
...
fn generate_data(count: usize, key_size: usize, value_size: usize, seed: u64) -> Vec<(Vec<u8>, Vec<u8>)> {
...
fn bench_page_encryption(c: &mut Criterion) {
    ...
    for page_size in &[4096, 8192, 16384, 32768] {
        ...
        group.bench_function(&format!("aes_gcm_{}_bytes", page_size), |b| {
            ...
            b.iter(|| {
                let encrypted = manager.encrypt_page(black_box(1), black_box(&page_data)).unwrap();
                black_box(encrypted);
            });
        });
...
fn bench_page_decryption(c: &mut Criterion) {
    ...
    for page_size in &[4096, 8192, 16384, 32768] {
        ...
        group.bench_function(&format!("aes_gcm_{}_bytes", page_size), |b| {
            ...
            b.iter(|| {
                let decrypted = manager.decrypt_page(black_box(1), black_box(&encrypted)).unwrap();
                black_box(decrypted);
            });
        });
...
fn bench_wal_encryption(c: &mut Criterion) {
    ...
    for entry_size in &[64, 256, 1024, 4096] {
        ...
        group.bench_function(&format!("encrypt_{}_bytes", entry_size), |b| {
            ...
            let mut entry_id = 0;
            b.iter(|| {
                let encrypted = manager.encrypt_wal_entry(black_box(entry_id), black_box(&entry_data)).unwrap();
                entry_id += 1;
                black_box(encrypted);
            });
        });
        ...
        group.bench_function(&format!("decrypt_{}_bytes", entry_size), |b| {
            ...
            b.iter(|| {
                let decrypted = manager.decrypt_wal_entry(black_box(1), black_box(&encrypted)).unwrap();
                black_box(decrypted);
            });
        });
    }
