...
fn bench_basic_operations(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("put_operations", size), size, |b, &size| {
...
fn bench_transaction_operations(c: &mut Criterion) {
    ...
    for batch_size in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("transaction_batch", batch_size), batch_size, |b, &batch_size| {
            let temp_dir = tempdir().unwrap();
            let db = Database::create(temp_dir.path(), LightningDbConfig::default()).unwrap();
            ...
            b.iter(|| {
                ...
                for i in 0..batch_size {
                    ...
                    db.put_tx(black_box(tx_id), black_box(key.as_bytes()), black_box(value.as_bytes())).unwrap();
                }
...
fn bench_mixed_workload(c: &mut Criterion) {
    ...
    for &size in sizes.iter() {
        ...
        group.bench_with_input(BenchmarkId::new("mixed_70_read_30_write", size), &size, |b, &size| {
            let temp_dir = tempdir().unwrap();
            let db = Database::create(temp_dir.path(), LightningDbConfig::default()).unwrap();
...
fn bench_different_value_sizes(c: &mut Criterion) {
    ...
    for &value_size in value_sizes.iter() {
        ...
        group.bench_with_input(BenchmarkId::new("put_variable_size", value_size), &value_size, |b, &value_size| {
            let temp_dir = tempdir().unwrap();
            let db = Database::create(temp_dir.path(), LightningDbConfig::default()).unwrap();
        ...
        group.bench_with_input(BenchmarkId::new("get_variable_size", value_size), &value_size, |b, &value_size| {
            let temp_dir = tempdir().unwrap();
            let db = Database::create(temp_dir.path(), LightningDbConfig::default()).unwrap();
            let value = vec![0u8; value_size];
