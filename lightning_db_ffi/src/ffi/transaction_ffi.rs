...
#[no_mangle]
pub extern "C" fn lightning_db_begin_transaction(db_handle: u64, out_handle: *mut u64) -> i32 {
    ...
    let (db_handle, tx_id) = match TRANSACTION_REGISTRY.remove(tx_handle) {
        ...
        None => {
            set_last_error(
                ...
                "Invalid transaction handle".to_string(),
            );
...
/// - key must be valid for key_len bytes
#[no_mangle]
pub extern "C" fn lightning_db_delete_tx(tx_handle: u64, key: *const u8, key_len: usize) -> i32 {
    if key.is_null() {
        set_last_error(
            ErrorCode::InvalidArgument,
            "Null pointer provided".to_string(),
