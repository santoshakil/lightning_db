...
#[no_mangle]
pub extern "C" fn lightning_db_create(
    path: *const c_char,
    out_handle: *mut u64,
) -> i32 {
    ...
    let path_str = ffi_try!(unsafe { c_str_to_string(path) }.map_err(|e| {
        lightning_db::Error::Generic(e)
    }));
    
    let db = ffi_try!(Database::create(Path::new(&path_str), LightningDbConfig::default()));
...
/// - Returns 0 on success with handle stored in out_handle, error code on failure
#[no_mangle]
pub extern "C" fn lightning_db_open(
    path: *const c_char,
    out_handle: *mut u64,
) -> i32 {
    if path.is_null() || out_handle.is_null() {
    ...
    let db = ffi_try!(Database::open(Path::new(&path_str), LightningDbConfig::default()));
    let handle = DATABASE_REGISTRY.insert(db);
...
#[no_mangle]
pub extern "C" fn lightning_db_get(
    handle: u64,
    key: *const u8,
    key_len: usize,
) -> ByteResult {
...
#[no_mangle]
pub extern "C" fn lightning_db_delete(
    handle: u64,
    key: *const u8,
    key_len: usize,
) -> i32 {
